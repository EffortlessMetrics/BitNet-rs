#!/usr/bin/env python3
"""Replace TL2 test function in ac1_quantized_linear_layers.rs"""
...
# New TL2 test implementation
new_tl2_test = '''/// AC1.4: TL2 Quantized Linear Layer Forward Pass Test
...
# Find and replace the TL2 test function
# Pattern: from "/// AC1.4:" to the closing brace "}" before the next test or EOF
pattern = r'/// AC1\.4:.*?^async fn test_ac1_tl2_quantized_linear_forward_pass\(\) -> Result<\(\)> \{.*?^\}'
...
content = re.sub(pattern, replacer, content, count=1, flags=re.DOTALL | re.MULTILINE)

if replacement_done:
    # Write the updated content
    with open('crates/bitnet-inference/tests/ac1_quantized_linear_layers.rs', 'w') as f:
        f.write(content)
    print("TL2 test successfully replaced!")
else:
    print("ERROR: Could not find TL2 test pattern to replace")
