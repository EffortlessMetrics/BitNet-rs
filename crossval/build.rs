...
fn compile_ffi() {
    ...
    if let Some(lib) = found_lib_dir {
        ...
        let lib_files = std::fs::read_dir(lib).unwrap_or_else(|_| panic!("Could not read lib directory"));
        ...
        for entry in lib_files.flatten() {
            ...
            if let Some(name) = path.file_stem().and_then(|s| s.to_str()) {
                ...
                if (name.starts_with("libbitnet") || name.starts_with("libllama") || name.starts_with("libggml")) 
                    && (path.extension().map_or(false, |ext| ext == "so" || ext == "dylib" || ext == "a")) {
        ...
        println!("cargo:warning=bitnet-crossval: Using mock C wrapper (no library directories found)");
    }
}