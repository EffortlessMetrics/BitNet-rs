...
fn smoke_env_preflight() {
    ...
    let model = env::var("CROSSVAL_GGUF")
        .expect("CROSSVAL_GGUF must be set to a valid GGUF file");
    ...
    assert!(
        ...
        "Library path not set to C++ build dirs: {}", ld
    );
}
...
fn smoke_first_token_logits_parity() {
    ...
    let model_path = env::var("CROSSVAL_GGUF")
        .expect("CROSSVAL_GGUF must be set");
    ...
    let cpp_dir = env::var("BITNET_CPP_DIR")
        .expect("BITNET_CPP_DIR must be set");
    assert!(
        Path::new(&cpp_dir).exists(),
        "C++ directory not found: {}", cpp_dir
    );
...
fn smoke_vocab_lock_validation() {
    ...
    // Read the lock file
    let lock_content = std::fs::read_to_string("crossval-models.lock.json")
        .expect("Failed to read lock file");
    let lock: serde_json::Value = serde_json::from_str(&lock_content)
        .expect("Invalid JSON in lock file");
    
    // Get expected vocab size
    let expected_vocab = lock["crossval_default"]["n_vocab"]
        .as_u64()
        .expect("Missing n_vocab in lock file");
