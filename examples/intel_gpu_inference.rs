...
fn run_gpu_demo(
    ...
    let embedding_table: Vec<f32> =
        (0..vocab_size * hidden_dim).map(|i| ((i as f32 * 0.0137).sin() * 0.1)).collect();
    ...
    let layer_weights: Vec<Vec<u8>> = (0..num_layers)
        .map(|layer| (0..packed_per_layer).map(|i| ((i + layer * 7 + 13) % 256) as u8).collect())
        .collect();
    ...
    for step in 0..max_tokens {
        ...
        let mut hidden: Vec<f32> =
            embedding_table[last_token * hidden_dim..(last_token + 1) * hidden_dim].to_vec();
