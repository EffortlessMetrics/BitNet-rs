...
## 2024-05-18 - Making scrollable output containers accessible
**Learning:** In HTML, `<label>` tags are strictly reserved for labellable form controls. To make scrollable, non-interactive generic containers (like divs with overflow-y: auto) accessible, using a label is invalid HTML and fails accessibility checks.
**Action:** Use a styled `div` for the text label and link it to the container via `aria-labelledby`, while ensuring the container has `tabindex="0"` and `role="region"`.
