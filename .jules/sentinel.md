...
**Vulnerability:** The `validate_model_request` function in `bitnet-server` validated model paths by checking string endings (e.g. `.ends_with(".gguf")`), but did not check for null bytes (`\0`).
**Learning:** When passing user-provided strings from Rust to underlying C APIs (like `llama.cpp`), string validations such as `.ends_with()` can be bypassed because the C API will truncate the path at the null byte. An attacker could request `/etc/passwd\0.gguf`, passing the Rust extension check, while the C API opens `/etc/passwd`.
**Prevention:** Always explicitly reject null bytes (`\0`) in file paths or inputs that will eventually be processed by C or OS-level APIs.
