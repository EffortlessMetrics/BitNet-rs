...
## 2024-04-21 - [Path Truncation Vulnerability via Null Bytes]
**Vulnerability:** The validate_model_request function validated file extensions (e.g., .gguf) without checking for null bytes (\0). An attacker could supply a path like /etc/passwd\0.gguf, which passes the Rust string extension check but gets truncated at the null byte by underlying C/OS APIs, potentially allowing arbitrary file access.
**Learning:** String validation functions like .ends_with() operate on the full Rust string, which can safely contain null bytes, whereas C APIs and the OS filesystem layer treat the null byte as a string terminator.
**Prevention:** Always explicitly reject null bytes (\0) when validating file paths derived from user input before performing suffix checks or passing them to file operations.
