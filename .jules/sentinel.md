...
**Vulnerability:** The model path validation in `SecurityValidator::validate_model_request` was vulnerable to path truncation. An attacker could inject a null byte (`\0`) into the file path (e.g., `sensitive_data.txt\0.gguf`). Standard string methods like `.ends_with(".gguf")` would pass, but underlying C/OS APIs (such as those in `llama.cpp`) would truncate the string at the null byte and read `sensitive_data.txt`.
**Learning:** Checking file extensions on paths derived from user input is insufficient if the underlying system APIs treat strings differently (e.g., C-style null-terminated strings vs. Rust strings with explicit lengths).
**Prevention:** Explicitly reject null bytes (`\0`) when validating any file paths or system inputs derived from user input.
