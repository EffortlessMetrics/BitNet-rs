...
## 2025-01-23 - Predictable API Key Generation
**Vulnerability:** API keys and request IDs were generated using predictable timestamps (`SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos()`) formatted as hex.
**Learning:** Using time-based values for security-sensitive tokens makes them vulnerable to brute-force prediction attacks, allowing an attacker to generate valid keys if they know approximately when the key was created.
**Prevention:** Always use a cryptographically secure random number generator, such as `uuid::Uuid::new_v4()`, for generating security tokens like API keys.
