...
## 2024-05-24 - Path Truncation Vulnerability in Model Loading
**Vulnerability:** The `validate_model_request` function in `bitnet-server` did not explicitly reject null bytes (`\0`) in the model path string, exposing the application to path truncation vulnerabilities when underlying C/C++ libraries (like llama.cpp) process the path.
**Learning:** Standard string validation methods in Rust like `.ends_with(".gguf")` or directory containment checks (like `starts_with`) only operate on the Rust string's length, oblivious to the fact that C APIs will terminate processing at the first null byte. This disconnect allows an attacker to bypass file extension and directory restrictions by appending a null byte followed by allowed extensions.
**Prevention:** Always explicitly check for and reject null bytes (`\0`) when validating or sanitizing file paths derived from user input before they are passed to FFI or underlying OS C APIs.
