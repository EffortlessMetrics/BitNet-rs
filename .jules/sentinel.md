...
## 2026-05-03 - Path Truncation Vulnerability in Model Validation
**Vulnerability:** The `validate_model_request` function in `bitnet-server` did not explicitly reject null bytes (`\0`) in the model path. While `.ends_with(".gguf")` validation occurred in Rust, passing a truncated path via C/OS APIs (like `llama.cpp`) could bypass the extension validation and allow access to arbitrary files.
**Learning:** Standard string validation methods in Rust (like `.ends_with()`) are insufficient to prevent Path Truncation vulnerabilities when the strings are eventually passed to underlying C/OS APIs that treat null bytes as string terminators.
**Prevention:** Always explicitly reject null bytes (`\0`) in file path validation logic, especially when paths are derived from user input or passed to FFI/C libraries.
