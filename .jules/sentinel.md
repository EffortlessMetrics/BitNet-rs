...
**Vulnerability:** The server validated model paths by checking for directory traversal sequences (`..`, `~`) and verifying the file extension (e.g., `.gguf`), but did not check for null bytes (`\0`). This allowed path truncation vulnerabilities where an attacker could provide a path like `malicious_file\0.gguf`. While the Rust string handles it gracefully and the extension check passes, underlying OS-level operations (like C-bindings or system calls) that expect null-terminated strings would truncate the path to `malicious_file`, bypassing the extension restriction.
**Learning:** When validating file paths from user input, checking for malicious characters like `..` and `~` or specific extensions is insufficient. The differences in how strings are handled between high-level languages (like Rust) and underlying OS implementations (C strings) can lead to bypasses.
**Prevention:** Always explicitly reject null bytes (`\0`) when validating user-provided file paths before passing them to OS-level operations.
