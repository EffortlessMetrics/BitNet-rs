...
**Vulnerability:** The `validate_model_request` function in `bitnet-server` checked for file extensions (e.g. `.gguf`) but failed to explicitly block null bytes (`\0`). This exposed the server to path truncation attacks where a payload like `/models/secret.txt\0.gguf` would pass the extension check but be truncated to `/models/secret.txt` when passed to C-level or underlying OS APIs.
**Learning:** Validating strings in higher-level languages (like Rust) before passing them to OS or C-level operations is dangerous if the boundary (such as string length/termination) is not consistent. A null byte is often ignored or treated as part of the string in Rust, but acts as a terminator in lower-level APIs.
**Prevention:** Always explicitly check for and block null bytes (`\0`) when validating user input that will be used as a file path, even when checking for specific extensions or parent directories.
