...
**Vulnerability:** The `validate_model_request` function in `bitnet-server` did not check for null bytes (`\0`) in the model path. This allows attackers to bypass the `.gguf` extension check by appending a null byte followed by the required extension (e.g., `secret.txt\0.gguf`). Underlying C APIs (like llama.cpp) process the string only up to the null byte, resulting in the server loading an unintended file (`secret.txt`).
**Learning:** String validation methods in Rust (like `.ends_with()`) do not naturally stop at null bytes, unlike C strings. When validating paths that will eventually be passed to C/OS APIs, explicit rejection of null bytes is mandatory to prevent path truncation vulnerabilities.
**Prevention:** Always explicitly check for and reject null bytes (`\0`) when validating file paths derived from user input, especially before they are passed to foreign functions or OS APIs.
