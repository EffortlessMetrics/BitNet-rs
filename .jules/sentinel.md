...
## 2024-05-18 - Prevent Path Truncation via Null Bytes in Model Paths
**Vulnerability:** The model path validation endpoint accepted null bytes (`\0`) in the user input. In Rust, standard `String` and `&str` operations do not terminate strings at null bytes. However, when these strings are passed down to underlying C/C++ libraries or OS system calls (e.g., via `CString` for libc `open`), everything after the null byte is truncated. An attacker could bypass extension constraints by supplying a malicious path like `../../../etc/passwd\0.gguf`. The Rust `.ends_with(".gguf")` check would pass, but the underlying OS call would truncate it to `../../../etc/passwd`.
**Learning:** String validation routines operating in higher-level languages (like Rust or JS) that pass paths down to lower-level system boundaries are vulnerable to C-string truncation attacks if null bytes are not explicitly rejected. Standard methods like `.ends_with()` or `contains()` operate on the full string buffer, ignoring the null byte's terminating behavior in lower-level contexts.
**Prevention:** Always explicitly check for and reject null bytes (`\0`) when validating or sanitizing user-supplied file paths, regardless of subsequent extension checks.
