...
## 2024-06-03 - Path Truncation via Null Bytes
**Vulnerability:** The server validated model paths based on allowed extensions and basic path traversal checks (`..`, `~`), but did not explicitly reject null bytes (`\0`). This could allow an attacker to bypass extension validation by truncating the path at the C level (e.g. `secret_model.bin\0.gguf`).
**Learning:** String validation in Rust that eventually passes paths to C or OS APIs must account for path truncation vulnerabilities by explicitly rejecting null bytes. Standard string checks like `.ends_with()` do not stop null bytes.
**Prevention:** Always explicitly check for and reject null bytes (`\0`) when validating strings that represent file paths, especially when relying on file extensions.
