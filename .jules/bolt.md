...
## 2025-01-24 - Division by power of constant in hot loop
**Learning:** In the `apply` method for repetition penalties, division by `self.count_penalty.powi(count)` occurs in a hot loop over `token_counts`. Although `count` varies per token, `self.count_penalty` is constant. Floating-point division is significantly slower than multiplication, and calculating `powi` unconditionally for both branches adds unnecessary overhead.
**Action:** Pre-calculate the inverse base (`1.0 / self.count_penalty`) entirely outside the loop. Inside the loop, compute `powi` lazily inside the respective branches, and multiply by the inverse power to safely eliminate the division overhead while maintaining O(log N) power scaling.
