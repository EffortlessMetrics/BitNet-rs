...
### Added
- **Property tests for `bitnet-runtime-profile-contract-core` and `bitnet-testing-policy-runtime`** (PR #743): 13 new proptest properties. `bitnet-runtime-profile-contract-core` (+7): `violations_consistent_with_is_supported` (is_supported ↔ empty violations), `missing_matches_violations_missing`, `forbidden_matches_violations_forbidden`, `from_context_preserves_scenario_and_environment`, `active_profile_for_is_deterministic`. `bitnet-testing-policy-runtime` (+6): `from_context_preserves_scenario_and_env`, `has_grid_cell_agrees_with_active_profile`, `summary_contains_expected_substrings`, `runtime_state_is_deterministic`. Workspace total: **3,485 tests, all passing**. Proptest coverage spans **35 crates**.
- **Fuzz targets for `bitnet-logits` and `bitnet-generation`** (PR #740): `logits_transforms.rs` fuzzes all 6 public functions in `bitnet-logits` (apply_repetition_penalty, apply_temperature, apply_top_k, softmax_in_place, apply_top_p, argmax) with arbitrary f32 data including NaN/infinity. `generation_stop_check.rs` fuzzes `bitnet_generation::check_stop()` with arbitrary StopCriteria, token IDs, and decoded tail text. Fuzz target total: **13 targets**.
- **Property tests for `bitnet-runtime-context-core`** (PR #740): 8 new tests covering TestingScenario/ExecutionEnvironment Display→FromStr round-trips, from_env_with_defaults default precedence (serial+temp-env), parse-error-not-panic on garbage strings.
