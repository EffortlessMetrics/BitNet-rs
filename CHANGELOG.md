...
### Waves 133–134: OpenCL Kernel Fleet

#### Wave 134

- **OpenCL position encoding variants** (#2558): sinusoidal, learned, RoPE, ALiBi, T5-relative, NTK-RoPE, and YaRN position encodings for OpenCL backend — 97 tests
- **OpenCL KV cache compressor** (#2542): INT8/INT4 quantized KV cache with eviction policies, token merging, and sink token retention — 78 tests
- **OpenCL batch scheduler** (#2553): FCFS, SJF, priority, and fair-share scheduling with preemption and admission control — 70 tests
- **OpenCL tensor parallel communication** (#2555): all-reduce, all-gather, reduce-scatter, and broadcast collective operations for tensor parallelism — 73 tests
- **OpenCL operator fusion optimizer** (#2557): pattern-matching graph transformation engine with A770-specific fusion rules — 91 tests

#### Wave 133

- **OpenCL quantization calibrator** (#2554): per-tensor, per-channel, and per-group calibration with MinMax, Percentile, MSE, and KL-divergence range methods — 74 tests
- **OpenCL token sampling strategies** (#2564): greedy, top-k, top-p, min-p, typical, and Mirostat sampling with composable penalty chain — 112 tests
- **OpenCL tensor serialization** (#2539): binary tensor format with LZ4/Zstd compression and CRC/XXHash integrity validation — 90 tests
- **OpenCL attention masks** (#2529): causal, padding, sliding-window, prefix, and block-sparse masks with mask combiners — 87 tests
- **CHANGELOG waves 129–132** (#2526): documentation update for OpenCL waves 129–132
