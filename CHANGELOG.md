...
- **Property tests for `bitnet-runtime-profile-contract-core` and `bitnet-testing-policy-runtime`** (PR #743): 13 new proptest properties. `bitnet-runtime-profile-contract-core` (+7): snapshot coherence, violation detection, feature-set completeness. `bitnet-testing-policy-runtime` (+6): `GridCompatibility` invariants. Proptest: 33→35 crates; workspace: 3,472→3,485 tests.
