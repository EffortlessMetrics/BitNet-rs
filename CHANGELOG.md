...
### Wave 136: OpenCL Embeddings, Tensors & Validation

- **OpenCL embedding quantization** (#2579): INT8/INT4 embedding quantization with product quantization and binary embeddings — 78 tests
- **OpenCL tensor concatenation** (#2584): concat/split/chunk/stack/unstack/slice tensor operations for OpenCL — 85 tests
- **OpenCL model validator** (#2586): tensor, architecture, health, and compatibility validation checks — 89 tests
- **OpenCL stop sequence detector** (#2594): Aho-Corasick multi-pattern matching with repeat detection and content filter — 67 tests
- **CHANGELOG waves 133-134** (#2568): Documentation update for waves 133-134

### Wave 135: OpenCL Sparse Ops, Memory & Profiling

- **OpenCL sparse matrix operations** (#2580): CSR/CSC/COO sparse formats with sparse-dense matmul and pruning masks — 95 tests
- **OpenCL memory bandwidth optimizer** (#2585): coalesced access patterns, vectorized loads, SLM tiling, and prefetch hints — 94 tests
- **OpenCL inference profiler** (#2577): hierarchical span tracking with Chrome trace export and bottleneck analysis — 94 tests
- **OpenCL error taxonomy** (#2583): OpenCL error code mapping with recovery strategies and circuit breaker — 75 tests
- **OpenCL workgroup size optimizer** (#2593): A770 occupancy tuning with register pressure and SLM capacity planning — 86 tests
