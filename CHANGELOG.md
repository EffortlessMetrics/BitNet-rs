...
### Added
- **Snapshot Tests for 5 Thin Policy Wrapper Crates** (PR #712): Adds `insta` snapshot coverage for the remaining policy and testing façade microcrates — `bitnet-runtime-profile-contract-core` (4 snapshots: `canonical_grid` row count, `active_profile_summary` starts with `scenario=`, Unit/Local has a grid cell, Unit/Local has no missing features in CPU mode), `bitnet-testing-profile` (2 snapshots: conversion helpers are identity no-ops, grid row count), `bitnet-testing-policy-contract` (3 snapshots: `PolicyContract::detect()` struct prefix, `feature_contract_snapshot` self-consistency, `drift_check()` matches consistency), `bitnet-testing-policy-interop` (3 snapshots: `Environment` alias, grid row count, `validate_unit_local` returns `Some`), `bitnet-testing-policy-tests` (3 snapshots: `GridScenario`/`GridEnvironment` aliases, `PolicyDiagnostics::from_context` has cell). Adds 15 snapshot tests total.
- **Snapshot Tests for `bitnet-st2gguf`, `bitnet-testing-policy-runtime`, `bitnet-testing-policy-kit`** (PR #711): Adds `insta` to dev-deps and creates `snapshot_tests.rs` for three more crates — `bitnet-st2gguf` (6 snapshots: `is_layernorm_tensor` true for 8 LN tensor names and false for 4 projection names, `count_layernorm_tensors` on a representative 14-tensor model, `TensorDType` GGUF type IDs F32=0/F16=1, element sizes F32=4/F16=2, `MetadataValue` Debug variant stability), `bitnet-testing-policy-runtime` (3 snapshots: `RuntimePolicyState::has_grid_cell` for Unit/Local, `summary()` bracket format, `active_profile_for` scenario round-trip), `bitnet-testing-policy-kit` (3 snapshots: type alias correctness, `active_feature_labels` count, `profile_snapshot` scenario/environment consistency). Adds 12 snapshot tests total.
