...
impl ToleranceValidator {
    ...
    fn is_within_tolerance(&self, difference: f64) -> bool {
        difference <= self.tolerance.float_tolerance
    }
    ...
    pub fn validate_float_arrays(
        ...
        for (a, b) in arr1.iter().zip(arr2.iter()) {
            ...
            if self.is_within_tolerance(diff) {
                within_tolerance_count += 1;
            }
        }

        let passes_tolerance = self.is_within_tolerance(max_difference);
...
mod tests {
    ...
    #[test]
    fn test_tolerance_validation_core_functionality() {
        // Test the core 1e-6 tolerance validation functionality
        let tolerance = ComparisonTolerance::default();
        let validator = ToleranceValidator::new(tolerance);

        // Test cases that clearly demonstrate 1e-6 tolerance validation
        let test_cases = vec![
            // (difference, should_pass, description)
            (0.0, true, "Exact match"),
            (1e-7, true, "One order of magnitude smaller"),
            (5e-7, true, "Half the tolerance"),
            (2e-6, false, "Double the tolerance"),
            (1e-5, false, "One order of magnitude larger"),
        ];

        for (difference, should_pass, description) in test_cases {
            let arr1 = vec![0.5];
            ...
            println!(
                "✓ {}: difference={:.2e}, passes={}",
                description, difference, result.passes_tolerance
            );
        }
    }
...
pub fn demonstrate_1e6_tolerance_validation() {
    ...
    println!(
        "3. Exceeds tolerance (2e-6): passes={}, max_diff={:.2e}",
        result.passes_tolerance, result.max_difference
    );
    ...
    fn test_complete_1e6_tolerance_workflow() {
        ...
        for (test_name, difference, should_pass) in test_cases {
            ...
            println!(
                ...
                test_name, difference, result.passes_tolerance
            );
        }
