...
use bitnet_tests::common::{
    harness::{TestHarness, ConsoleReporter},
    config::TestConfig,
};
...
async fn test_resource_management_suite() {
    ...
    match result {
        Ok(suite_result) => {
            ...
            assert!(suite_result.summary.success_rate >= 80.0, 
...
async fn test_memory_leak_detection() {
    ...
    use bitnet_tests::common::{fixtures::FixtureManager, config::FixtureConfig};
    
    let _ = tracing_subscriber::fmt::try_init();
    
    let fixture_config = FixtureConfig::default();
    let fixtures = FixtureManager::new(&fixture_config).await.expect("Failed to create fixture manager");
