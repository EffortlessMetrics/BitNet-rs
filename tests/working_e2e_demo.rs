...
fn test_multi_quantizer_comparison() {
    ...
    for (name, quantizer) in quantizers {
        ...
        assert_eq!(recovered.shape(), tensor.shape(), 
            "{} should preserve tensor shape", name);
    }
}
...
trait TestQuantizer {
    fn test_quantize(&self, tensor: &MockTensor) -> Result<bitnet_quantization::QuantizedTensor, Box<dyn std::error::Error>>;
    fn test_dequantize(&self, quantized: &bitnet_quantization::QuantizedTensor) -> Result<bitnet_common::BitNetTensor, Box<dyn std::error::Error>>;
}
