...
impl ToleranceValidator {
    ...
    /// Check if a difference is within tolerance (accounting for floating point precision)
    fn is_within_tolerance(&self, difference: f64) -> bool {
        // Use a small epsilon to account for floating point precision issues
        difference <= self.tolerance.float_tolerance + f64::EPSILON
    }
...
mod tests {
    ...
    fn test_boundary_case_exactly_1e6() {
        ...
        assert!((result.max_difference - 1e-6).abs() < 1e-14);
        assert_eq!(result.within_tolerance_count, result.total_comparisons);
    ...
    fn test_various_tolerance_levels() {
        ...
        let test_cases = vec![
            ...
            (9.9e-7, true),  // Just under tolerance should pass
            (1.1e-6, false), // Slightly over tolerance should fail
    ...
    fn test_tolerance_precision_edge_cases() {
        ...
        let edge_cases = vec![
            (9.99999e-7, true),  // Just under 1e-6
            (1.00000e-6, true),  // Exactly 1e-6
            (1.00001e-6, false), // Just over 1e-6
        ];

        for (difference, should_pass) in edge_cases {
            let arr1 = vec![0.5];
            let arr2 = vec![0.5 + difference];
            ...
            assert_eq!(
                result.passes_tolerance,
                should_pass,
                "Difference {:.8e} should {} tolerance",
                difference,
                if should_pass { "pass" } else { "fail" }
            );
            println!(
...
mod integration_tests {
    ...
    fn test_complete_1e6_tolerance_workflow() {
        ...
        let test_cases = vec![
            ...
            ("at_boundary", 9.9e-7, true), // Use slightly under 1e-6 to avoid precision issues
