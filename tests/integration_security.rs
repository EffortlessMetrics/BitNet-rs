...
fn test_unsafe_code_documentation() {
    ...
    assert!(Path::new("unsafe_report.md").exists(), 
        "unsafe_report.md must exist to document all unsafe code");
    
    // Read the report
    let report = std::fs::read_to_string("unsafe_report.md")
        .expect("Failed to read unsafe_report.md");
    ...
    assert!(report.contains("Safety") || report.contains("SAFETY"), 
...
fn test_third_party_licenses() {
    ...
    assert!(Path::new("THIRD_PARTY.md").exists(), 
        "THIRD_PARTY.md must exist to document third-party licenses");
    
    // Read the license file
    let licenses = std::fs::read_to_string("THIRD_PARTY.md")
        .expect("Failed to read THIRD_PARTY.md");
    
    // Should not be empty
    assert!(!licenses.trim().is_empty(), 
        "THIRD_PARTY.md should not be empty");
    
    // Should contain license information
    assert!(licenses.contains("License") || licenses.contains("LICENSE"), 
...
fn test_deny_configuration() {
    assert!(Path::new("deny.toml").exists(), 
        "deny.toml must exist for supply chain security");
    
    let config = std::fs::read_to_string("deny.toml")
...
fn test_model_security_verification() {
    use bitnet_models::security::{ModelVerifier, ModelSecurity};
...
fn test_fuzzing_setup() {
    assert!(Path::new("fuzz").exists(), 
        "fuzz directory should exist");
    ...
    for target in &fuzz_targets {
        assert!(Path::new(target).exists(), 
            "Fuzz target {} should exist", target);
    }
}
...
fn test_security_workflow() {
    assert!(Path::new(".github/workflows/security.yml").exists(), 
...
fn test_performance_regression_detection() {
    ...
    assert!(duration.as_millis() < 1000, 
        "Quantization took too long: {:?}", duration);
}
...
impl bitnet_quantization::Quantize for MockTensor {
    fn quantize(&self, qtype: bitnet_quantization::QuantizationType) -> Result<bitnet_quantization::QuantizedTensor, bitnet_quantization::BitNetError> {
