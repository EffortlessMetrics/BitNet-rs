...
mod model_format_compatibility {
    ...
    fn validate_safetensors_format(model_path: &str) -> Result<(), String> {
        ...
        if size == 0 || size > 1024 * 1024 { // Reasonable header size limit
...
mod performance_compatibility {
    ...
    fn test_model_loading_memory() {
        ...
        assert!(memory_used < 10 * 1024 * 1024 * 1024, "Model uses too much memory"); // 10GB limit
    }
    ...
    fn test_inference_memory() {
        ...
        assert!(memory_used < 1024 * 1024 * 1024, "Inference uses too much memory"); // 1GB limit
    }
    ...
    fn test_memory_cleanup() {
        ...
        assert!(memory_diff.abs() < 100 * 1024 * 1024, "Memory not properly cleaned up"); // 100MB tolerance
    }
