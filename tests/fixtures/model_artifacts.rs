...
impl MockBitNetModel {
    ...
    pub fn create_small_bitnet() -> Self {
        ...
        for layer in 0..8 {
            // Attention weights
            mock_tensors.insert(
                format!("layers.{}.self_attn.q_proj.weight", layer),
                MockTensor {
            ...
            mock_tensors.insert(
                format!("layers.{}.self_attn.k_proj.weight", layer),
                MockTensor {
            ...
            mock_tensors.insert(
                format!("layers.{}.self_attn.v_proj.weight", layer),
                MockTensor {
            ...
            mock_tensors.insert(
                format!("layers.{}.mlp.gate_proj.weight", layer),
                MockTensor {
            ...
            mock_tensors.insert(
                format!("layers.{}.mlp.up_proj.weight", layer),
                MockTensor {
            ...
            mock_tensors.insert(
                format!("layers.{}.mlp.down_proj.weight", layer),
                MockTensor {
        ...
        Self { config, mock_tensors, mock_metadata }
    }
    ...
    pub fn generate_mock_gguf(&self) -> Vec<u8> {
        ...
        self.add_kv_pair(
            ...
            &self.mock_metadata.attention_heads,
        );
        self.add_kv_pair(
            &mut buffer,
    ...
    fn add_kv_pair<T: Serialize>(&self, buffer: &mut Vec<u8>, key: &str, value: &T) {
        ...
        // Simple value serialization for testing - just use 42 as a placeholder
...
impl RealModelFixtures {
    ...
    pub fn new(config: &TestEnvironmentConfig) -> Self {
        ...
        Self { model_configs, cached_models: HashMap::new(), test_env_config: config.clone() }
    }
    ...
    pub async fn ensure_model_available(&mut self, model_id: &str) -> Result<PathBuf> {
        ...
        let config = self.get_model_config(model_id).ok_or_else(|| {
            BitNetError::Model(ModelError::NotFound { path: model_id.to_string() })
        })?;

        // Check if model exists at expected path
        if config.model_path.exists() {
            let model_path = config.model_path.clone();
            self.cached_models.insert(model_id.to_string(), model_path.clone());
        ...
        if std::env::var("CI").is_ok() && !config.model_path.exists() {
            return Err(BitNetError::Model(ModelError::NotFound {
                path: format!(
                    "Model {} not available in CI environment. Set BITNET_GGUF or use download-model xtask.",
                    model_id
                ),
            }));
        }
...
impl ModelFixtures {
    pub fn new(config: &TestEnvironmentConfig) -> Self {
        ...
        Self { mock_models, real_models: RealModelFixtures::new(config), config: config.clone() }
    }
    ...
    async fn create_mock_files(&self) -> Result<()> {
        ...
        fs::create_dir_all("tests/fixtures").await.map_err(BitNetError::Io)?;

        for (name, mock_model) in &self.mock_models {
            // Generate mock GGUF file
            let gguf_content = mock_model.generate_mock_gguf();
            fs::write(&mock_model.config.model_path, gguf_content)
                .await
            ...
            if let Some(tokenizer_path) = &mock_model.config.tokenizer_path {
                ...
                fs::write(tokenizer_path, tokenizer_content).await.map_err(BitNetError::Io)?;
            }
    ...
    fn generate_mock_tokenizer_json(&self, _config: &TestModelConfig) -> String {
        serde_json::json!({
            "version": "1.0",
            "truncation": null,
            "padding": null,
    ...
    pub async fn get_model_for_tier(&mut self, tier: TestTier) -> Result<PathBuf> {
        match tier {
            TestTier::Fast => {
                ...
                Ok(self
                    .get_mock_model("small")
                    .ok_or_else(|| {
                        BitNetError::Model(ModelError::NotFound { path: "mock/small".to_string() })
                    })?
                    .config
                    .model_path
                    .clone())
            ...
            TestTier::Standard | TestTier::Full => {
                ...
                if let Ok(real_model_path) =
                    self.real_models.ensure_model_available("bitnet-2b").await
                {
                    Ok(real_model_path)
                } else {
                    ...
                    Ok(self
                        .get_mock_model("small")
                        .ok_or_else(|| {
                            BitNetError::Model(ModelError::NotFound {
    ...
    pub async fn cleanup(&mut self) -> Result<()> {
        ...
        for mock_model in self.mock_models.values() {
            if mock_model.config.model_path.exists() {
                fs::remove_file(&mock_model.config.model_path).await.map_err(BitNetError::Io)?;
            }

            if let Some(tokenizer_path) = &mock_model.config.tokenizer_path {
                if tokenizer_path.exists() {
                    fs::remove_file(tokenizer_path).await.map_err(BitNetError::Io)?;
                }
            }
        }
