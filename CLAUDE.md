...
- **3,472 tests run, all pass** in a normal `cargo nextest run --workspace --no-default-features --features cpu` run
...
These test suites pass reliably (3,472 tests run: 3,472 passed):
...
- **property tests**: Randomised invariants via proptest (33 crates, 196+ properties)
...
- **3,472 tests run: 3,472 passed** in `cargo nextest run --workspace --no-default-features --features cpu`
