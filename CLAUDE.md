...
- **3,485 tests run, all pass** in a normal `cargo nextest run --workspace --no-default-features --features cpu` run
...
These test suites pass reliably (3,485 tests run: 3,485 passed):
...
- **property tests**: Randomised invariants via proptest (35 crates, 209+ properties)
...
- **3,485 tests run: 3,485 passed** in `cargo nextest run --workspace --no-default-features --features cpu`
