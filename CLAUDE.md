...
- **3,493 tests run, all pass** in a normal `cargo nextest run --workspace --no-default-features --features cpu` run
...
These test suites pass reliably (3,493 tests run: 3,493 passed):
...
- **property tests**: Randomised invariants via proptest (36 crates, 221+ properties)
...
- **3,493 tests run: 3,493 passed** in `cargo nextest run --workspace --no-default-features --features cpu`
