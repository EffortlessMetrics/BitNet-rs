...
- **3,464 tests run, all pass** in a normal `cargo nextest run --workspace --no-default-features --features cpu` run
...
These test suites pass reliably (3,464 tests run: 3,464 passed):
...
- **property tests**: Randomised invariants via proptest (32 crates, 188+ properties)
...
- **3,464 tests run: 3,464 passed** in `cargo nextest run --workspace --no-default-features --features cpu`
