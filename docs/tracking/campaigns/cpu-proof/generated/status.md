...
| CPU-ANSWER-004 | in_progress | TBD | `codex/cpu-answer-004-bitnet-chat-prompt-parity` | `codex_premerge` | `automerge_when_green` | `on_blocker_only` | The Rust BitNet answer path can render the Microsoft BitNet.cpp reference chat envelope, use externally supplied llama-bpe tokenizer authority, record prompt-render metadata, and prove the math_2_plus_2 prompt token IDs match the known-good reference runner before classifying the first generated-token divergence as downstream of prompt/template/tokenizer. |
