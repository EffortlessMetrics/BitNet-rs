...
| Qwen3 0.6B | product CLI ready for bounded ask/chat user paths after artifact, CPU sanity, all-layer plan, one-token, short-decode, warm-session, benchmark-review, ask user-path, chat user-path, and product-promotion review evidence; speed, server, full-residency, broad dense GGUF, Qwen2.5 inheritance, and BitNet QK256 claims remain false | `ci/hardware/windows-9950x3d-rtx5070ti/2026-05-18/qwen3-0_6b-chat-user-path-cuda.json` | separate server smoke/readiness review or exact-profile performance comparator evidence before any server, speed, benchmark-qualified, or full-residency promotion |
