...
| CUDA-MODEL-SMOLLM2-002 | pending | Syncs the CUDA/model coverage view to the SLM-CPU-017 strict CPU preflight blocker: SmolLM2 360M reached strict CPU model-load preflight on the 9950X3D, then failed closed at the LayerNorm gamma guard before tokenizer/prompt/generation. The next proof is an SLM CPU loader policy follow-up, not CUDA. |
