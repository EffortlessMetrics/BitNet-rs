...
| CUDA-DENSE-032 | pr_open | Define the full one-layer CPU reference harness contract so the next dense CUDA proof can compare the CUDA-routable one-layer plan against deterministic CPU-reference ordering, tolerances, and receipt fields without claiming dense inference, token/decode/chat, speedup, or full residency. |
