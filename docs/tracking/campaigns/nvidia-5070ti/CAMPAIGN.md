...
| SmolLM2 360M | structurally valid artifact contract; strict CPU preflight is blocked before tokenizer/prompt/generation | `ci/slm-cpu/windows-9950x3d-rtx5070ti/2026-05-15/smollm2-360m-strict-cpu-preflight-blocker.json` | resolve strict loader/model-family normalization policy or record a governed exception before CPU sanity, all-layer planning, or CUDA |
...
| CUDA-MODEL-SMOLLM2-002 | pr_open | PR #5048 syncs the CUDA/model coverage view to the SLM-CPU-017 strict CPU preflight blocker: SmolLM2 360M reached strict CPU model-load preflight on the 9950X3D, then failed closed at the LayerNorm gamma guard before tokenizer/prompt/generation. The next proof is an SLM CPU loader policy follow-up, not CUDA. |
