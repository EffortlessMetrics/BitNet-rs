...
| Qwen3 0.6B | product CLI ready for bounded ask/chat user paths after artifact, CPU sanity, all-layer plan, one-token, short-decode, warm-session, benchmark-review, ask user-path, chat user-path, and product-promotion review evidence; CUDA-MODEL-014 rejected server-readiness promotion because no current-source Qwen3 server-smoke receipt is committed; speed, server, full-residency, broad dense GGUF, Qwen2.5 inheritance, and BitNet QK256 claims remain false | `ci/hardware/windows-9950x3d-rtx5070ti/2026-05-18/qwen3-0_6b-chat-user-path-cuda.json` | current-source Qwen3 non-streaming `/v1/chat/completions` server-smoke receipt before any exact-profile server-readiness promotion; exact-profile performance comparator evidence before speed, benchmark-qualified, or full-residency promotion |
