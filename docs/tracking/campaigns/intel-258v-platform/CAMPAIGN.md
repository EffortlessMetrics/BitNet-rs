...
| CPU258V-021 | merged | Instrument or script the external BitNet reference boundary so generated-token IDs and first-token logits/top-k are captured when available, or blocked with precise evidence when the reference cannot expose them; landed in #4315. |
