...
| CPU258V-028 | pr_open | Fix the metadata-authoritative BitNet prompt policy mismatch identified by CPU258V-027 so fixed prompt strings and token IDs match the external HF `apply_chat_template` boundary; open in #4512. |
