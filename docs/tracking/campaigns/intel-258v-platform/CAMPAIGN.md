...
| CPU-BITNET-REF-003 | in_progress | Record direct Microsoft BitNet.cpp generated-token IDs and first-token top-k/logit evidence for the fixed 258V prompts, then rerun the first-token divergence classifier without broad answer-quality, speed, Arc/NPU, QK256, or full-correctness claims. |
