...
| LNL258V-ASK-002 | pr_open | Restore the live dense Qwen2.5 CPU operator ask route for bounded math prompts, preserve dedicated `output.weight` receipt identity, add an optional `--expect-contains` answer gate, and commit live 258V ask receipts without claiming broad dense SLM quality, speedup, Arc/NPU execution, acceleration, or BitNet QK256/I2_S proof; open in #4654. |
