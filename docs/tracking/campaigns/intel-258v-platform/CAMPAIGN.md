...
| CPU258V-028 | ready | Fix the metadata-authoritative BitNet prompt policy mismatch identified by CPU258V-027 so fixed prompt strings and token IDs match the external HF `apply_chat_template` boundary. |
