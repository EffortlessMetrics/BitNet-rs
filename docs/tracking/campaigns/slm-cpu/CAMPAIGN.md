...
| SLM-CPU-021 | in_progress | Diagnose the SmolLM2 wrong-first-token blocker from committed reference-runner, prompt/tokenizer, and strict CPU retry evidence; CPU answer readiness and CUDA planning remain blocked until a reference-compatible first-token/top-k or checkpoint comparator localizes the fault. |
