...
| NPU-011 | ready | TBD | `codex/intel-npu/NPU-011-ffn-subgraph-parity` | `codex_premerge` | `automerge_when_green` | `on_blocker_only` | Add the next selected static BitNet-shaped OpenVINO NPU subgraph parity experiment, focused on an FFN/ReLU2 block or another explicitly scoped post-linear BitNet operation, with CPU reference comparison, static shape metadata, tolerance, timing, selected backend/runtime identity, fallback_used=false, and no full inference, acceleration, QK256, or CPU fallback proof claims. |
