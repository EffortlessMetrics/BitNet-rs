...
| M3MBA-017 | in_progress | Align M3 Air device identity help and rejection surfaces across CLI config and Mac wrappers. |

Current focus: `M3MBA-017` closes the remaining device-model discoverability
gap after `M3MBA-016`. After it lands, `M3MBA-007` is the next bounded M3 Air
artifact work surface, with `M3MBA-008` reserved for any accepted-artifact
handoff. `M3MBA-006` is
