...
The CUDA hardware proof ladder through `RTX5070TI-008` is receipt-backed, and `CUDA-BITNET-001` through `CUDA-BITNET-006` now cover the persistent CUDA substrate, reusable I2S primitive, real QK256 fused dequant GEMV kernel, load-time CUDA weight handling, transformer BitNetLinear routing, and a strict one-token proof. The current strict proof is `ci/hardware/windows-9950x3d-rtx5070ti/2026-05-06/strict-bitnet-cuda-proof.json`; it records `selected_backend=nvidia-rtx-5070-ti-cuda`, 210 QK256 CUDA invocations, zero BitNet linear CPU fallback, CPU/CUDA greedy and top-1 agreement, and `speedup_claim=false`. The receipt still records per-token weight upload in the routed proof path, so upload-once execution must be receipt-backed before optimization or throughput claims.

The next CUDA BitNet work item is `CUDA-BITNET-007` short decode proof. Do not claim multi-token CUDA decode or full performance speedup until the short decode and benchmark receipts land.
