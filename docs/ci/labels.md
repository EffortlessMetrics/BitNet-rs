...
**Note**: On `main` and `nightly` branches, receipt gates run strictly (blocking merge).
...
These workflows run on every PR and push to `main`/`nightly`:
...
**Runs on**: All PRs, all `main`/`nightly` pushes
...
- [Branch Protection](https://github.com/EffortlessMetrics/BitNet-rs/settings/branches) - Required checks
