...
- `bitnet_model_new_from_file()` → `llama_load_model_from_file()`
- `bitnet_eval()` → `llama_decode()` + `llama_get_logits()`
- `bitnet_prefill()` → `llama_decode()` (similar but only logits for last token)
- `bitnet_decode_greedy()` → Implements greedy token generation loop
