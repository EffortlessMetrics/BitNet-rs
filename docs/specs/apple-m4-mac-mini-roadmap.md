...
1. Multi-token CPU/NEON local-answer smoke prompts. The first command uses
   ...
   `bitnet mac ask` with the accepted GGUF plus external tokenizer.
   `M4-BITNET-ASK-001` adds one committed user-route receipt for that path with
   text `2+2 equals 4.`, generated token IDs, accepted model/tokenizer identity,
   `fallback_used=false`, and timing fields. BitNet remains disabled for
   `bitnet mac chat` and `bitnet mac serve`.
