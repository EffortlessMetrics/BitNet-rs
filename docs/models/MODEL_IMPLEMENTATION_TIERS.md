...
The tiers describe what can be responsibly attempted on current local hardware. They are planning labels, not performance estimates.
...
`design_only` is a positive state: it preserves safe rails without pretending the local machine validated the model.
