...
- name: Run benchmark with strict mode
  env:
    ...
    BITNET_DETERMINISTIC: "1"
    BITNET_SEED: "42"
  run: cargo run --no-default-features -p xtask -- benchmark --model model.gguf --tokens 128
...
|--------|--------|---------|---------|
| **Cosine Similarity** | ≥ 0.99 | Logit vector alignment | `cargo run --no-default-features -p xtask -- crossval --metric cosine` |
| **Exact Match Rate** | = 1.0 | Greedy decode token match (N=4) | `cargo run --no-default-features -p xtask -- crossval --metric exact-match` |
| **Max Logit Diff** | < 1e-4 | Largest per-token divergence | `cargo run --no-default-features -p xtask -- crossval --metric max-diff` |
| **Runtime Latency** | < 110% of C++ | Relative performance | `cargo run --no-default-features -p xtask -- crossval --metric latency` |
...
cargo run --no-default-features -p xtask -- crossval --model models/model.gguf --tokens 128
