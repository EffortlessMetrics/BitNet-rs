...
> **Last updated**: reflects implementation state after PRs #608–#755.
...
| docs: update test counts (3,493→3,520) and roadmap for PRs #749–#750; add `bitnet-test-support` and `bitnet-testing-scenarios-profile-core` to proptest crate list | `CLAUDE.md`, `CHANGELOG.md`, `docs/reference/dual-backend-roadmap.md` | #751 |
| security: `BITNET_ALLOWED_MODEL_DIRECTORIES` allow-list restricts model loading to permitted directories; unset = all paths allowed (backward-compatible) | `crates/bitnet-models/src/`, `crates/bitnet-inference/src/` | #753 |
| feat(wasm): keyboard navigation for WASM browser example tab bar; `tabindex`/`aria-selected` attributes for screen-reader compatibility | `examples/wasm-browser/` | #754 |
| chore: rename project BitNet.rs → BitNet-rs across 1,531 files (6,281 occurrences); crate names, docs, CI config, and all metadata updated | workspace-wide | #755 |
