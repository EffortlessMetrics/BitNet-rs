...
- Honest-compute receipt schema v1.0.0 validation (`xtask verify-receipt`).
- GPU kernel detection and validation infrastructure.
...
### 3) Validation & Receipt Infrastructure (Governance)

- **Done:** Receipt schema v1.0.0 implemented, portable tests, GPU kernel detection (`xtask verify-receipt`).
- **Ship:** Implement deterministic CPU and GPU microbenchmarks with receipt generation, and enforce quantized hot-paths.
- **Acceptance:** Receipts generated by real compute and successfully verified in CI workflow.

### 4) Benchmarks & Gating (Governance)
...
- Enforce quantized hot-path with debug assertions
...
- Add receipt fingerprinting for fast GPUs to prevent false positives
