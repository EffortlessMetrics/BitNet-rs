# CUDA-DENSE-025 One-Layer Attention-Softmax Route

`CUDA-DENSE-025` updates the dense GGUF one-layer planner state after
`CUDA-DENSE-024` proved strict RTX 5070 Ti CUDA attention-softmax parity for the
verified Qwen2.5 0.5B Q8_0 metadata-derived fixture.

The one-layer receipt now treats `attention_softmax` as
...
| `execution_plan.cuda_dense_regular_llm_ops` | `12` |
| `execution_plan.unsupported_ops` | `2` |
...
| `one_layer_plan.attention_softmax_cuda_ops_total` | `1` |
| `gap_audit.attention_score_cuda_parity_available` | `true` |
| `gap_audit.attention_softmax_cuda_parity_available` | `true` |
| `gap_audit.next_candidate_gap` | `attention_v_mix` |
| `fallback_used` | `false` |
| `speedup_claim` | `false` |
...
- The dense GGUF one-layer planner now routes `attention_softmax` to the dense
  regular-LLM CUDA route based on `CUDA-DENSE-024` parity evidence.
- The one-layer gap receipt records 12 CUDA-routable dense ops, including 7
  linears, 2 RMSNorm ops, 1 RoPE op, 1 attention-score op, and 1
  attention-softmax op.
- Attention V-mix is now the next governed dense one-layer gap candidate.
...
The next dense CUDA slice should target attention V-mix fixture extraction and
parity. It should remain below dense GGUF token/decode/chat, speedup, server,
and full-residency claims.
