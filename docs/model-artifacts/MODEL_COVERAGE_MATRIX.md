...
| `dense_qwen3_06b_q8_candidate` | Qwen3 0.6B Q8_0 GGUF | `product_cli_ready` | Product CLI-ready dense SLM row for bounded RTX 5070 Ti `dense_regular_llm_cuda` ask/chat user paths after artifact, CPU sanity, all-layer plan, one-token, short-decode, warm-session, benchmark-review, ask user-path, chat user-path, and product review receipts. CUDA-MODEL-014 rejected server-readiness promotion until a current-source Qwen3 non-streaming `/v1/chat/completions` server-smoke receipt is committed; speedup, benchmark-qualified speed, server, full-residency, broad dense GGUF, Qwen2.5 inheritance, and BitNet QK256 claims remain false. |
