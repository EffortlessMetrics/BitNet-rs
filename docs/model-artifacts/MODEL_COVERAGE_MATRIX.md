...
| `dense_smollm2_360m_candidate` | SmolLM2 360M GGUF candidate | `structurally_valid` | Exact artifact contract plus strict CPU preflight blocker, normalization-policy audit, exact metadata-scoped validation, strict CPU retry evidence, and wrong-first-token diagnosis. The retry reaches one-token generation but fails the math quality gate; a reference-compatible first-token/top-k or checkpoint comparator is required before CPU answer readiness or CUDA planning. |
