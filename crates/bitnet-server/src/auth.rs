...
mod tests {
    ...
    fn test_multiple_keys() {
        let mut store = KeyStore::new(AuthMode::Required);
        store.add_key("admin", "admin_key");
        store.add_key("user", "user_key");
        assert!(store.authenticate(Some("admin_key")).is_allowed());
        assert!(store.authenticate(Some("user_key")).is_allowed());
        assert!(!store.authenticate(Some("other")).is_allowed());
    }
}
