...
impl FleetMonitor {
    ...
    pub fn new(stale_threshold: Duration) -> Self {
        Self { devices: Arc::new(RwLock::new(HashMap::new())), stale_threshold }
    }
    ...
    pub fn heartbeat(&self, uid: &str, utilization: f64, used_memory_bytes: u64) -> bool {
    ...
    pub fn status(&self) -> FleetStatusResponse {
        ...
        let avg_util = if healthy > 0 { util_sum / healthy as f64 } else { 0.0 };
...
/// as JSON.
pub fn fleet_routes(monitor: Arc<FleetMonitor>) -> axum::Router {
    ...
    axum::Router::new().route("/api/v1/fleet/status", get(fleet_status_handler)).with_state(monitor)
}
...
mod tests {
    ...
    fn test_fleet_stats_aggregation() {
        ...
        assert_eq!(status.stats.total_memory_bytes, 24 * 1024 * 1024 * 1024);
        assert_eq!(status.stats.used_memory_bytes, 12 * 1024 * 1024 * 1024);
