...
impl Default for QueueConfig {
    fn default() -> Self {
        ...
        Self { max_per_priority: m, max_total: 960 }
    }
}
...
pub enum QueueError {
    ...
    PriorityQueueFull { priority: Priority, limit: usize },
...
impl PriorityRequestQueue {
    pub fn new(config: QueueConfig) -> Self {
        Self { inner: Arc::new(RwLock::new(Inner::new(config))) }
    }
    ...
    pub fn enqueue(&self, entry: QueueEntry) -> Result<(), QueueError> {
        ...
        if let Some(&limit) = inner.config.max_per_priority.get(key) {
            if count >= limit {
                return Err(QueueError::PriorityQueueFull { priority: entry.priority, limit });
            }
        }
        ...
        if total >= inner.config.max_total {
            return Err(QueueError::GlobalQueueFull { limit: inner.config.max_total });
        }
    ...
    pub fn depth(&self, priority: Priority) -> usize {
        let inner = self.inner.read().unwrap();
        inner.counts.get(priority_key(priority)).copied().unwrap_or(0)
    }
...
mod tests {
    ...
    fn entry_at(
        ...
        QueueEntry { id: id.to_string(), priority, deadline, enqueued_at, token_budget: 32 }
    }
    ...
    fn priority_ordering() {
        ...
        q.enqueue(entry_at("bg", Priority::Background, t, None)).unwrap();
        q.enqueue(entry_at("norm", Priority::Normal, t, None)).unwrap();
        q.enqueue(entry_at("inter", Priority::Interactive, t, None)).unwrap();
        q.enqueue(entry_at("batch", Priority::Batch, t, None)).unwrap();
    ...
    fn deadline_ordering_within_priority() {
        ...
        q.enqueue(entry_at("late", Priority::Normal, t, Some(dl_late))).unwrap();
        q.enqueue(entry_at("early", Priority::Normal, t, Some(dl_early))).unwrap();
    ...
    fn per_priority_backpressure() {
        ...
        cfg.max_per_priority.insert("Normal".into(), 2);
        cfg.max_total = 100;
        let q = PriorityRequestQueue::new(cfg);
        q.enqueue(entry("a", Priority::Normal)).unwrap();
        q.enqueue(entry("b", Priority::Normal)).unwrap();
        let err = q.enqueue(entry("c", Priority::Normal)).unwrap_err();
        assert_eq!(err, QueueError::PriorityQueueFull { priority: Priority::Normal, limit: 2 });
    ...
    fn global_backpressure() {
        ...
        let err = q.enqueue(entry("c", Priority::Interactive)).unwrap_err();
    ...
    fn deadline_miss_tracking() {
        ...
        q.enqueue(entry_at("expired", Priority::Normal, t, Some(past_deadline))).unwrap();
    ...
    fn wait_time_metric() {
        let q = PriorityRequestQueue::default();
        let t0 = Instant::now();
        q.enqueue(entry_at("a", Priority::Normal, t0, None)).unwrap();
    ...
    fn drain_expired_removes_past_deadline() {
        ...
        q.enqueue(entry_at("soon", Priority::Normal, t, Some(dl_soon))).unwrap();
        q.enqueue(entry_at("far", Priority::Normal, t, Some(dl_far))).unwrap();
        q.enqueue(entry_at("none", Priority::Normal, t, None)).unwrap();
