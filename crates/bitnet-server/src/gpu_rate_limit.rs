...
impl DeviceBucket {
    ...
    fn refill(&mut self, now: Instant) {
        ...
        self.tokens = (self.tokens + rate * elapsed).min(self.config.bucket_capacity);
    ...
    fn effective_rate(&self) -> f64 {
        if self.memory_utilisation > self.config.memory_threshold {
            ...
            // at 1.0 → 10% of rate.
            let excess = (self.memory_utilisation - self.config.memory_threshold)
    ...
    fn try_acquire(&mut self, now: Instant) -> RateLimitDecision {
        ...
        if self.active >= self.config.max_concurrent {
            return RateLimitDecision::Rejected {
                ...
                reason: format!("max concurrent requests ({}) reached", self.config.max_concurrent),
            };
        }
...
impl GpuRateLimiter {
    ...
    pub fn register_device(&self, device_id: &str, config: RateLimitConfig) {
        let mut inner = self.inner.write().unwrap();
        inner.insert(device_id.to_string(), DeviceBucket::new(config));
    }
    ...
    pub fn update_memory_utilisation(&self, device_id: &str, utilisation: f64) {
        let mut inner = self.inner.write().unwrap();
    ...
    pub fn check_at(&self, device_id: &str, now: Instant) -> RateLimitDecision {
        let mut inner = self.inner.write().unwrap();
...
mod tests {
    ...
    fn reject_at_max_concurrent() {
        ...
        assert_eq!(rl.check("g0"), RateLimitDecision::Allowed);
        let d = rl.check("g0");
        assert!(matches!(d, RateLimitDecision::Rejected { .. }), "3rd request should be rejected");
    }
    ...
    fn release_frees_slot() {
        ...
        assert_eq!(rl.check("g0"), RateLimitDecision::Allowed);
        assert!(matches!(rl.check("g0"), RateLimitDecision::Rejected { .. }));
        rl.release("g0");
        assert_eq!(rl.check("g0"), RateLimitDecision::Allowed);
    }
    ...
    fn token_bucket_exhaustion() {
        ...
        for _ in 0..10 {
            assert_eq!(rl.check_at("g0", now), RateLimitDecision::Allowed,);
            rl.release("g0");
        }
        // 11th should be rejected
        let d = rl.check_at("g0", now);
        assert!(matches!(d, RateLimitDecision::Rejected { .. }), "bucket should be exhausted");
    }
    ...
    fn tokens_refill_over_time() {
        ...
        // Drain bucket
        for _ in 0..10 {
            assert_eq!(rl.check_at("g0", t0), RateLimitDecision::Allowed,);
            rl.release("g0");
        }
        // After 1 second at 10 tok/s, 10 tokens should refill
        let t1 = t0 + Duration::from_secs(1);
        assert_eq!(rl.check_at("g0", t1), RateLimitDecision::Allowed,);
    }
    ...
    fn retry_after_in_rejection() {
        ...
        match rl.check("g0") {
            RateLimitDecision::Rejected { retry_after, .. } => {
                assert!(retry_after.as_millis() > 0, "retry_after should be non-zero");
            }
