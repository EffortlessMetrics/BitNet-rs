...
impl AbTestConfig {
    ...
    pub fn new(name: impl Into<String>) -> Self {
        Self {
            ...
            b_ratio: 0.5,
            metrics: vec!["latency".into(), "throughput".into(), "quality_score".into()],
            active: true,
        }
    }
...
impl VariantSamples {
    fn new() -> Self {
        Self { latencies_us: Vec::new(), throughputs: Vec::new(), quality_scores: Vec::new() }
    }

    fn push(&mut self, sample: &Sample) {
        self.latencies_us.push(sample.latency.as_micros() as u64);
...
mod tests {
    ...
    fn test_runner_records_and_counts() {
        ...
        runner.record(
            Variant::A,
            Sample { latency: Duration::from_millis(10), throughput: 100.0, quality_score: 0.9 },
        );
        runner.record(
            Variant::B,
            Sample { latency: Duration::from_millis(12), throughput: 95.0, quality_score: 0.85 },
        );
    ...
    fn test_significant_quality_difference() {
        ...
        assert!(results.quality_significant(), "p={} should be < 0.05", results.quality_p_value);
    }
}
