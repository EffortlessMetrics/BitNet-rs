...
mod tests {
    ...
    async fn test_quantization_optimization_allocation() {
        ...
        // 7 requests with "TL1", 3 with "I2S"
        for i in 0..10 {
            let gen_config = GenerationConfig::default();
            let hint = if i < 7 { "TL1" } else { "I2S" };
            let request = BatchRequest::new(format!("prompt {}", i), gen_config)
        ...
        assert!(optimization.is_some(), "Optimization should be returned");
        let opt = optimization.unwrap();

        assert_eq!(opt.quantization_type, "TL1");
        assert_eq!(opt.batch_compatible_requests.len(), 7);
    }
}
