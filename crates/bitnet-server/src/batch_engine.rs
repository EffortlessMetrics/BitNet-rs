...
mod tests {
    ...
    fn create_pending_request(hint: Option<&str>) -> PendingRequest {
    ...
    async fn test_optimize_batch_allocation_correctness() {
        ...
        let candidates = vec![
            create_pending_request(Some("I2S")),
            create_pending_request(Some("I2S")),
            create_pending_request(Some("TL1")),
        ];

        let opt = engine.optimize_batch_for_quantization(&candidates).await.unwrap();
        assert_eq!(opt.quantization_type, "I2S");
        assert_eq!(opt.batch_compatible_requests.len(), 2);
    }
}
