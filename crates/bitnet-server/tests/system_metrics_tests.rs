use bitnet_server::monitoring::{metrics::MetricsCollector, MonitoringConfig};
use anyhow::Result;
...
async fn test_system_metrics_collection() -> Result<()> {
    ...
    assert!(memory_info.usage_percent >= 0.0 && memory_info.usage_percent <= 100.0, "Memory usage percentage should be between 0 and 100");
...
async fn test_performance_regression_detection() -> Result<()> {
    ...
    assert!(alerts.is_empty() || !alerts.is_empty(), "Performance regression check should not panic");
