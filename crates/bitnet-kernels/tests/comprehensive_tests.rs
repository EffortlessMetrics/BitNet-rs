...
use bitnet_kernels::*;
use bitnet_common::{QuantizationType, KernelError, BitNetError, Result};
        ...
        if available_providers.len() > 1 {
            ...
            if results.len() > 1 {
                ...
                for (i, result) in results.iter().enumerate().skip(1) {
                    for (j, (&ref_val, &test_val)) in reference.iter().zip(result.iter()).enumerate() {
                        let diff = (ref_val - test_val).abs();
                        assert!(diff < 1e-5, 
                               "Kernel {} result differs from reference at position {}: {} vs {}", 
                               available_providers[i], j, test_val, ref_val);
                    }
                }
            }
        }
...
mod performance_tests {
    ...
    fn test_cache_efficiency() {
        ...
        let start = Instant::now();
        for _ in 0..100 {
            kernel.matmul_i2s(&a_small, &b_small, &mut c_small, small_size, small_size, small_size).unwrap();
        }
        ...
        let start = Instant::now();
        for _ in 0..10 {
            kernel.matmul_i2s(&a_large, &b_large, &mut c_large, large_size, large_size, large_size).unwrap();
        }
        let large_duration = start.elapsed();
        
        println!("Small matrix ({}x{}) 100 iterations: {:?}", small_size, small_size, small_duration);
        println!("Large matrix ({}x{}) 10 iterations: {:?}", large_size, large_size, large_duration);
...
mod integration_tests {
    ...
    fn test_end_to_end_quantization_workflow() {
        ...
        let model_weights: Vec<f32> = (0..1024)
        ...
        let start = Instant::now();
        let result = kernel.quantize(&model_weights, &mut quantized_weights, &mut scales, QuantizationType::I2S);
        ...
        let start = Instant::now();
        let result = kernel.matmul_i2s(&input, &quantized_weights[..32*32], &mut output, 32, 32, 32);
    ...
    fn test_multi_kernel_comparison() {
        ...
        if available_kernels.len() > 1 {
            ...
            if results.len() > 1 {
                let reference = &results[0];
                for (i, result) in results.iter().enumerate().skip(1) {
                    let max_diff = reference.iter()
                        .zip(result.iter())
                        .map(|(a, b)| (a - b).abs())
                        .fold(0.0f32, f32::max);
                    
                    println!("Max difference between {} and {}: {}", 
    ...
    fn test_real_world_simulation() {
        ...
        let start = Instant::now();
        let result = kernel.matmul_i2s(&inputs, &weights, &mut outputs, batch_size, output_dim, input_dim);
        let duration = start.elapsed();
        ...
        println!("  Batch size: {}, Input dim: {}, Output dim: {}", batch_size, input_dim, output_dim);
        println!("  Time: {:?}", duration);
        println!("  Performance: {:.2} GFLOPS", gflops);
