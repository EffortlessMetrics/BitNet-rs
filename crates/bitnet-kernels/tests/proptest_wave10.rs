...
proptest! {
    ...
    fn prop_sigmoid_range(input in finite_f32_vec(256)) {
        let output = activations::activate(&input, activations::ActivationType::Sigmoid);
    ...
    fn prop_relu_nonnegative(input in finite_f32_vec(256)) {
        let output = activations::activate(&input, activations::ActivationType::ReLU);
        for (i, &v) in output.iter().enumerate() {
    ...
    fn prop_tanh_range(input in finite_f32_vec(256)) {
        let output = activations::activate(&input, activations::ActivationType::Tanh);
        for (i, &v) in output.iter().enumerate() {
            prop_assert!(
                (-1.0..=1.0).contains(&v),
                "tanh[{i}]={v} not in [-1,1]"
    ...
    fn prop_sigmoid_monotone(input in finite_f32_vec(256)) {
        ...
        let output = activations::activate(&sorted, activations::ActivationType::Sigmoid);
    ...
    fn prop_gelu_lower_bound(input in finite_f32_vec(256)) {
        let output = activations::activate(&input, activations::ActivationType::GELU);
        for (i, &v) in output.iter().enumerate() {
            prop_assert!(
                v >= -0.2,
                "gelu[{i}]={v} below -0.2"
    ...
    fn prop_silu_at_zero_and_small(
        ...
        let output = activations::activate(&input, activations::ActivationType::SiLU);
        for (i, &v) in output.iter().enumerate() {
            prop_assert!(
                v.abs() < 0.02,
