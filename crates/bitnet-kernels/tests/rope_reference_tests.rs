...
fn build_freq_tables(max_seq_len: usize, head_dim: usize, base: f32) -> (Vec<f32>, Vec<f32>) {
    ...
    for seq_pos in 0..seq_len {
        for head_idx in 0..num_heads {
            for pair_idx in 0..half_dim {
                let base_idx = (seq_pos * num_heads + head_idx) * head_dim + 2 * pair_idx;
...
mod tests {
    ...
    fn test_rope_identity_at_position_zero() {
        ...
        rope_apply_reference(&mut x, &freq_cos, &freq_sin, seq_len, num_heads, head_dim, 0);
    ...
    fn test_rope_preserves_vector_norm() {
        ...
        let mut x: Vec<f32> =
            (0..(seq_len * num_heads * head_dim)).map(|i| (i as f32) * 0.1 - 1.0).collect();
    ...
    fn test_rope_different_positions_give_different_results() {
        ...
        let any_diff = x0.iter().zip(x5.iter()).any(|(a, b)| (a - b).abs() > 1e-6);
    ...
    fn test_rope_known_rotation_90_degrees() {
        ...
        assert!((x[0] - (-4.0)).abs() < 1e-6, "expected -4.0, got {}", x[0]);
        assert!((x[1] - 3.0).abs() < 1e-6, "expected 3.0, got {}", x[1]);
    }
}
