...
impl KernelSource {
    ...
    pub fn full_source(&self) -> String {
        ...
        if preamble.is_empty() { self.source.clone() } else { format!("{preamble}{}", self.source) }
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(loc) = &self.location {
            write!(
                f,
                "{}:{}:{}: {}: {}",
                loc.file, loc.line, loc.column, self.severity, self.message
            )
        } else {
...
impl KernelCompiler {
    ...
    fn validate_source(preprocessed: &str, name: Option<&str>) -> Option<CompilationError> {
        ...
        for (line_no, line) in trimmed.lines().enumerate() {
            for ch in line.chars() {
                ...
                if depth < 0 {
                    return Some(CompilationError::from_log(
                        format!("<kernel>:{}:0: error: unmatched closing brace", line_no + 1),
                        name.map(String::from),
                    ));
                }
            }
...
mod tests {
    ...
    fn test_diagnostic_display() {
        let d = Diagnostic {
            ...
            location: Some(DiagnosticLocation { file: "test.cl".into(), line: 5, column: 3 }),
            message: "bad".into(),
        };
    ...
    fn test_compile_reset_stats() {
        let mut compiler = KernelCompiler::with_default_cache("dev");
        let _ = compiler
            .compile(&KernelSource::new("__kernel void f() {}"), &CompilationOptions::new());
    ...
    fn test_compile_binary_size_estimation() {
        ...
        let mut compiler = KernelCompiler::with_default_cache("dev");
        let bin =
            compiler.compile(&KernelSource::new(source_text), &CompilationOptions::new()).unwrap();
    ...
    fn test_warmup_with_failure() {
        ...
        warmup.add(KernelSource::new("").with_name("bad"), CompilationOptions::new());
        let mut compiler = KernelCompiler::with_default_cache("dev");
        let summary = warmup.warmup(&mut compiler);
        assert_eq!(summary.successes(), 1);
    ...
    fn test_warmup_populates_cache() {
        ...
        let mut compiler = KernelCompiler::new("dev", cache.clone());
        let mut warmup = WarmupCompiler::new();
        warmup.add(KernelSource::new("__kernel void w() {}"), CompilationOptions::new());
    ...
    fn test_different_source_different_hash() {
        let hashes: HashSet<u64> =
            (0..20).map(|i| hash_str(&format!("__kernel void f{i}() {{}}"))).collect();
