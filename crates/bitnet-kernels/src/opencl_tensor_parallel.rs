                    ...
                    .map(|dev| {
                        ...
                        for r in 0..rows {
                            ...
                            shard.extend_from_slice(
                                &data[row_start + c_start..row_start + c_end],
                            );
                        }
                        shard
                    })
...
mod tests {
    ...
    fn make_devices(n: usize, len: usize) -> Vec<SimulatedDevice> {
        (0..n)
            .map(|i| {
                let data: Vec<f32> =
                    (0..len).map(|j| (i * len + j) as f32).collect();
    ...
    fn property_sum_div_n_equals_mean() {
        for n in [2, 3, 4, 5, 8] {
            ...
            let divided: Vec<f32> =
                sum_devs[0].data.iter().map(|v| v / n as f32).collect();
            assert!(
                approx_eq(&divided, &mean_devs[0].data, 1e-4),
                "failed for n={n}"
            );
        }
    }
    ...
    fn allreduce_sum_with_zeros() {
        let mut devs = vec![
            SimulatedDevice::new(0, vec![0.0; 4]),
            SimulatedDevice::new(1, vec![0.0; 4]),
        ];
        AllReduce::sum(&mut devs, CommunicationTopology::Ring);
        for d in &devs {
    ...
    fn column_shard_allgather_roundtrip() {
        ...
        let mut devs: Vec<SimulatedDevice> = shards
            .into_iter()
            .enumerate()
            .map(|(i, s)| SimulatedDevice::new(i, s))
            .collect();
        AllGather::run(&mut devs, CommunicationTopology::Ring);
