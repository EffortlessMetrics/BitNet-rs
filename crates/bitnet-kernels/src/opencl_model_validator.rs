...
impl TensorValidator {
        ...
        if let Some(data) = &tensor.data {
            ...
            if nan_count > 0 {
                report.add(ValidationFinding {
                    ...
                    recommendation: Some(
                        "re-export or re-train; NaN weights cause garbage output"
                            .into(),
                    ),
                });
            }
    ...
    pub fn check_range(
        ...
        if let Some(data) = &tensor.data {
            let oob: usize = data
                .iter()
                .filter(|v| !v.is_nan() && (**v < min_val || **v > max_val))
                .count();
            if oob > 0 {
                report.add(ValidationFinding {
...
impl ArchitectureValidator {
    ...
    pub fn check_tensor_names(
        ...
        let expected: HashSet<String> =
            spec.expected_tensor_names().into_iter().collect();
        ...
        for name in &expected {
            if !actual.contains(name) {
                report.add(ValidationFinding {
                    ...
                    recommendation: Some(
                        "ensure model file contains all required weights".into(),
                    ),
                });
            }
        }
    ...
    pub fn check_tensor_count(
        ...
        if actual < expected {
            report.add(ValidationFinding {
                ...
                tensor_name: None,
                message: format!(
                    "tensor count too low: expected {}, got {}",
                    expected, actual
                ),
            ...
            report.add(ValidationFinding {
                ...
                tensor_name: None,
                message: format!(
                    "extra tensors present: expected {}, got {}",
                    expected, actual
                ),
                recommendation: None,
            });
...
impl WeightHealthChecker {
    ...
    pub fn check_all_zeros(
            ...
            && data.iter().all(|&v| v == 0.0)
        {
            report.add(ValidationFinding {
                ...
                recommendation: Some(
                    "tensor may not have been initialized during training"
                        .into(),
                ),
            });
        }
    ...
    pub fn check_extreme_values(
        ...
        if let Some(data) = &tensor.data {
            ...
            if extreme_count > 0 {
                let pct =
                    (extreme_count as f64 / data.len().max(1) as f64) * 100.0;
                report.add(ValidationFinding {
                    severity: if pct > 1.0 {
                    ...
                    recommendation: Some(
                        "large weights may indicate training instability".into(),
                    ),
                });
            }
        }
    ...
    pub fn check_low_variance(
        ...
        if let Some(data) = &tensor.data {
            ...
            let var =
                data.iter().map(|&v| (v as f64 - mean).powi(2)).sum::<f64>()
                    / n;
            if (var as f32) < min_variance {
                report.add(ValidationFinding {
                    ...
                    tensor_name: Some(tensor.name.clone()),
                    message: format!(
                        "very low variance ({:.2e}), weights may be degenerate",
                        var
                    ),
                    recommendation: Some(
                        "model may not have converged during training".into(),
                    ),
                });
            }
        }
    ...
    pub fn check_memory_fit(
        ...
        if model_bytes > device.available_memory_bytes {
            report.add(ValidationFinding {
                ...
                recommendation: Some(
                    "use a smaller model or a device with more memory".into(),
                ),
            });
        } else {
            let utilization =
                model_bytes as f64 / device.available_memory_bytes as f64;
            if utilization > 0.9 {
                report.add(ValidationFinding {
                    severity: ValidationSeverity::Warning,
                    tensor_name: None,
                    ...
                    recommendation: Some(
                        "consider reducing context length or batch size".into(),
                    ),
                });
            }
        }
    ...
    pub fn check_dtype_support(
        ...
        if !device.supported_dtypes.contains(&spec.weight_dtype) {
            report.add(ValidationFinding {
                ...
                recommendation: Some(
                    "use a model quantized to a supported dtype".into(),
                ),
            });
        }
    }
        ...
        for t in tensors {
            ...
            if bytes > device.max_allocation_bytes {
                report.add(ValidationFinding {
                    ...
                    recommendation: Some(
                        "split large tensors or use tensor parallelism".into(),
                    ),
                });
            }
        }
...
impl FullCheck {
    ...
    pub fn run(
        ...
        if let Some(dev) = device {
            CompatibilityChecker::full_compatibility_check(
                spec, tensors, dev, &mut report,
            );
        }
...
mod tests {
    ...
    fn missing_tensor_detected() {
        ...
        let mut report = ValidationReport::new();
        ArchitectureValidator::check_tensor_names(&spec, &tensors, &mut report);
        assert!(report.errors().iter().any(|f| {
            f.tensor_name.as_deref() == Some("token_embd.weight")
        }));
    }
    ...
    fn extreme_in_output_head_detected() {
        ...
        assert!(report.findings.iter().any(|f| {
            f.tensor_name.as_deref() == Some("output.weight")
                && f.message.contains("extreme")
        }));
    }
