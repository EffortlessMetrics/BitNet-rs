...
impl CompressionCodec {
    ...
    pub fn decompress(self, data: &[u8]) -> Result<Vec<u8>, SerdeError> {
        match self {
            ...
            Self::Zstd { level } => {
                let inner = if level > 10 {
                    cpu_rle_decompress(data)?
                } else {
                    data.to_vec()
                };
...
fn cpu_rle_decompress(data: &[u8]) -> Result<Vec<u8>, SerdeError> {
    ...
    while i < data.len() {
        if data[i] == MARKER {
            if i + 3 >= data.len() {
                return Err(SerdeError::DecompressionFailed(
                    "truncated RLE marker".into(),
                ));
            }
            let byte = data[i + 1];
            let count =
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ShapeMismatch { expected, actual } => {
                write!(
                    f,
                    "shape mismatch: expected {expected} bytes, got {actual}"
                )
            }
...
impl TensorHeader {
    ...
    fn decode(data: &[u8]) -> Result<(Self, usize), SerdeError> {
        ...
        let dtype = DType::from_u8(data[0])
            .ok_or(SerdeError::InvalidDType(data[0]))?;
        let byte_order = ByteOrder::from_u8(data[1])
            .ok_or(SerdeError::InvalidByteOrder(data[1]))?;
        let comp_tag = data[2];
        let comp_level = data[3];
        let compression =
            CompressionCodec::from_tag_level(comp_tag, comp_level)
                .ok_or(SerdeError::InvalidCompression(comp_tag))?;
        let checksum_algo = ChecksumAlgorithm::from_u8(data[4])
            .ok_or(SerdeError::InvalidChecksum(data[4]))?;
...
impl TensorWriter {
    ...
    pub fn write<W: Write>(
        ...
        let checksum =
            ChecksumValidator::compute(self.checksum_algo, &compressed);
...
impl TensorBundle {
    ...
    pub fn write<W: Write>(
        ...
        for (i, (name, ..)) in self.tensors.iter().enumerate() {
            ...
            writer
                .write_all(&(blobs[i].len() as u64).to_le_bytes())?;
        }
    ...
    pub fn read<R: Read>(
        reader: &mut R,
    ) -> Result<(BundleIndex, Vec<u8>), SerdeError> {
        let mut buf = Vec::new();
    ...
    pub fn read_from_slice(
        ...
        for _ in 0..tensor_count {
            ...
            let name =
                String::from_utf8_lossy(&data[off..name_end]).to_string();
            off = name_end;
    ...
    pub fn read_tensor(
        ...
        let entry = index
            .get(name)
            .ok_or_else(|| SerdeError::TensorNotFound(name.into()))?;
...
impl BundleIndex {
    ...
    fn insert(&mut self, name: String, offset: usize, length: usize) {
        self.entries.insert(
            name.clone(),
            BundleIndexEntry { offset, length },
        );
...
mod tests {
    ...
    fn test_compression_codec_tag_roundtrip() {
        ...
        for c in codecs {
            let decoded =
                CompressionCodec::from_tag_level(c.tag(), c.level())
                    .unwrap();
    ...
    fn test_checksum_none() {
        assert_eq!(
            ChecksumValidator::compute(ChecksumAlgorithm::None, b"data"),
            0
        );
    }
    ...
    fn test_checksum_xxhash64_deterministic() {
        let data = b"hello world";
        let a =
            ChecksumValidator::compute(ChecksumAlgorithm::XxHash64, data);
        let b =
            ChecksumValidator::compute(ChecksumAlgorithm::XxHash64, data);
        assert_eq!(a, b);
        assert_ne!(a, 0);
    ...
    fn test_checksum_crc32_differs_for_different_data() {
        let a =
            ChecksumValidator::compute(ChecksumAlgorithm::Crc32, b"aaa");
        let b =
            ChecksumValidator::compute(ChecksumAlgorithm::Crc32, b"bbb");
        assert_ne!(a, b);
    }

    #[test]
    fn test_checksum_xxhash64_differs_for_different_data() {
        let a = ChecksumValidator::compute(
            ChecksumAlgorithm::XxHash64,
            b"aaa",
        );
        let b = ChecksumValidator::compute(
            ChecksumAlgorithm::XxHash64,
            b"bbb",
        );
        assert_ne!(a, b);
    ...
    fn test_checksum_validate_ok() {
        ...
        let cksum =
            ChecksumValidator::compute(ChecksumAlgorithm::Crc32, data);
        ChecksumValidator::validate(ChecksumAlgorithm::Crc32, data, cksum)
            .unwrap();
    }
    ...
    fn test_checksum_validate_corrupted() {
        ...
        let cksum =
            ChecksumValidator::compute(ChecksumAlgorithm::Crc32, data);
        let result = ChecksumValidator::validate(
    ...
    fn test_checksum_validate_none_always_ok() {
        ChecksumValidator::validate(ChecksumAlgorithm::None, b"x", 999)
            .unwrap();
    }
    ...
    fn test_header_decode_invalid_dtype() {
        ...
        assert!(matches!(
            TensorHeader::decode(&buf),
            Err(SerdeError::InvalidDType(99))
        ));
    }
    ...
    fn test_header_decode_invalid_byte_order() {
        ...
        assert!(matches!(
            TensorHeader::decode(&buf),
            Err(SerdeError::InvalidByteOrder(99))
        ));
    }
    ...
    fn test_header_decode_invalid_compression() {
        ...
        assert!(matches!(
            TensorHeader::decode(&buf),
            Err(SerdeError::InvalidCompression(99))
        ));
    }
    ...
    fn test_header_decode_invalid_checksum() {
        ...
        assert!(matches!(
            TensorHeader::decode(&buf),
            Err(SerdeError::InvalidChecksum(99))
        ));
    }
    ...
    fn test_write_read_f32_uncompressed() {
        ...
        let (blob, wstats) =
            tw.write_to_vec(DType::F32, &[3, 4], &data).unwrap();
        assert_eq!(wstats.raw_bytes, data.len() as u64);

        let (hdr, raw, _rstats) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_f32_lz4() {
        ...
        let tw = TensorWriter::new()
            ...
            .with_checksum(ChecksumAlgorithm::Crc32);
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[256], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
        assert_eq!(hdr.dtype, DType::F32);
        assert_eq!(raw, data);
    ...
    fn test_write_read_f32_zstd() {
        ...
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[128], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_i8() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::I8, &[10, 10], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_u8() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::U8, &[8, 8], &data).unwrap();
    ...
    fn test_write_read_i32() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::I32, &[20], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_f16() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::F16, &[4, 4], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_bf16() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::BF16, &[10], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_i16() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::I16, &[8], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_f64() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::F64, &[4], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_i2() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::I2, &[32], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_big_endian() {
        ...
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[4], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_xxhash64_checksum() {
        ...
        let tw = TensorWriter::new()
            .with_checksum(ChecksumAlgorithm::XxHash64);
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[8], &data).unwrap();
        let (_, raw, _) = TensorReader::read_from_slice(&blob).unwrap();
        assert_eq!(raw, data);
    }
    ...
    fn test_read_unsupported_version() {
        ...
        assert!(matches!(
            result,
            Err(SerdeError::UnsupportedVersion(99))
        ));
    }
    ...
    fn test_read_corrupted_data_checksum_fails() {
        ...
        let tw = TensorWriter::new()
            .with_checksum(ChecksumAlgorithm::Crc32);
        let (mut blob, _) =
            tw.write_to_vec(DType::F32, &[4, 4], &data).unwrap();
    ...
    fn test_mmap_reader_compressed() {
        ...
        let tw = TensorWriter::new()
            .with_compression(CompressionCodec::Lz4);
        let (blob, _) =
            tw.write_to_vec(DType::U8, &[512], &data).unwrap();
    ...
    fn test_streaming_reader_basic() {
        ...
        let tw = TensorWriter::new()
            ...
            .with_checksum(ChecksumAlgorithm::None);
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[8, 4], &data).unwrap();
    ...
    fn test_streaming_reader_read_all() {
        ...
        let tw = TensorWriter::new()
            ...
            .with_checksum(ChecksumAlgorithm::None);
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[20], &data).unwrap();
    ...
    fn test_streaming_reader_single_byte_chunks() {
        ...
        let tw = TensorWriter::new()
            ...
            .with_checksum(ChecksumAlgorithm::None);
        let (blob, _) =
            tw.write_to_vec(DType::U8, &[4], &data).unwrap();
    ...
    fn test_bundle_single_tensor() {
        ...
        let (index, raw_blob) =
            TensorBundle::read_from_slice(&blob).unwrap();
        assert_eq!(index.len(), 1);
        assert!(index.contains("weight"));

        let (hdr, raw, _) =
            TensorBundle::read_tensor("weight", &index, &raw_blob)
                .unwrap();
        assert_eq!(hdr.dtype, DType::F32);
        assert_eq!(raw, data);
    }
    ...
    fn test_bundle_multiple_tensors() {
        ...
        assert_eq!(
            index.names(),
            &["layer.0.weight", "layer.0.bias", "layer.1.weight"]
        );

        let (h1, d1, _) = TensorBundle::read_tensor(
            "layer.0.weight",
            &index,
            &raw_blob,
        )
        ...
        let (_, d2, _) = TensorBundle::read_tensor(
            "layer.0.bias",
            &index,
            &raw_blob,
        )
        ...
        let (_, d3, _) = TensorBundle::read_tensor(
            "layer.1.weight",
            &index,
            &raw_blob,
        )
    ...
    fn test_bundle_compressed() {
        ...
        let mut bundle =
            TensorBundle::new().with_compression(CompressionCodec::Lz4);
        ...
        let (index, raw_blob) =
            TensorBundle::read_from_slice(&blob).unwrap();
        let (_, raw, _) =
            TensorBundle::read_tensor("t", &index, &raw_blob).unwrap();
        assert_eq!(raw, data);
    }
    ...
    fn test_bundle_not_found() {
        ...
        let (index, raw_blob) =
            TensorBundle::read_from_slice(&blob).unwrap();
        let result =
            TensorBundle::read_tensor("nonexistent", &index, &raw_blob);
        assert!(matches!(
            result,
            Err(SerdeError::TensorNotFound(_))
        ));
    }
    ...
    fn test_bundle_xxhash_checksum() {
        ...
        let (index, raw_blob) =
            TensorBundle::read_from_slice(&blob).unwrap();
        let (_, raw, _) =
            TensorBundle::read_tensor("w", &index, &raw_blob).unwrap();
        assert_eq!(raw, data);
    }
    ...
    fn test_serde_stats_throughput() {
        ...
        assert!(
            (s.throughput_bytes_per_sec() - 1_000_000.0).abs() < 1.0
        );
    }
    ...
    fn test_large_tensor_roundtrip() {
        ...
        let tw = TensorWriter::new()
            ...
            .with_checksum(ChecksumAlgorithm::Crc32);
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[100, 100], &data).unwrap();
        let (hdr, raw, _) =
            TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_large_tensor_zeros_compresses_well() {
        ...
        let tw = TensorWriter::new()
            .with_compression(CompressionCodec::Lz4);
        let (_, stats) =
            tw.write_to_vec(DType::U8, &[100_000], &data).unwrap();
        assert!(
    ...
    fn test_error_display() {
        ...
        let err = SerdeError::ChecksumMismatch {
            expected: 1,
            actual: 2,
        };
    ...
    fn test_identity_all_dtypes_no_compression() {
        ...
        for dtype in dtypes {
            ...
            let tw = TensorWriter::new()
                ...
                .with_checksum(ChecksumAlgorithm::Crc32);
            let (blob, _) =
                tw.write_to_vec(dtype, &[16], &data).unwrap();
            let (hdr, raw, _) =
                TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_identity_all_compressions() {
        ...
        for codec in codecs {
            ...
            assert_eq!(
                raw, data,
                "data roundtrip failed for codec {codec:?}"
            );
        }
    }
    ...
    fn test_identity_various_shapes() {
        ...
        for shape in &shapes {
            ...
            let data: Vec<u8> =
                (0..elems).map(|i| (i % 256) as u8).collect();
            let tw = TensorWriter::new();
            let (blob, _) =
                tw.write_to_vec(DType::U8, shape, &data).unwrap();
            let (hdr, raw, _) =
                TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_identity_f32_values_preserved() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[values.len()], &data)
                .unwrap();
        let (_, raw, _) = TensorReader::read_from_slice(&blob).unwrap();
    ...
    fn test_write_read_empty_tensor() {
        let tw = TensorWriter::new();
        let (blob, stats) =
            tw.write_to_vec(DType::F32, &[0], &[]).unwrap();
    ...
    fn test_bundle_with_empty_tensor() {
        ...
        let (index, raw_blob) =
            TensorBundle::read_from_slice(&blob).unwrap();
        let (_, d1, _) =
            TensorBundle::read_tensor("empty", &index, &raw_blob)
                .unwrap();
        assert!(d1.is_empty());
        let (_, d2, _) =
            TensorBundle::read_tensor("notempty", &index, &raw_blob)
                .unwrap();
    ...
    fn test_streaming_compressed_read_all() {
        ...
        let tw = TensorWriter::new()
            ...
            .with_checksum(ChecksumAlgorithm::None);
        let (blob, _) =
            tw.write_to_vec(DType::U8, &[256], &data).unwrap();
        let cursor = io::Cursor::new(blob);
    ...
    fn test_strides_written_correctly() {
        ...
        let tw = TensorWriter::new();
        let (blob, _) =
            tw.write_to_vec(DType::F32, &[2, 3, 4], &data).unwrap();
    ...
    fn test_write_to_write_trait() {
        ...
        let stats =
            tw.write(&mut buf, DType::F32, &[4], &data).unwrap();
        assert!(stats.raw_bytes > 0);
    ...
    fn test_bundle_read_from_reader() {
        ...
        let (index, raw_blob) =
            TensorBundle::read(&mut cursor).unwrap();
        let (_, raw, _) =
            TensorBundle::read_tensor("t", &index, &raw_blob).unwrap();
        assert_eq!(raw, data);
    }
