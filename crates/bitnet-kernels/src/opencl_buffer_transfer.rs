...
impl DeviceToDevice {
    ...
    pub fn copy_range(
        ...
        dst.check_live()?;
        let src_end =
            src_offset.checked_add(len).ok_or(TransferError::SourceTooSmall {
                required: usize::MAX,
                available: src.data.len(),
            })?;
        ...
        let dst_end =
            dst_offset.checked_add(len).ok_or(TransferError::DestinationTooSmall {
                required: usize::MAX,
                available: dst.data.len(),
            })?;
...
impl TransferStats {
    ...
    pub fn stats_for_direction(&self, dir: TransferDirection) -> DirectionStats {
        ...
        DirectionStats { direction: dir, total_bytes, count, total_ns, peak_bandwidth_gbps: peak_bw }
    }
...
mod tests {
    ...
    fn scheduler_alternates_slots() {
        let sched =
            TransferScheduler::new(SchedulerConfig { slot_size: 64 }).unwrap();
    ...
    fn scheduler_double_buffer_swap() {
        let mut sched =
            TransferScheduler::new(SchedulerConfig { slot_size: 32 }).unwrap();
    ...
    fn scheduler_tracks_stats() {
        let mut sched =
            TransferScheduler::new(SchedulerConfig { slot_size: 128 }).unwrap();
    ...
    fn scheduler_data_too_large() {
        let mut sched =
            TransferScheduler::new(SchedulerConfig { slot_size: 16 }).unwrap();
