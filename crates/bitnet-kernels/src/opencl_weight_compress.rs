...
impl TernaryQuantizer {
    ...
    pub fn quantize(&self, weights: &[f32]) -> TernaryResult {
        ...
        let scale = if nonzero_count > 0 {
            nonzero_abs_sum / nonzero_count as f32
        } else {
            0.0
        };
...
impl AwqQuantizer {
    ...
    pub fn quantize(&self, weights: &[f32], importance: &[f32]) -> AwqResult {
        ...
        let importance_scales: Vec<f32> = importance
            .iter()
            .map(|&imp| imp.abs().powf(self.alpha).max(f32::EPSILON))
            .collect();
...
impl WeightClustering {
    pub fn new(num_centroids: usize, max_iterations: usize) -> Self {
        assert!(
            (2..=256).contains(&num_centroids),
            "num_centroids must be 2..=256"
        );
...
mod tests {
    ...
    fn test_decompress_2bit_identity() {
        ...
        DecompressionKernel::matvec_2bit(&packed_w, &scales, &zeros, &x, &mut y, rows, cols, group_size);
    ...
    fn test_decompress_4bit_identity() {
        ...
        DecompressionKernel::matvec_4bit(&packed_w, &scales, &zeros, &x, &mut y, rows, cols, group_size);
