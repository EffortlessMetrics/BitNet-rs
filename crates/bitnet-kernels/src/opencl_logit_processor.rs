...
mod tests {
    ...
    fn pipeline_order_matters() {
        ...
        let finite_a: Vec<usize> = logits_a
            .iter()
            .enumerate()
            .filter(|(_, v)| v.is_finite())
            .map(|(i, _)| i)
            .collect();
        let finite_b: Vec<usize> = logits_b
            .iter()
            .enumerate()
            .filter(|(_, v)| v.is_finite())
            .map(|(i, _)| i)
    ...
    fn pipeline_warper_names() {
        ...
        assert_eq!(
            pipeline.warper_names(),
            vec!["TemperatureWarper", "TopKWarper", "TopPWarper"]
        );
    }
    ...
    fn property_filtered_logits_le_original() {
        ...
        for (filtered, &orig) in logits.iter().zip(original.iter()) {
            assert!(
                *filtered <= orig,
                "filtered {filtered} > original {orig}"
            );
        }
    }
