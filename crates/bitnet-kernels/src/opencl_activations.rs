...
impl ActivationKind {
    ...
    pub fn derivative(self, x: f32) -> f32 {
        match self {
            ...
            Self::HardSigmoid => {
                if !(-3.0..=3.0).contains(&x) { 0.0 } else { 1.0 / 6.0 }
            }
            Self::QuickGELU => {
                let bx = 1.702 * x;
                let s = sigmoid(bx);
    ...
    pub fn has_parameter(self) -> bool {
        matches!(
            self,
            Self::LeakyReLU(_) | Self::ELU(_) | Self::Softplus(_)
        )
    }
...
impl ApproximateActivation {
    ...
    pub fn apply_approx(x: f32, kind: ActivationKind) -> f32 {
        match kind {
            ...
            ActivationKind::SiLU | ActivationKind::Swish => {
                x * fast_sigmoid(x)
            }
            ActivationKind::Mish => {
...
mod tests {
    ...
    fn test_gelu_vs_gelu_tanh_relative_error() {
        ...
        for i in -50..=50 {
            ...
            if exact.abs() > 0.05 {
                ...
                assert!(
                    rel < 0.05,
                    "GELU vs GELUTanh rel error {rel} at x={x}"
                );
            }
        }
    }
    ...
    fn test_all_derivatives_numerical() {
        ...
        for kind in all_kinds() {
            for &x in &[-2.0, -1.0, -0.5, 0.1, 0.5, 1.0, 2.0] {
                ...
                let numerical =
                    numerical_derivative(|v| kind.apply(v), x, h);
                assert_close(
                    analytical,
                    numerical,
                    tol,
    ...
    fn test_fused_matches_separate(kind: ActivationKind) {
        ...
        let input: Vec<f32> =
            (0..batch * in_f).map(|i| i as f32 * 0.1).collect();
        let weight: Vec<f32> =
            (0..out_f * in_f).map(|i| (i as f32 - 5.0) * 0.1).collect();
        let bias: Vec<f32> =
            (0..out_f).map(|i| i as f32 * 0.05).collect();
        ...
        fused
            .apply_fused_ref(
                &input, &weight, &bias, &mut fused_out, batch, in_f, out_f,
            )
            .unwrap();
    ...
    fn test_fused_no_bias() {
        ...
        fused
            .apply_fused_ref(&input, &weight, &[], &mut output, 1, 3, 1)
            .unwrap();
    ...
    fn test_fused_error_on_short_output() {
        ...
        let result =
            fused.apply_fused_ref(&input, &weight, &[], &mut output, 2, 2, 2);
        assert!(result.is_err());
    }
    ...
    fn test_approx_gelu_closeness() {
        ...
        for i in -20..=20 {
            ...
            if e.abs() > 0.05 {
                ...
                assert!(
                    rel < 0.05,
                    "approx GELU rel error {rel} at x={x}"
                );
            }
        }
    }
    ...
    fn test_approx_ref_output() {
        ...
        for (i, (&x, &y)) in input.iter().zip(output.iter()).enumerate() {
            let expected =
                ApproximateActivation::apply_approx(x, ActivationKind::GELU);
    ...
    fn test_hard_sigmoid_monotonic() {
        ...
        for i in -99..=100 {
            ...
            assert!(
                y >= prev,
                "HardSigmoid not monotonic at x={x}: {prev} > {y}"
            );
            prev = y;
        }
    }
    ...
    fn test_stats_basic() {
        ...
        let mut output = vec![0.0_f32; input.len()];
        let stats = ActivationStats::apply_and_collect(
            ActivationKind::ReLU,
            &input,
            &mut output,
        )
    ...
    fn test_stats_error_short_output() {
        ...
        assert!(ActivationStats::apply_and_collect(
            ActivationKind::ReLU,
            &input,
            &mut output,
        )
    ...
    fn test_opencl_source_contains_all_kernels() {
        ...
        for name in &[
            "relu", "gelu", "gelu_tanh", "silu", "swish", "sigmoid_act",
            "tanh_act", "leaky_relu", "elu", "softplus", "mish",
            "hard_swish", "hard_sigmoid", "quick_gelu",
        ] {
            assert!(
    ...
    fn test_opencl_source_contains_derivatives() {
        let src = ACTIVATIONS_CL;
        for name in &[
            "relu_deriv", "sigmoid_deriv", "silu_deriv", "tanh_deriv",
        ] {
            assert!(
                src.contains(&format!("__kernel void {name}")),
