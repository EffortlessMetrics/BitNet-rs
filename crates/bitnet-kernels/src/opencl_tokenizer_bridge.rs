    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for &id in &[st.bos_id, st.eos_id, st.pad_id, st.unk_id] {
            if id as usize >= self.vocab_size {
                return Err(format!(
                    "special token id {id} >= vocab_size {}",
                    self.vocab_size
                ));
            }
        }
    ...
    fn truncate(&self, seq: &[u32], max_len: usize) -> Result<Vec<u32>, String> {
        ...
        match self.config.truncation {
            ...
            TruncationStrategy::TruncateStart => {
                Ok(seq[seq.len() - max_len..].to_vec())
            }
            TruncationStrategy::Error => Err(format!(
...
impl TokenEncoder {
    ...
    pub fn tiny_test() -> Self {
        ...
        let words = [
            "hello", "world", "the", "a", "is", "of", "to", "and", "in",
            "that", "it", "for", "you", "was", "on", "are", "with", "this",
            "from", "or", "an", "be", "one", "had", "by", "not", "but",
            "what", "all", "were", "when", "we", "there", "can", "your",
            "which", "their", "will", "each", "about", "how", "up", "out",
            "them", "then", "she", "many", "some", "so", "these", "would",
            "other", "into", "has", "more", "two", "her", "like", "him",
        ];
    ...
    pub fn encode(&self, text: &str) -> Result<Vec<u32>, String> {
        ...
        for word in tokenize_words(text) {
            let id = self
                ...
                .get(word)
                .copied()
                .unwrap_or(self.config.special_tokens.unk_id);
            ids.push(id);
        }
...
mod tests {
    ...
    fn batch_single_sequence() {
        let cfg = TokenizerConfig::new(100, 128);
        let batch =
            TokenBatch::from_sequences(&[vec![10, 20, 30]], &cfg).unwrap();
    ...
    fn batch_truncation_end() {
        let cfg = TokenizerConfig::new(100, 3)
            .with_truncation(TruncationStrategy::TruncateEnd);
    ...
    fn batch_truncation_error() {
        let cfg = TokenizerConfig::new(100, 3)
            .with_truncation(TruncationStrategy::Error);
        let seqs = vec![vec![1, 2, 3, 4, 5]];
    ...
    fn batch_sequence_oob() {
        let cfg = TokenizerConfig::new(100, 128);
        let batch =
            TokenBatch::from_sequences(&[vec![1, 2]], &cfg).unwrap();
    ...
    fn batch_max_seq_len_clamps() {
        let cfg = TokenizerConfig::new(100, 4)
            .with_padding_side(PaddingSide::Right);
    ...
    fn encode_batch_with_stats_basic() {
        let enc = test_encoder();
        let (ids, stats) =
            encode_batch_with_stats(&enc, &["hello", "world the"]).unwrap();
        assert_eq!(ids.len(), 2);
    ...
    fn prepare_batch_basic() {
        let enc = test_encoder();
        let (batch, stats) =
            prepare_batch(&enc, &["hello", "world the"]).unwrap();
