...
impl AdmissionController {
    ...
    pub fn should_admit(
        ...
        if current_pending_memory + request.estimated_memory_bytes
            > self.max_pending_memory_bytes
        {
            return false;
        }
        true
...
impl PreemptionManager {
    ...
    pub fn select_victims(
        ...
        let mut candidates: Vec<(usize, Priority, u64)> = running
            ...
            .enumerate()
            .filter(|(_, (_, req))| {
                incoming.priority.0 >= req.priority.0 + self.priority_gap
            })
...
impl BatchScheduler {
    ...
    pub fn submit(&mut self, request: BatchRequest) -> Result<(), BatchRequest> {
        let pending_mem: u64 =
            self.queue.iter().map(|r| r.estimated_memory_bytes).sum();
    ...
    pub fn try_preempt(
        ...
        self.preemption.select_victims(
            incoming,
            running_slots,
            running_requests,
            needed,
    ...
    fn fair_share_sort(requests: &mut [BatchRequest]) {
        ...
        let mut buckets: BTreeMap<std::cmp::Reverse<u32>, VecDeque<usize>> =
            BTreeMap::new();
        for (i, req) in requests.iter().enumerate() {
            buckets
                .entry(std::cmp::Reverse(req.priority.0))
                .or_default()
                .push_back(i);
        }
...
mod tests {
    ...
    fn fcfs_ignores_priority() {
        ...
        let mut sched = default_scheduler(SchedulingPolicy::Fcfs, 10);
        sched
            .submit(req_at(1, Priority::LOW, 32, base, Duration::from_millis(0)))
            .unwrap();
        sched
            .submit(req_at(2, Priority::CRITICAL, 32, base, Duration::from_millis(10)))
            .unwrap();
    ...
    fn sjf_orders_by_max_tokens() {
        ...
        for (i, tokens) in [128, 16, 64, 8, 256].iter().enumerate() {
            sched
                .submit(req_at(i as u64, Priority::NORMAL, *tokens, base, Duration::from_millis(i as u64)))
                .unwrap();
        }
        let (slots, _) = sched.schedule_batch();
    ...
    fn sjf_ties_preserved_stably() {
        ...
        for i in 0..4 {
            sched
                .submit(req_at(i, Priority::NORMAL, 32, base, Duration::from_millis(i)))
                .unwrap();
        }
        let (slots, _) = sched.schedule_batch();
    ...
    fn priority_orders_highest_first() {
        ...
        sched
            .submit(req_at(2, Priority::CRITICAL, 32, base, Duration::from_millis(1)))
            .unwrap();
        sched
            .submit(req_at(3, Priority::NORMAL, 32, base, Duration::from_millis(2)))
            .unwrap();
        sched
            .submit(req_at(4, Priority::HIGH, 32, base, Duration::from_millis(3)))
            .unwrap();
        let (slots, _) = sched.schedule_batch();
        let ids: Vec<u64> = slots.iter().map(|s| s.request_id).collect();
    ...
    fn fair_share_interleaves_priorities() {
        ...
        sched
            .submit(req_at(11, Priority::HIGH, 32, base, Duration::from_millis(1)))
            .unwrap();
        sched
            .submit(req_at(20, Priority::NORMAL, 32, base, Duration::from_millis(2)))
            .unwrap();
        sched
            .submit(req_at(21, Priority::NORMAL, 32, base, Duration::from_millis(3)))
            .unwrap();
        sched
            .submit(req_at(30, Priority::LOW, 32, base, Duration::from_millis(4)))
            .unwrap();
        sched
            .submit(req_at(31, Priority::LOW, 32, base, Duration::from_millis(5)))
            .unwrap();
    ...
    fn fair_share_uneven_tiers() {
        ...
        sched
            .submit(req_at(2, Priority::HIGH, 32, base, Duration::from_millis(1)))
            .unwrap();
        sched
            .submit(req_at(3, Priority::HIGH, 32, base, Duration::from_millis(2)))
            .unwrap();
        sched
            .submit(req_at(4, Priority::LOW, 32, base, Duration::from_millis(3)))
            .unwrap();
    ...
    fn memory_limits_batch_size() {
        ...
        let mut sched =
            BatchScheduler::new(SchedulingPolicy::Fcfs, 10, mem, admission, preempt);
    ...
    fn memory_releases_after_batch_complete() {
        ...
        sched
            .submit(req_at(1, Priority::NORMAL, 32, base, Duration::ZERO))
            .unwrap();
        let (slots, reqs) = sched.schedule_batch();
        assert_eq!(slots.len(), 1);
    ...
    fn preemption_via_scheduler() {
        ...
        let preempted: Vec<BatchSlot> =
            victims.iter().map(|&i| running_slots[i].clone()).collect();
    ...
    fn slot_indices_are_sequential() {
        ...
        for i in 0..5 {
            sched
                .submit(req_at(i, Priority::NORMAL, 16, base, Duration::from_millis(i)))
                .unwrap();
        }
        let (slots, _) = sched.schedule_batch();
    ...
    fn property_memory_never_exceeds_budget() {
        ...
        let preempt = PreemptionManager::new(2);
        let mut sched =
            BatchScheduler::new(SchedulingPolicy::Fcfs, 100, mem, admission, preempt);
        ...
        assert!(
            ...
            "scheduled {total_reserved} bytes but budget is {total}",
        );
    }
    ...
    fn property_batch_size_never_exceeds_max() {
        for max in [1, 2, 5, 10] {
            ...
            assert!(
                slots.len() <= max,
                "max_batch_size={max} but got {} slots",
                slots.len(),
            );
        }
    }
    ...
    fn deadline_exact_boundary() {
        let base = Instant::now();
        let reqs =
            vec![BatchRequest::new(1, Priority::NORMAL, 32).with_deadline(base)];
    ...
    fn sjf_large_spread() {
        ...
        for (i, &s) in sizes.iter().enumerate() {
            sched
                .submit(req_at(i as u64, Priority::NORMAL, s, base, Duration::from_millis(i as u64)))
                .unwrap();
        }
        let (slots, _) = sched.schedule_batch();
        let tokens: Vec<usize> = slots.iter().map(|s| s.max_tokens).collect();
