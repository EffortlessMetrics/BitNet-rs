...
pub fn tensor_concat_ref(
    ...
    if axis >= ndim {
        return Err(KernelError::InvalidArguments {
            reason: format!(
                "tensor_concat: axis {} out of range for {}-d tensor",
                axis, ndim
            ),
        }
        .into());
    ...
    for (tensor_data, shape) in inputs.iter().zip(shapes.iter()) {
        ...
        for o in 0..outer_size {
            for a in 0..src_axis {
                ...
                let dst_start =
                    (o * dst_axis + (axis_offset + a)) * inner_size;
                output[dst_start..dst_start + inner_size]
                    .copy_from_slice(
                        &tensor_data[src_start..src_start + inner_size],
                    );
            }
        }
...
pub fn tensor_split_ref(
    ...
    if n == 0 {
        return Err(KernelError::InvalidArguments {
            reason: "tensor_split: n must be > 0".into(),
        }
        .into());
    }
    ...
    for (part_idx, part_buf) in outputs.iter_mut().enumerate() {
        ...
        for o in 0..outer {
            for a in 0..chunk {
                let src_start =
                    (o * src_axis + (axis_offset + a)) * inner;
...
pub fn tensor_stack_ref(
    ...
    let outer: usize = if axis == 0 {
        1
    } else {
        shapes[0].dims[..axis].iter().product()
    };
    // inner = product of dims from axis onward in *input* shape
    let inner: usize = if axis >= ndim {
        1
    } else {
        shapes[0].dims[axis..].iter().product()
    };
...
pub fn tensor_slice_ref(
    ...
    let out_dims: Vec<usize> =
        specs.iter().map(|s| s.output_size()).collect();
...
impl ConcatStats {
    ...
    pub fn with_timing(mut self, elapsed_secs: f64) -> Self {
        ...
        if elapsed_secs > 0.0 {
            self.bandwidth_elem_per_sec =
                Some(self.elements as f64 / elapsed_secs);
        }
        self
    }
...
impl std::fmt::Display for ConcatStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "ConcatStats {{ elements: {}, inputs: {}",
            self.elements, self.num_inputs
        )?;
...
mod tests {
    ...
    fn concat_four_tensors_axis1() {
        ...
        let data: Vec<Vec<f32>> =
            (0..4).map(|i| vec![i as f32]).collect();
        let s = shape(&[1, 1]);
    ...
    fn split_axis0_into_two() {
        ...
        let shapes =
            tensor_split_ref(&data, &s, 0, 2, &mut outs).unwrap();
    ...
    fn split_axis1_2d() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_split_ref(&data, &s, 1, 2, &mut outs).unwrap();
        assert_eq!(shapes.len(), 2);
    ...
    fn split_concat_roundtrip_axis0() {
        ...
        let part_shapes =
            tensor_split_ref(&data, &s, 0, 3, &mut parts).unwrap();
        let refs: Vec<&[f32]> = parts.iter().map(|v| v.as_slice()).collect();
        let shape_refs: Vec<&TensorShape> =
            part_shapes.iter().collect();
    ...
    fn split_concat_roundtrip_axis1() {
        ...
        let mut parts = Vec::new();
        let part_shapes =
            tensor_split_ref(&data, &s, 1, 3, &mut parts).unwrap();
        let refs: Vec<&[f32]> = parts.iter().map(|v| v.as_slice()).collect();
        let shape_refs: Vec<&TensorShape> =
            part_shapes.iter().collect();
    ...
    fn split_concat_roundtrip_3d_axis2() {
        ...
        let mut parts = Vec::new();
        let part_shapes =
            tensor_split_ref(&data, &s, 2, 2, &mut parts).unwrap();
        let refs: Vec<&[f32]> = parts.iter().map(|v| v.as_slice()).collect();
        let shape_refs: Vec<&TensorShape> =
            part_shapes.iter().collect();
        let mut recon = vec![0.0; 24];
    ...
    fn chunk_even_split() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_chunk_ref(&data, &s, 0, 2, &mut outs).unwrap();
    ...
    fn chunk_with_remainder() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_chunk_ref(&data, &s, 0, 3, &mut outs).unwrap();
        assert_eq!(shapes.len(), 3);
    ...
    fn chunk_axis1_2d() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_chunk_ref(&data, &s, 1, 2, &mut outs).unwrap();
        assert_eq!(shapes.len(), 3);
        assert_eq!(shapes[0].dims, vec![2, 2]);
    ...
    fn chunk_single_element() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_chunk_ref(&data, &s, 0, 1, &mut outs).unwrap();
    ...
    fn stack_empty_inputs_error() {
        let mut out = vec![0.0; 1];
        let r: Result<TensorShape> =
            tensor_stack_ref(&[], &[], 0, &mut out);
        assert!(r.is_err());
    }
    ...
    fn unstack_axis0_2d() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_unstack_ref(&data, &s, 0, &mut outs).unwrap();
        assert_eq!(shapes.len(), 2);
        assert_eq!(shapes[0].dims, vec![3]);
    ...
    fn unstack_axis1_2d() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_unstack_ref(&data, &s, 1, &mut outs).unwrap();
        assert_eq!(shapes.len(), 3);
        assert_eq!(shapes[0].dims, vec![2]);
    ...
    fn stack_unstack_roundtrip_axis0() {
        ...
        let stacked_shape = tensor_stack_ref(
            &[&a, &b],
            &[&s, &s],
            0,
            &mut stacked,
        ...
        tensor_unstack_ref(&stacked, &stacked_shape, 0, &mut unstacked)
            .unwrap();
    ...
    fn stack_unstack_roundtrip_axis1() {
        ...
        .unwrap();
        let mut unstacked = Vec::new();
        tensor_unstack_ref(&stacked, &stacked_shape, 1, &mut unstacked)
            .unwrap();
        assert_eq!(unstacked[0], a);
        assert_eq!(unstacked[1], b);
    }
    ...
    fn concat_output_size_equals_sum_of_inputs_axis0() {
        ...
        for n_tensors in 1..=5 {
            let data: Vec<Vec<f32>> =
                (0..n_tensors).map(|i| iota((i + 1) * 3)).collect();
            let shapes: Vec<TensorShape> =
                (0..n_tensors).map(|i| shape(&[(i + 1) * 3])).collect();
            let refs: Vec<&[f32]> =
            ...
            let r = tensor_concat_ref(&refs, &shape_refs, 0, &mut out)
                .unwrap();
    ...
    fn split_produces_correct_count() {
        for n in 1..=6 {
            ...
            let mut outs = Vec::new();
            let shapes =
                tensor_split_ref(&data, &s, 0, n, &mut outs).unwrap();
    ...
    fn chunk_total_elements_preserved() {
        for chunk_size in 1..=5 {
            ...
            let s = shape(&[10]);
            let mut outs = Vec::new();
            tensor_chunk_ref(&data, &s, 0, chunk_size, &mut outs)
                .unwrap();
    ...
    fn split_into_one() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_split_ref(&data, &s, 0, 1, &mut outs).unwrap();
        assert_eq!(shapes.len(), 1);
    ...
    fn chunk_larger_than_dim() {
        ...
        let mut outs = Vec::new();
        let shapes =
            tensor_chunk_ref(&data, &s, 0, 10, &mut outs).unwrap();
        assert_eq!(shapes.len(), 1);
        assert_eq!(outs[0], data);
    }
