...
impl MaxPool1d {
    ...
    pub fn forward(
        ...
        if output.len() < out_len {
            return Err(KernelError::InvalidArguments {
                reason: format!("output buffer too small: need {out_len}, got {}", output.len()),
            }
            .into());
        }
        ...
        {
            return Err(KernelError::InvalidArguments {
                reason: format!("indices buffer too small: need {out_len}, got {}", idx.len()),
            }
            .into());
        }
...
impl AdaptivePool {
    ...
    pub fn forward(&self, input: &[f32], output: &mut [f32]) -> Result<()> {
        ...
        if input_len < self.output_size {
            return Err(KernelError::InvalidArguments {
                reason: format!("input_len({input_len}) < output_size({})", self.output_size),
            }
            .into());
        }
...
impl PoolStats {
    ...
    pub fn new(input_len: usize, output_len: usize) -> Self {
        let reduction_ratio =
            if output_len > 0 { input_len as f32 / output_len as f32 } else { 0.0 };
...
mod tests {
    ...
    fn test_property_avgpool_in_range() {
        ...
        pool.forward(&input, &mut output).unwrap();
        for (i, &v) in output.iter().enumerate() {
            assert!(v >= min_in && v <= max_in, "avg_pool[{i}]={v} not in [{min_in}, {max_in}]");
        }
    }
