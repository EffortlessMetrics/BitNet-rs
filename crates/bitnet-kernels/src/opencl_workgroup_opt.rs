...
impl OccupancyCalculator {
    ...
    pub fn calculate(
        ...
        let sg = constraints
            ...
            .unwrap_or(self.profile.preferred_subgroup_size);
        ...
        let wgs_by_threads = if subgroups_per_wg == 0 {
            0
        } else {
            max_wf / subgroups_per_wg
        };
...
impl GlobalSizeAligner {
    ...
    pub fn align_3d(
        ...
        (round_up(x.max(1), wg.local_x), round_up(y.max(1), wg.local_y), round_up(z.max(1), wg.local_z))
    }
        ...
        while lx <= effective_max {
            ...
            while lx * ly <= effective_max {
                if lx * ly >= constraints.min_workgroup_size {
                    ...
                    if let Some(stats) = self.calculator.calculate(&wg, constraints) {
                        let eff = GlobalSizeAligner::efficiency_2d(
                            rows.max(1),
                            cols.max(1),
                            &wg,
                        );
...
mod tests {
    ...
    fn occupancy_unsupported_subgroup_returns_none() {
        ...
        let constraints = KernelConstraints {
            required_subgroup_size: Some(64),
            ..Default::default()
        };
        assert!(calc.calculate(&wg, &constraints).is_none());
    }
    ...
    fn occupancy_slm_limits_concurrent_wgs() {
        ...
        let constraints = KernelConstraints {
            required_local_memory_bytes: 60000,
            ..Default::default()
        };
    ...
    fn occupancy_slm_overflow_returns_none() {
        ...
        let wg = WorkgroupConfig::new_1d(16);
        let constraints = KernelConstraints {
            required_local_memory_bytes: 70000,
            ..Default::default()
        };
        assert!(calc.calculate(&wg, &constraints).is_none());
    }
    ...
    fn occupancy_register_overflow_returns_none() {
        ...
        let wg = WorkgroupConfig::new_1d(16);
        let constraints = KernelConstraints {
            estimated_registers_per_item: 200,
            ..Default::default()
        };
        assert!(calc.calculate(&wg, &constraints).is_none());
    }
    ...
    fn occupancy_wasted_items_non_aligned() {
        ...
        let constraints = KernelConstraints {
            required_subgroup_size: Some(16),
            ..Default::default()
        };
        let stats = calc.calculate(&wg, &constraints).unwrap();
    ...
    fn optimizer_1d_basic() {
        ...
        let (wg, stats) = opt
            .optimize_1d(4096, &KernelConstraints::default())
            .unwrap();
    ...
    fn optimizer_1d_small_problem() {
        let opt = WorkgroupOptimizer::a770();
        let (wg, _) = opt
            .optimize_1d(8, &KernelConstraints::default())
            .unwrap();
        assert!(wg.total_size() <= 1024);
        assert!(wg.is_subgroup_aligned(16));
    }
    ...
    fn optimizer_1d_very_large_problem() {
        let opt = WorkgroupOptimizer::a770();
        let (wg, stats) = opt
            .optimize_1d(10_000_000, &KernelConstraints::default())
            .unwrap();
        assert!(wg.total_size() <= 1024);
    ...
    fn optimizer_2d_basic() {
        let opt = WorkgroupOptimizer::a770();
        let (wg, stats) = opt
            .optimize_2d(512, 512, &KernelConstraints::default())
            .unwrap();
        assert!(wg.total_size() <= 1024);
        assert!(stats.occupancy > 0.0);
    }
    ...
    fn optimizer_2d_nonsquare() {
        let opt = WorkgroupOptimizer::a770();
        let (wg, _) = opt
            .optimize_2d(4, 8192, &KernelConstraints::default())
            .unwrap();
        assert!(wg.total_size() <= 1024);
    }
    ...
    fn property_selected_within_device_max() {
        ...
        for size in [1, 7, 16, 100, 256, 1024, 4096, 100_000, 1_000_000] {
            if let Some((wg, _)) =
                opt.optimize_1d(size, &KernelConstraints::default())
            {
                assert!(
    ...
    fn property_selected_2d_within_device_max() {
        ...
        for (r, c) in [(1, 1), (4, 4), (16, 16), (100, 100), (512, 512), (2048, 2048)] {
            if let Some((wg, _)) =
                opt.optimize_2d(r, c, &KernelConstraints::default())
            {
                assert!(
                    wg.total_size() <= profile.max_workgroup_size,
                    "r={r} c={c} wg={}",
    ...
    fn edge_slm_limits_workgroup() {
        let opt = WorkgroupOptimizer::a770();
        let constraints = KernelConstraints {
            required_local_memory_bytes: 32768,
            ..Default::default()
        };
    ...
    fn edge_register_pressure_limits() {
        ...
        let constraints = KernelConstraints {
            estimated_registers_per_item: 120,
            ..Default::default()
        };
        let result = opt.optimize_1d(4096, &constraints);
        assert!(result.is_some());
