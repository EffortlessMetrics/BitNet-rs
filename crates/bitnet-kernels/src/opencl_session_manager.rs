...
static NEXT_SESSION_COUNTER: std::sync::atomic::AtomicU64 =
...
impl SessionId {
    ...
    pub fn new() -> Self {
        let counter =
            NEXT_SESSION_COUNTER.fetch_add(1, Ordering::Relaxed);
...
impl fmt::Display for SessionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Timeout { elapsed, limit } => write!(
                f,
                "session timed out after {elapsed:?} (limit {limit:?})"
            ),
...
impl InferenceSession {
    ...
    pub fn push_token(
        ...
        if let Some(start) = self.start_time {
            ...
            if elapsed >= self.config.timeout {
                ...
                self.last_error =
                    Some(format!("timeout after {elapsed:?}"));
        ...
        if let Some(first_t) = self.first_token_time {
            ...
            if decode_secs > 0.0 {
                self.metrics.tokens_per_sec =
                    self.generated_tokens.len() as f64 / decode_secs;
            }
        }
    ...
    pub fn restore(
        ...
        if cp.state == SessionState::Generating
            || cp.state == SessionState::Prefilling
        {
    ...
    fn finalize_metrics(&mut self) {
        ...
        if let (Some(start), Some(first)) =
            (self.start_time, self.first_token_time)
        {
        ...
        self.metrics.memory_used_bytes =
            1024 + self.generated_tokens.len() * 4;
    }
}
...
mod tests {
    ...
    fn config_negative_temperature_invalid() {
        let cfg =
            SessionConfig { temperature: -1.0, ..default_cfg() };
        assert!(matches!(
            cfg.validate(),
            Err(SessionError::InvalidConfig(_))
        ));
    ...
    fn config_zero_temperature_invalid() {
        let cfg =
            SessionConfig { temperature: 0.0, ..default_cfg() };
        assert!(matches!(
            cfg.validate(),
            Err(SessionError::InvalidConfig(_))
        ));
    ...
    fn config_nan_temperature_invalid() {
        let cfg =
            SessionConfig { temperature: f32::NAN, ..default_cfg() };
        assert!(matches!(
            cfg.validate(),
            Err(SessionError::InvalidConfig(_))
        ));
    ...
    fn idle_can_transition_to_prefilling() {
        assert!(SessionState::Idle
            .can_transition_to(SessionState::Prefilling));
    }

    #[test]
    fn idle_cannot_transition_to_generating() {
        assert!(!SessionState::Idle
            .can_transition_to(SessionState::Generating));
    }

    #[test]
    fn idle_cannot_transition_to_complete() {
        assert!(!SessionState::Idle
            .can_transition_to(SessionState::Complete));
    }

    #[test]
    fn prefilling_can_transition_to_generating() {
        assert!(SessionState::Prefilling
            .can_transition_to(SessionState::Generating));
    }

    #[test]
    fn generating_can_transition_to_complete() {
        assert!(SessionState::Generating
            .can_transition_to(SessionState::Complete));
    }
    ...
    fn any_non_terminal_can_transition_to_error() {
        for state in
            [SessionState::Idle, SessionState::Prefilling, SessionState::Generating]
        {
            assert!(
    ...
    fn state_display() {
        ...
        assert_eq!(
            SessionState::Generating.to_string(),
            "Generating"
        );
    ...
    fn error_display_invalid_config() {
        let e =
            SessionError::InvalidConfig("bad temp".into());
    ...
    fn error_is_std_error() {
        let e: Box<dyn std::error::Error> =
            Box::new(SessionError::Cancelled);
    ...
    fn session_with_id() {
        ...
        let s =
            InferenceSession::with_id(id, default_cfg()).unwrap();
        assert_eq!(s.id(), id);
    }
    ...
    fn property_error_eq() {
        ...
        let c =
            SessionError::InvalidConfig("x".into());
        let d =
            SessionError::InvalidConfig("x".into());
        assert_eq!(c, d);
    }
