...
impl OpNode {
    pub fn new(id: usize, op_type: OpType, name: impl Into<String>, output_shape: Vec<usize>) -> Self {
...
impl OpGraph {
    ...
    pub fn add_node(&mut self, op_type: OpType, name: impl Into<String>, output_shape: Vec<usize>) -> usize {
        let id = self.next_id;
        self.next_id += 1;
    ...
    pub fn merge_nodes(
        ...
        let combined_flops: u64 = ids.iter().filter_map(|id| self.nodes.get(id)).map(|n| n.flops).sum();
...
impl FusionOptimizer {
    ...
    pub fn optimize(
        ...
        for fk in fused_kernels.iter().rev() {
            let _new_id =
                graph.merge_nodes(&fk.original_node_ids, OpType::MatMul, &fk.name)?;
    ...
    fn build_chain(&self, graph: &OpGraph, start_id: usize, consumed: &HashSet<usize>) -> Vec<usize> {
...
mod tests {
    ...
    fn test_fused_vs_unfused_scale_silu() {
        ...
        let _fused = cpu_ref_execute_fused(
            &[OpType::Scale, OpType::SiLU],
            &input,
            Some(&scale),
        );
