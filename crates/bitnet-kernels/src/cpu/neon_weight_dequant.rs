...
pub fn neon_dequant_fp16_to_f32(fp16_bytes: &[u8]) -> Vec<f32> {
    ...
    unsafe {
        ...
        for chunk in 0..full_chunks {
            ...
            let half_vals = [h0, h1, h2, h3];
            let hv = vld1_u16(half_vals.as_ptr());
            // Reinterpret u16x4 as float16x4 for vcvt.
            let hf: float16x4_t = vreinterpret_f16_u16(hv);
            let fv: float32x4_t = vcvt_f32_f16(hf);

            let mut buf = [0.0f32; 4];
            vst1q_f32(buf.as_mut_ptr(), fv);
            out.extend_from_slice(&buf);
        }
        ...
        for i in (full_chunks * 4)..count {
            ...
            // Single-element NEON conversion for consistency.
            let half_arr = [bits, 0, 0, 0];
            let hv = vld1_u16(half_arr.as_ptr());
            let hf: float16x4_t = vreinterpret_f16_u16(hv);
            let fv: float32x4_t = vcvt_f32_f16(hf);
            let mut buf = [0.0f32; 4];
            vst1q_f32(buf.as_mut_ptr(), fv);
