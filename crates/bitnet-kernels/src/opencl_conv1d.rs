...
impl ConvStats {
    ...
    pub fn for_conv1d(
        ...
        let macs_per_elem =
            (in_channels / groups.max(1)) as u64 * kernel_size as u64;
        ...
        let input_bytes =
            (batch * in_channels * (output_length + kernel_size - 1)) as u64 * 4;
...
impl Conv1dKernel {
    ...
    pub fn forward(
        ...
        validate_conv1d_args(input, weight, bias, output, cfg, batch, in_channels, in_length, out_channels)?;
                    ...
                    for ic_local in 0..ch_per_group {
                        ...
                        for o in 0..out_len {
                            ...
                            for k in 0..cfg.kernel_size {
                                ...
                                if i_pos >= cfg.padding
                                    && i_pos - cfg.padding < in_length
                                {
...
impl DepthwiseConv1d {
    ...
    pub fn forward(
        ...
        Conv1dKernel::forward(input, weight, bias, output, cfg, batch, channels, in_length, channels)
    }
}
...
impl Conv1dTranspose {
    ...
    pub fn forward(
        ...
        validate_transpose_args(
            input, weight, bias, output, cfg, batch, in_channels, in_length, out_channels, out_len,
        )?;
                ...
                for ic_local in 0..ch_per_group {
                    ...
                    for oc_local in 0..oc_per_group {
                        ...
                        for i in 0..in_length {
                            let in_idx =
                                n * in_channels * in_length + ic * in_length + i;
                            let in_val = input[in_idx];
                            for k in 0..cfg.kernel_size {
                                let o_pos_raw =
                                    i * cfg.stride + k * cfg.dilation;
...
impl WinoGrad1d {
    ...
    pub fn forward(
        ...
        validate_conv1d_args(
            input, weight, bias, output, cfg, batch, in_channels, in_length, out_channels,
        )?;
                ...
                for oc_local in 0..oc_per_group {
                    ...
                    for ic_local in 0..ch_per_group {
                        ...
                        for tile in 0..num_tiles {
                            ...
                            let d = [
                                get_padded(input, n, ic, in_channels, in_length, in_start, cfg.padding),
                                get_padded(input, n, ic, in_channels, in_length, in_start + 1, cfg.padding),
                                get_padded(input, n, ic, in_channels, in_length, in_start + 2, cfg.padding),
                                get_padded(input, n, ic, in_channels, in_length, in_start + 3, cfg.padding),
                            ];
...
fn validate_conv1d_args(
    ...
    if input.len() != expected_input {
        return Err(KernelError::InvalidArguments {
            reason: format!(
                "input size mismatch: expected {expected_input}, got {}",
                input.len()
            ),
        }
        .into());
        ...
        && b.len() != out_channels
    {
        return Err(KernelError::InvalidArguments {
            reason: format!(
                "bias size mismatch: expected {out_channels}, got {}",
                b.len()
            ),
        }
        .into());
...
mod tests {
                        ...
                        for ic_l in 0..ch_per_g {
                            ...
                            for k in 0..kernel_size {
                                ...
                                if i_pos >= padding && i_pos - padding < in_len {
                                    let idx =
                                        n * in_ch * in_len + ic * in_len + (i_pos - padding);
                                    let w_idx = oc * ch_per_g * kernel_size + ic_l * kernel_size + k;
    ...
    fn conv1d_kernel1_with_bias() {
        ...
        Conv1dKernel::forward(&input, &weight, Some(&bias), &mut output, &cfg, 1, 1, 3, 1)
            .unwrap();
    ...
    fn conv1d_kernel3_cross_check_with_ref() {
        ...
        Conv1dKernel::forward(&input, &weight, Some(&bias), &mut output, &cfg, 1, 3, 4, 2)
            .unwrap();
    ...
    fn depthwise_rejects_wrong_groups() {
        ...
        assert!(DepthwiseConv1d::forward(&input, &weight, None, &mut output, &cfg, 1, 4, 5).is_err());
    }
    ...
    fn depthwise_with_bias() {
        ...
        DepthwiseConv1d::forward(&input, &weight, Some(&bias), &mut output, &cfg, 1, 2, 5)
            .unwrap();
    ...
    fn causal_with_dilation() {
        ...
        let out_len = causal_output_length(in_len, &cfg);
        let mut output = vec![0.0; out_len];
        CausalConv1d::forward(&input, &weight, None, &mut output, &cfg, 1, 1, in_len, 1)
            .unwrap();
    ...
    fn winograd_matches_naive_groups() {
        ...
        Conv1dKernel::forward(&input, &weight, None, &mut out_naive, &cfg, 1, in_ch, in_len, out_ch)
            .unwrap();
    ...
    fn winograd_rejects_kernel_size_5() {
        ...
        assert!(WinoGrad1d::forward(&input, &weight, None, &mut output, &cfg, 1, 1, 10, 1).is_err());
    }
    ...
    fn error_on_zero_batch() {
        ...
        let weight = vec![1.0; 3];
        let mut output = vec![];
        assert!(Conv1dKernel::forward(&input, &weight, None, &mut output, &cfg, 0, 1, 5, 1).is_err());
    }
    ...
    fn error_on_wrong_output_size() {
        ...
        assert!(Conv1dKernel::forward(&input, &weight, None, &mut output, &cfg, 1, 1, 5, 1).is_err());
    }
    ...
    fn error_on_channels_not_divisible_by_groups() {
        ...
        let weight = vec![0.0; 3];
        let mut output = vec![0.0; 6];
        assert!(Conv1dKernel::forward(&input, &weight, None, &mut output, &cfg, 1, 4, 5, 4).is_err());
    }
