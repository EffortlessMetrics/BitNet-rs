    ...
    pub fn generate_with_global(
        ...
        for i in 0..seq_len {
            ...
            for j in 0..kv_len {
                ...
                let block_j = j / block_size;
                data[i * kv_len + j] =
                    is_global_i || is_global_j || block_i == block_j;
            }
        }
        MaskData::new(data, seq_len, kv_len)
    }
...
impl MaskExpander {
    ...
    pub fn expand(
        ...
        // Build one 2D plane
        let plane: Vec<f32> = mask
            .as_slice()
            .iter()
            .map(|&v| MaskConverter::bool_to_float(v, format))
            .collect();
...
impl MaskConverter {
    ...
    pub fn bool_to_float(value: bool, format: MaskFormat) -> f32 {
        match format {
            ...
            MaskFormat::Float => {
                if value { 0.0 } else { f32::NEG_INFINITY }
            }
            MaskFormat::Additive => {
                if value { 0.0 } else { ADDITIVE_MASK_VALUE }
            }
    ...
    pub fn from_float(
        ...
        let data: Vec<bool> =
            values.iter().map(|&v| Self::float_to_bool(v, format)).collect();
        MaskData::new(data, seq_len, kv_len)
    }
...
mod tests {
    ...
    fn sliding_window_active_count() {
        ...
        let expected: usize =
            (1..=w.min(n)).sum::<usize>() + (n.saturating_sub(w)) * w;
    ...
    fn convert_bool_to_multiplicative_roundtrip() {
        ...
        let back =
            MaskConverter::from_float(&mult, 4, 4, MaskFormat::Multiplicative);
        assert_eq!(back.as_slice(), m.as_slice());
    }
    ...
    fn convert_float_to_additive() {
        ...
        let additive = MaskConverter::convert(
            &float_vals,
            MaskFormat::Float,
            MaskFormat::Additive,
        );
    ...
    fn convert_multiplicative_to_bool() {
        ...
        let mult = MaskConverter::to_float(&m, MaskFormat::Multiplicative);
        let bool_vals =
            MaskConverter::convert(&mult, MaskFormat::Multiplicative, MaskFormat::Bool);
    ...
    fn convert_values_true() {
        ...
        assert_eq!(
            MaskConverter::bool_to_float(true, MaskFormat::Multiplicative),
            1.0
        );
    }
    ...
    fn convert_values_false() {
        ...
        assert_eq!(
            MaskConverter::bool_to_float(false, MaskFormat::Float),
            f32::NEG_INFINITY
        );
        assert_eq!(
            MaskConverter::bool_to_float(false, MaskFormat::Additive),
            ADDITIVE_MASK_VALUE
        );
        assert_eq!(
            MaskConverter::bool_to_float(false, MaskFormat::Multiplicative),
            0.0
        );
    }
    ...
    fn property_causal_mask_true_count() {
        for n in 1..=20 {
            ...
            assert_eq!(
                m.count_true(),
                n * (n + 1) / 2,
                "n={n}: expected n*(n+1)/2 true entries"
            );
        }
    }
                ...
                for i in 0..n {
                    for j in 0..n {
                        if j > i {
                            ...
                            assert!(
                                !m.allows(i, j),
                                "outside window ({i},{j}) w={w}"
                            );
                        } else {
                            // Within window: allowed.
                            assert!(
                                m.allows(i, j),
                                "within window ({i},{j}) w={w}"
                            );
                        }
                    }
                }
