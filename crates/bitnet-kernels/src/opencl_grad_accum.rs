...
impl GradClipping {
    ...
    pub fn clip_by_global_norm(
        buffers: &mut [&mut GradBuffer],
        max_norm: f32,
    ) -> f32 {
        let total_sq: f32 =
            buffers.iter().flat_map(|b| b.data().iter()).map(|v| v * v).sum();
...
// ---------------------------------------------------------------------------
// MixedPrecisionGrad — FP16 ↔ FP32 helpers
// ---------------------------------------------------------------------------
...
impl MixedPrecisionGrad {
    /// Convert an FP32 slice to FP16 (returned as `half::f16` words).
        ...
        for (name, data) in &checkpoint.buffers {
            if let Some(buf) = accumulator.buffers.get_mut(name) {
                assert_eq!(
                    buf.len(),
                    data.len(),
                    "checkpoint buffer size mismatch for '{name}'"
                );
...
mod tests {
    ...
    fn test_accumulator_basic_flow() {
        ...
        let g1: HashMap<String, Vec<f32>> =
            [("w".into(), vec![1.0, 2.0, 3.0, 4.0])].into();
        let g2: HashMap<String, Vec<f32>> =
            [("w".into(), vec![2.0, 3.0, 4.0, 5.0])].into();
        let g3: HashMap<String, Vec<f32>> =
            [("w".into(), vec![3.0, 4.0, 5.0, 6.0])].into();
    ...
    fn test_clip_multi_buffer_global_norm() {
        ...
        let orig = GradClipping::clip_by_global_norm(
            &mut [&mut b1, &mut b2],
            1.0,
        );
        assert!((orig - 5.0).abs() < 1e-5);
        let total_sq: f32 = b1.data().iter().chain(b2.data().iter())
            .map(|v| v * v).sum();
    ...
    fn test_checkpoint_load_creates_missing_buffer() {
        ...
        let g: HashMap<String, Vec<f32>> =
            [("existing".into(), vec![1.0, 2.0])].into();
        acc.accumulate(&g);
        let ckpt = GradCheckpointer::save(&acc, None);
