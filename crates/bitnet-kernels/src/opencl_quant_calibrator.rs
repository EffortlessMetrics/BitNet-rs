...
impl CalibrationDataset {
        ...
        if !self.bin_edges.is_empty() {
            ...
            if range <= 0.0 {
                ...
                for &v in values {
                    let vf = v as f64;
                    let idx =
                        ((vf - lo) / range * n_bins as f64).floor() as usize;
    ...
    fn rebuild_histogram(&mut self) {
        ...
        self.bin_edges = (0..=n_bins)
            .map(|i| lo + range * (i as f64 / n_bins as f64))
            .collect();
...
impl ScaleComputer {
    ...
    pub fn determine_range(
        ...
        match method {
            ...
            QuantRangeMethod::Entropy => {
                Self::entropy_optimal_range(dataset)
            }
        }
    }
    ...
    fn entropy_optimal_range(dataset: &CalibrationDataset) -> (f64, f64) {
        ...
        let p_dist: Vec<f64> =
            dataset.histogram.iter().map(|&c| c as f64 / total as f64).collect();
    ...
    fn build_quantised_distribution(
        p: &[f64],
        src_bins: usize,
        n_quant_levels: usize,
    ) -> Vec<f64> {
        if src_bins == 0 {
            return Vec::new();
        }
...
impl QuantSimulator {
    ...
    pub fn quantize_asymmetric(value: f32, scale: f64, zero_point: i32, bits: u32) -> i32 {
        ...
        ((value as f64) / scale + zero_point as f64)
            .round()
            .clamp(0.0, qmax) as i32
    }
    ...
    pub fn simulate(
        ...
        let snr_db = if mse > 0.0 {
            10.0 * (sum_sq_signal / n / mse).log10()
        } else {
            f64::INFINITY
        };
...
impl fmt::Display for LayerReport {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "[{}] {} {}bit {}: {}  rec={}",
            self.layer_name, self.scheme, self.bits, self.method, self.metrics,
            self.recommendation,
        )
    }
}
    ...
    pub fn analyse(&self, observer: &CalibrationObserver) -> Vec<LayerQuantDecision> {
        ...
        for name in sorted_names {
            ...
            for &method in &methods {
                let sz =
                    ScaleComputer::compute(dataset, method, self.default_bits, self.symmetric);
...
pub fn cpu_quant_simulate_symmetric(
    input: &[f32],
    scale: f64,
    bits: u32,
) -> (Vec<f32>, Vec<f32>) {
...
mod tests {
    ...
    fn test_scale_from_dataset_percentile() {
        ...
        let sz_pc =
            ScaleComputer::compute(&ds, QuantRangeMethod::Percentile(99.0), 8, true);
    ...
    fn test_quantize_dequantize_asymmetric_roundtrip() {
        ...
        for v in [0.0f32, 0.25, 0.5, 0.75, 1.0] {
            let q =
                QuantSimulator::quantize_asymmetric(v, sz.scale, sz.zero_point, 8);
    ...
    fn test_cpu_asymmetric_simulate() {
        ...
        let sz = ScaleComputer::compute_asymmetric(0.0, 1.0, 8);
        let (output, errors) =
            cpu_quant_simulate_asymmetric(&input, sz.scale, sz.zero_point, 8);
