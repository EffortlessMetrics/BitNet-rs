...
impl KvQuantizer {
    ...
    fn quantize_int8(
        ...
        for h in 0..num_heads {
            ...
            let absmax =
                head_data.iter().copied().map(f32::abs).fold(0.0f32, f32::max);
        ...
        for h in 0..num_heads {
            ...
            for i in (0..total_per_head).step_by(2) {
                ...
                let q1 = if i + 1 < total_per_head {
                    let v1 =
                        (head_data[i + 1] * inv_scale).round().clamp(-7.0, 7.0) as i8;
                    (v1 as u8) & 0x0F
                } else {
                    0
                };
                    ...
                    EvictionPolicy::Hybrid { attention_weight } => {
                        ...
                        let max_order =
                            self.entries.iter().map(|e| e.insertion_order).max().unwrap_or(1)
                                as f64;
                        let recency =
                            if max_order > 0.0 { e.insertion_order as f64 / max_order } else { 0.0 };
...
mod tests {
    ...
    fn merge_identical_pairs() {
        ...
        for &v in &merged {
            ...
            assert!((v - data[merged.len() - 3 + merged.iter().position(|&x| x == v).unwrap()]).abs() < 1e-6);
        }
    }
    ...
    fn int8_compressed_size_leq_original() {
        ...
        for heads in [1, 2, 4] {
            for seq in [1, 8, 32] {
                let dim = 16;
                let data: Vec<f32> =
                    (0..(heads * seq * dim)).map(|i| i as f32 * 0.01).collect();
