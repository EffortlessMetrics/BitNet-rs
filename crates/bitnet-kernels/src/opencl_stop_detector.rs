...
impl AhoCorasick {
    ...
    fn new(patterns: &[Vec<u32>]) -> Self {
        ...
        let root_children: Vec<usize> =
            nodes[0].children.values().copied().collect();
        ...
        while let Some(u) = queue.pop_front() {
            ...
            for (tok, v) in transitions {
                ...
                let fail_out =
                    nodes[nodes[v].failure].output.clone();
    ...
    fn feed(&mut self, token: u32) -> &[usize] {
        loop {
            if let Some(&next) =
                self.nodes[self.current].children.get(&token)
            {
                self.current = next;
...
impl StopDetector {
    ...
    pub fn new(sequences: Vec<StopSequence>) -> Self {
        let patterns: Vec<Vec<u32>> =
            sequences.iter().map(|s| s.tokens.clone()).collect();
...
impl StopCondition for StopDetector {
    fn feed_token(&mut self, token: u32) -> Option<StopReason> {
        ...
        idx.map(|i| {
            StopReason::StopSequence(self.sequences[i].name.clone())
        })
    }
...
impl StopCondition for RepeatDetector {
    fn feed_token(&mut self, token: u32) -> Option<StopReason> {
        ...
        self.check_repeat()
            .map(|(period, count)| StopReason::RepeatLoop { period, count })
    }
...
impl StopCondition for ContentFilter {
    fn feed_token(&mut self, token: u32) -> Option<StopReason> {
        let idx = self.automaton.feed(token).first().copied();
        idx.map(|i| {
            StopReason::ContentBlock(self.pattern_names[i].clone())
        })
    }
...
mod tests {
    ...
    fn test_stop_reason_eq() {
        ...
        assert_eq!(
            StopReason::StopSequence("a".into()),
            StopReason::StopSequence("a".into()),
        );
        assert_ne!(
            StopReason::StopSequence("a".into()),
            StopReason::StopSequence("b".into()),
        );
    }
    ...
    fn test_single_stop_sequence() {
        let mut det = StopDetector::new(vec![
            StopSequence::new("end", vec![10, 20, 30]),
        ]);
    ...
    fn test_stop_detector_partial_match_then_diverge() {
        let mut det = StopDetector::new(vec![
            StopSequence::new("seq", vec![1, 2, 3]),
        ]);
    ...
    fn test_stop_detector_repeated_token_pattern() {
        let mut det = StopDetector::new(vec![
            StopSequence::new("aaa", vec![7, 7, 7]),
        ]);
    ...
    fn test_repeat_period_three() {
        ...
        assert!(matches!(
            result,
            Some(StopReason::RepeatLoop { period: 3, .. })
        ));
    }
    ...
    fn test_repeat_single_token() {
        ...
        assert!(matches!(
            r,
            Some(StopReason::RepeatLoop { period: 1, count: 3 })
        ));
    }
    ...
    fn test_content_filter_first_token_block() {
        ...
        assert_eq!(
            r,
            Some(StopReason::ContentBlock("single".into()))
        );
    }
    ...
    fn test_chain_stop_seq_priority() {
        let mut chain = DetectorChain::new()
            .with(Box::new(StopDetector::new(vec![
                StopSequence::new("s", vec![5]),
            ])))
            .with(Box::new(MaxTokensGuard::new(1)));
    ...
    fn test_chain_stats_tracking() {
        let mut chain = DetectorChain::new()
            .with(Box::new(EosDetector::new(9)));
        chain.feed_token(1);
        chain.feed_token(9);
    ...
    fn test_chain_reset() {
        let mut chain = DetectorChain::new()
            .with(Box::new(MaxTokensGuard::new(2)));
        chain.feed_token(0);
        chain.feed_token(0);
        chain.reset();
    ...
    fn test_chain_first_trigger_wins() {
        ...
        let mut chain = DetectorChain::new()
            .with(Box::new(StopDetector::new(vec![
                StopSequence::new("first", vec![1]),
            ])))
            .with(Box::new(StopDetector::new(vec![
                StopSequence::new("second", vec![1]),
            ])));
        let r = chain.feed_token(1);
    ...
    fn test_chain_content_filter_blocks() {
        let mut chain = DetectorChain::new()
            .with(Box::new(ContentFilter::new(vec![
                ("blocked".into(), vec![8, 9]),
            ])))
        ...
        assert_eq!(
            r,
            Some(StopReason::ContentBlock("blocked".into()))
        );
    }
    ...
    fn test_chain_all_detectors_combined() {
        let mut chain = DetectorChain::new()
            ...
            .with(Box::new(StopDetector::new(vec![
                StopSequence::new("stop", vec![10, 20]),
            ])))
            .with(Box::new(ContentFilter::new(vec![
                ("bad".into(), vec![30, 40]),
            ])))
    ...
    fn test_edge_very_long_sequence() {
        let mut det = StopDetector::new(vec![
            StopSequence::new("end", vec![99_999]),
        ]);
        for tok in 0..10_000 {
            assert!(det.feed_token(tok).is_none());
        }
