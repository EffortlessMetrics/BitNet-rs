...
pub const SCATTER_GATHER_CL: &str =
    include_str!("gpu/kernels/scatter_gather.cl");
    ...
    pub fn new(
        ...
        if axis > 1 {
            return Err(KernelError::InvalidArguments {
                reason: format!(
                    "gather axis must be 0 or 1, got {axis}"
                ),
            }
            .into());
        }
...
impl ScatterOp {
    ...
    pub fn new(
        ...
        if axis > 1 {
            return Err(KernelError::InvalidArguments {
                reason: format!(
                    "scatter axis must be 0 or 1, got {axis}"
                ),
            }
            .into());
        }
    ...
    pub fn execute(
        ...
        for i in 0..i_rows {
            for j in 0..i_cols {
                ...
                let clamped = idx.min(bound.saturating_sub(1));
                let dst_off = if self.axis == 0 {
                    clamped * d_cols + j
                } else {
                    i * d_cols + clamped
                };
                dst[dst_off] =
                    self.reduce.combine(dst[dst_off], src[flat]);
    ...
    pub fn execute_mean(
        ...
        let add_op = ScatterOp {
            reduce: ScatterReduce::Mean,
            ..self.clone()
        };
    ...
    pub fn execute(
        ...
        for (out_row, &idx) in indices.iter().enumerate() {
            if self.bounds_check && idx >= s_rows {
                return Err(KernelError::InvalidArguments {
                    reason: format!(
                        "index_select index {idx} >= rows {s_rows}",
                    ),
                }
                .into());
            }
...
impl TopKSelect {
    ...
    pub fn execute(&self, input: &[f32]) -> Result<TopKResult> {
        ...
        let mut pairs: Vec<(f32, usize)> = input
            .iter()
            .copied()
            .enumerate()
            .map(|(i, v)| (v, i))
        ...
        let mut selected: Vec<(f32, usize)> =
            pairs[..self.k].to_vec();
        ...
        let indices: Vec<usize> =
            selected.iter().map(|p| p.1).collect();
...
impl ScatterGatherStats {
    ...
    pub fn bandwidth_bytes_per_sec(&self) -> f64 {
        let secs = self.elapsed.as_secs_f64();
        if secs > 0.0 {
            (self.bytes_read + self.bytes_written) as f64 / secs
        } else {
            0.0
        }
    }
...
mod tests {
    ...
    fn kernel_source_is_non_empty() {
        assert!(
            !SCATTER_GATHER_CL.is_empty(),
            "OpenCL source must be embedded"
        );
    }
    ...
    fn gather_large_tensor() {
        ...
        let cols = 64;
        let src: Vec<f32> =
            (0..(rows * cols) as u32).map(|x| x as f32).collect();
        let n_sel = 50;
        let indices: Vec<usize> =
            (0..n_sel).flat_map(|i| vec![i * 2; cols]).collect();
        let op =
            GatherOp::new((rows, cols), (n_sel, cols), 0, true).unwrap();
    ...
    fn scatter_op_rejects_axis2() {
        assert!(
            ScatterOp::new(
                (3, 2), (1, 2), 2, ScatterReduce::Assign, true
            )
            .is_err()
        );
    }
    ...
    fn scatter_op_rejects_shape_mismatch_axis0() {
        assert!(
            ScatterOp::new(
                (3, 2), (1, 3), 0, ScatterReduce::Assign, true
            )
            .is_err()
        );
    }
    ...
    fn scatter_op_rejects_shape_mismatch_axis1() {
        assert!(
            ScatterOp::new(
                (3, 2), (4, 1), 1, ScatterReduce::Assign, true
            )
            .is_err()
        );
    }
    ...
    fn scatter_assign_axis0() {
        ...
        let indices = [2, 2];
        let op = ScatterOp::new(
            (3, 2), (1, 2), 0, ScatterReduce::Assign, true,
        )
        .unwrap();
    ...
    fn scatter_add_accumulates() {
        ...
        let op =
            ScatterOp::new((2, 2), (2, 2), 0, ScatterReduce::Add, true)
                .unwrap();
    ...
    fn scatter_mul_accumulates() {
        ...
        let indices = [0, 0, 0, 0]; // all target row 0
        let op =
            ScatterOp::new((2, 2), (2, 2), 0, ScatterReduce::Mul, true)
                .unwrap();
    ...
    fn scatter_max_keeps_max() {
        ...
        let indices = [0, 0, 0, 0];
        let op =
            ScatterOp::new((2, 2), (2, 2), 0, ScatterReduce::Max, true)
                .unwrap();
    ...
    fn scatter_min_keeps_min() {
        ...
        let indices = [0, 0, 0, 0];
        let op =
            ScatterOp::new((2, 2), (2, 2), 0, ScatterReduce::Min, true)
                .unwrap();
    ...
    fn scatter_axis1() {
        ...
        let op = ScatterOp::new(
            (2, 3), (2, 1), 1, ScatterReduce::Assign, true,
        )
        .unwrap();
        let mut dst = [0.0f32; 6];
        op.execute(&src, &indices, &mut dst, None).unwrap();
    ...
    fn scatter_oob_with_bounds_check() {
        ...
        let indices = [99, 0];
        let op = ScatterOp::new(
            (2, 2), (1, 2), 0, ScatterReduce::Assign, true,
        )
        .unwrap();
        let mut dst = [0.0f32; 4];
        assert!(
            op.execute(&src, &indices, &mut dst, None).is_err()
        );
    }
    ...
    fn scatter_src_too_small_error() {
        let src = [1.0];
        let op = ScatterOp::new(
            (2, 2), (1, 2), 0, ScatterReduce::Assign, false,
        )
        .unwrap();
        let mut dst = [0.0f32; 4];
        assert!(
            op.execute(&src, &[0, 0], &mut dst, None).is_err()
        );
    }
    ...
    fn scatter_mean_basic() {
        ...
        let op = ScatterOp::new(
            (2, 1), (3, 1), 0, ScatterReduce::Mean, true,
        )
        .unwrap();
    ...
    fn scatter_with_counts() {
        ...
        let indices = [0, 0, 1];
        let op =
            ScatterOp::new((2, 1), (3, 1), 0, ScatterReduce::Add, true)
                .unwrap();
        let mut dst = [0.0f32; 2];
        let mut counts = [0u32; 2];
        op.execute(&src, &indices, &mut dst, Some(&mut counts))
            .unwrap();
    ...
    fn gather_scatter_roundtrip_identity() {
        ...
        let gather_indices: Vec<usize> =
            perm.iter().flat_map(|&r| vec![r; cols]).collect();
        let gather_op =
            GatherOp::new((3, cols), (3, cols), 0, true).unwrap();
        let mut gathered = [0.0f32; 6];
        gather_op
            .execute(&src, &gather_indices, &mut gathered)
            .unwrap();
        ...
        scatter_op
            .execute(&gathered, &scatter_indices, &mut result, None)
            .unwrap();
    ...
    fn masked_select_length_mismatch() {
        assert!(
            MaskedSelect::execute(&[1.0, 2.0], &[true]).is_err()
        );
    }
    ...
    fn masked_select_into_buffer() {
        ...
        let mut out = [0.0f32; 2];
        let n =
            MaskedSelect::execute_into(&input, &mask, &mut out)
                .unwrap();
        assert_eq!(n, 2);
    ...
    #[test]
    fn stats_throughput() {
        let stats = ScatterGatherStats::new(
            1000,
    ...
    fn stats_bandwidth() {
        ...
        assert!(
            (stats.bandwidth_bytes_per_sec() - 8000.0).abs() < 1e-6
        );
    }
    ...
    fn gather_timed_returns_stats() {
        ...
        let mut out = [0.0f32; 2];
        let stats =
            gather_timed(&op, &src, &[1, 0], &mut out).unwrap();
    ...
    fn scatter_timed_returns_stats() {
        ...
        let mut dst = [0.0f32; 4];
        let stats =
            scatter_timed(&op, &src, &[1, 1], &mut dst).unwrap();
        assert_eq!(stats.element_count, 2);
    ...
    fn property_gather_preserves_values() {
        ...
        let src: Vec<f32> =
            (0..20).map(|x| x as f32 * 1.5).collect();
        let indices: Vec<usize> = (0..10).map(|i| i % 4).collect();
        let op =
            GatherOp::new((4, 5), (2, 5), 0, true).unwrap();
    ...
    fn property_scatter_add_commutative() {
        ...
        let op1 = ScatterOp::new(
            (2, 2), (1, 2), 0, ScatterReduce::Add, true,
        )
        .unwrap();
        let op2 = ScatterOp::new(
            (2, 2), (1, 2), 0, ScatterReduce::Add, true,
        )
        .unwrap();
    ...
    fn property_masked_fill_select_inverse() {
        ...
        let selected =
            MaskedSelect::execute(&filled, &inv_mask).unwrap();
    ...
    fn property_gather_identity_with_sequential_indices() {
        ...
        let src: Vec<f32> = (0..12).map(|x| x as f32).collect();
        let indices: Vec<usize> =
            (0..4).flat_map(|r| vec![r; 3]).collect();
        let op =
            GatherOp::new((4, 3), (4, 3), 0, true).unwrap();
