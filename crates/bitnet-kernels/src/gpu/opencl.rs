        ...
        for platform in &platforms {
            ...
            for device_id in device_ids {
                ...
                if vendor.to_lowercase().contains("intel") {
                    ...
                    let rope_program =
                        Self::compile_program(&context, crate::kernels::ROPE_SRC, "rope");
...
impl OpenClKernel {
    ...
    pub fn rope_apply(
        ...
        let program = ctx
            .rope_program
            .as_ref()
            .ok_or_else(|| KernelError::GpuError { reason: "rope program not compiled".into() })?;
        ...
        let mut buf_x = unsafe {
            ...
            .map_err(|e| KernelError::GpuError { reason: format!("Buffer x create: {}", e) })?
        };
        ...
        unsafe {
            ctx.queue
                ...
                .map_err(|e| KernelError::GpuError { reason: format!("Write x: {}", e) })?;
            ctx.queue
                .enqueue_write_buffer(&mut buf_cos, CL_BLOCKING, 0, freq_cos, &[])
                .map_err(|e| KernelError::GpuError { reason: format!("Write freq_cos: {}", e) })?;
            ctx.queue
                .enqueue_write_buffer(&mut buf_sin, CL_BLOCKING, 0, freq_sin, &[])
                .map_err(|e| KernelError::GpuError { reason: format!("Write freq_sin: {}", e) })?;
        }

        let kernel = Kernel::create(program, "rope_apply")
            .map_err(|e| KernelError::GpuError { reason: format!("Kernel create: {}", e) })?;
        ...
        unsafe {
            ctx.queue
                ...
                .map_err(|e| KernelError::GpuError { reason: format!("Read x: {}", e) })?;
        }
