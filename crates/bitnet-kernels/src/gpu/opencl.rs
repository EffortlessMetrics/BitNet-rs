        ...
        for platform in &platforms {
            ...
            for device_id in device_ids {
                ...
                if vendor.to_lowercase().contains("intel") {
                    ...
                    let rmsnorm_program =
                        Self::compile_program(&context, crate::kernels::RMSNORM_SRC, "rmsnorm");
...
impl OpenClKernel {
    ...
    pub fn rms_norm(
        ...
        let mut buf_input = unsafe {
            Buffer::<f32>::create(&ctx.context, CL_MEM_READ_ONLY, input.len(), std::ptr::null_mut())
                .map_err(|e| KernelError::GpuError {
        ...
        let mut buf_weight = unsafe {
            ...
            .map_err(|e| KernelError::GpuError { reason: format!("Buffer weight create: {}", e) })?
        };
        let buf_output = unsafe {
            ...
            .map_err(|e| KernelError::GpuError { reason: format!("Buffer output create: {}", e) })?
        };

        unsafe {
            ctx.queue
                .enqueue_write_buffer(&mut buf_input, CL_BLOCKING, 0, input, &[])
                .map_err(|e| KernelError::GpuError { reason: format!("Write input: {}", e) })?;
            ctx.queue
                .enqueue_write_buffer(&mut buf_weight, CL_BLOCKING, 0, weight, &[])
                .map_err(|e| KernelError::GpuError { reason: format!("Write weight: {}", e) })?;
        }

        let kernel = Kernel::create(program, "rms_norm_parallel")
            .map_err(|e| KernelError::GpuError { reason: format!("Kernel create: {}", e) })?;
        ...
        unsafe {
            ctx.queue
                ...
                .map_err(|e| KernelError::GpuError { reason: format!("Read output: {}", e) })?;
        }
