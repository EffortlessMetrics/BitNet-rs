...
pub fn qk256_dequantize_ref(packed_data: &[u8], scales: &[f32], num_blocks: usize) -> Vec<f32> {
...
pub fn qk256_apply_scales_ref(accumulated: &[i32], scales: &[f32]) -> Vec<f32> {
    accumulated.iter().zip(scales.iter()).map(|(&acc, &scale)| acc as f32 * scale).collect()
}
...
pub fn tl1_pack_ref(input: &[f32], group_size: usize) -> (Vec<u8>, Vec<f32>) {
    ...
    for group_id in 0..num_groups {
        ...
        let absmax = input[start..end].iter().map(|x| x.abs()).fold(0.0f32, f32::max);
