...
impl SpecConfig {
    pub fn new(
        draft_model_id: impl Into<String>,
        gamma: usize,
...
impl SpeculativeDecoder {
    ...
    pub fn step(
        ...
        for (i, target_logits_row) in
            target_logits_per_pos.iter().enumerate().take(gamma)
        {
        ...
        let corrected_token = self
            .verifier
            .sample_corrected(&target_logits_per_pos[correction_pos]);
...
mod tests {
    ...
    fn test_typical_accepts_high_prob_token() {
        let mut v = TokenVerifier::new(
            AcceptanceMethod::Typical { threshold: 0.9 },
            42,
        );
    ...
    fn test_typical_rejects_low_prob_token() {
        let mut v = TokenVerifier::new(
            AcceptanceMethod::Typical { threshold: 0.1 },
            42,
        );
    ...
    fn test_adaptive_gamma_respects_min_bound() {
        let mut ag = AdaptiveGamma::new(2)
            .with_bounds(2, 8)
            .with_thresholds(0.8, 0.3)
            .with_window(2);
        for _ in 0..10 {
    ...
    fn test_decoder_gamma_adapts() {
        ...
        for _ in 0..10 {
            let draft = DraftProposal::new(
                vec![1; dec.current_gamma()],
                vec![-0.1; dec.current_gamma()],
            );
            let gamma = dec.current_gamma();
            let target: Vec<Vec<f32>> = (0..gamma + 1)
                .map(|_| logits_with_max(10, 1))
                .collect();
    ...
    fn test_cpu_verify_batch_greedy_all_match() {
        ...
        let mask = cpu_verify_batch(&draft_tokens, &draft_lps, &target, 10, AcceptanceMethod::Greedy, 42);
    ...
    fn test_output_always_has_at_least_one_token() {
        for gamma in 1..=6 {
            ...
            assert!(
                !res.output_tokens.is_empty(),
                "output must never be empty (gamma={gamma})"
            );
        }
    }
