...
impl PoolInner {
    ...
    fn alloc_best_fit(&mut self, size: usize) -> Option<AllocMeta> {
        ...
        let meta =
            AllocMeta { id, offset, size, aligned_size: aligned, last_access: ts };
    ...
    fn free(&mut self, id: u64) -> bool {
        ...
        let pos =
            self.free_blocks.partition_point(|b| b.offset < new_block.offset);
...
impl fmt::Debug for ArenaAllocator {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ArenaAllocator")
            .field("live_allocations", &self.handles.len())
            .finish()
    }
}
...
impl ArenaAllocator {
    ...
    pub fn allocated_bytes(&self) -> usize {
        ...
        self.handles
            .iter()
            .filter_map(|id| inner.allocations.get(id))
            .map(|m| m.aligned_size)
            .sum()
    }
...
impl SlabAllocator {
    ...
    pub fn alloc(&mut self) -> Result<AllocMeta, PoolError> {
        ...
        let mut inner = self.inner.lock().unwrap();
        let meta = inner
            .alloc_best_fit(self.element_size)
            .ok_or_else(|| PoolError::OutOfMemory {
...
mod tests {
    ...
    fn test_error_is_error_trait() {
        let e: Box<dyn std::error::Error> =
            Box::new(PoolError::InvalidHandle(7));
