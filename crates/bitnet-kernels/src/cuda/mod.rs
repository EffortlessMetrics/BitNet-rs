...
//! - [`memory_bandwidth`]: Memory bandwidth optimization (coalescing, alignment, prefetch,
//!   vectorized loads/stores, bank-conflict-free access, hierarchy statistics)
//!
...
pub use memory_bandwidth::{
    BandwidthConfig, CachePolicy, MemoryAccessPattern, MemoryAnalysis, MemoryHierarchyStats,
    align_buffer, analyze_access_pattern, bank_conflict_free_access, coalesced_copy,
    estimate_bandwidth, memory_hierarchy_stats, optimize_layout, prefetch_buffer,
    transpose_for_coalescing, vectorized_load, vectorized_store,
};

#[cfg(any(feature = "gpu", feature = "cuda"))]
pub use memory_bandwidth::{
    MEMORY_BANDWIDTH_KERNEL_SRC, launch_coalesced_copy, launch_transpose_coalesced,
    launch_vectorized_copy,
};
