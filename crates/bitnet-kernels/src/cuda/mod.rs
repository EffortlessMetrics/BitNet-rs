...
//! - [`error_correction`]: Software ECC — parity, Hamming(7,4), CRC-32, checksum, TMR
...
pub use attention::{
    ...
    AttentionVMixConfig, CUDA_DENSE_ATTENTION_SCORE_KERNEL_ID,
    CUDA_DENSE_ATTENTION_SCORE_REFERENCE_BACKEND, CUDA_DENSE_ATTENTION_SCORE_TARGET_BACKEND,
    CUDA_DENSE_ATTENTION_SCORE_TOLERANCE, CUDA_DENSE_ATTENTION_SOFTMAX_KERNEL_ID,
    CUDA_DENSE_ATTENTION_SOFTMAX_TOLERANCE, CUDA_DENSE_ATTENTION_V_MIX_KERNEL_ID,
    CUDA_DENSE_ATTENTION_V_MIX_TOLERANCE, CudaAttentionConfig, CudaDenseAttentionScoreStats,
    CudaDenseAttentionSoftmaxStats, CudaDenseAttentionVMixStats,
    ...
    DenseGgufAttentionVMixCudaFixture, DenseGgufAttentionVMixCudaParity, attention_cpu_fallback,
    attention_forward, attention_forward_cpu, batch_attention_cpu, chunked_attention_cpu,
    launch_attention, launch_dense_attention_scores_f32_cuda,
    launch_dense_attention_softmax_f32_cuda, launch_dense_attention_v_mix_f32_cuda,
    masked_attention_cpu_fallback, multi_head_attention_cpu_fallback,
    run_dense_gguf_attention_score_cuda_parity, run_dense_gguf_attention_softmax_cuda_parity,
    run_dense_gguf_attention_v_mix_cuda_parity,
};
...
pub use error_correction::{
    BufferId, ECCConfig, ECCMode, ECCStats, ErrorCorrectionManager, compare_redundant_outputs,
    compute_checksum, compute_parity, correct_single_bit, crc32_compute, detect_errors,
    ecc_scrub_buffer, hamming_decode, hamming_encode, verify_checksum,
};

#[cfg(any(feature = "gpu", feature = "cuda"))]
pub use error_correction::{
    ECC_KERNEL_SRC, ECCLaunchConfig, launch_compute_checksum, launch_compute_parity,
    launch_detect_errors,
};
