...
impl VocabTrie {
    ...
    fn collect_completions(node: &TrieNode, current: &mut String, out: &mut Vec<(String, u32)>) {
...
impl CompactTrie {
    ...
    pub fn from_vocab_trie(trie: &VocabTrie) -> Self {
        ...
        for &node in &bfs_nodes {
            ...
            for (&ch, child) in &node.children {
                ...
                children_list
                    .push(CompactChildEntry { codepoint: ch as u32, node_index: child_idx });
            }
    ...
    pub fn from_gpu_buffer(buf: &[u32]) -> Option<Self> {
        ...
        for _ in 0..num_children {
            children
                .push(CompactChildEntry { codepoint: buf[offset], node_index: buf[offset + 1] });
...
pub fn batch_prefix_search(trie: &VocabTrie, queries: &[&str]) -> Vec<TrieSearchResult> {
...
pub fn batch_compact_search(compact: &CompactTrie, queries: &[&str]) -> Vec<(Option<u32>, u32)> {
    queries
        .iter()
        .map(|q| {
...
mod tests {
    ...
    fn test_compact_search_equivalence() {
        ...
        for (tok, id) in sample_vocab() {
            assert_eq!(compact.search(tok), Some(id), "compact search mismatch for '{tok}'");
        }
    }
    ...
    fn test_cache_hit() {
        let mut cache = PrefixCache::new(4);
        let result =
            TrieSearchResult { exact_match: Some(42), prefix_match_count: 1, completions: vec![] };
    ...
    fn test_cache_overwrite_existing() {
        let mut cache = PrefixCache::new(4);
        let r1 =
            TrieSearchResult { exact_match: Some(1), prefix_match_count: 1, completions: vec![] };
        let r2 =
            TrieSearchResult { exact_match: Some(2), prefix_match_count: 1, completions: vec![] };
    ...
    fn test_search_result_not_empty_with_prefix() {
        let r = TrieSearchResult { exact_match: None, prefix_match_count: 5, completions: vec![] };
        assert!(!r.is_empty());
    }
