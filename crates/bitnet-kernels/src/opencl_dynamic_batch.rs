...
/// it is padding.
pub fn build_padding_mask(token_lengths: &[usize], padded_length: usize) -> Vec<Vec<bool>> {
    token_lengths
        .iter()
        .map(|&len| {
...
impl BatchStats {
    ...
    pub fn record_batch(&mut self, batch: &DynamicBatch, now: Instant) {
        ...
        let lengths: Vec<usize> = batch.requests.iter().map(|r| r.token_ids.len()).collect();
        self.total_padding_waste += padding_waste_ratio(&lengths, batch.padded_seq_len);
...
impl BatchScheduler {
    ...
    pub fn try_form_batch(&mut self) -> Option<DynamicBatch> {
        if self.should_form_batch() { self.force_form_batch() } else { None }
    }
...
mod tests {
    ...
    fn former_padded_tokens_correct_length() {
        let cfg = BatchConfig { padding_strategy: PaddingStrategy::PadToMax, ..default_config() };
        let former = BatchFormer::new(cfg);
        let reqs = vec![
    ...
    fn former_pad_to_bucket() {
        let cfg =
            BatchConfig { padding_strategy: PaddingStrategy::PadToBucket, ..default_config() };
        let former = BatchFormer::new(cfg);
        let reqs = vec![
    ...
    fn large_batch_all_same_length() {
        ...
        let former = BatchFormer::new(cfg);
        let reqs: Vec<_> = (0..64).map(|i| make_request(i, 32, RequestPriority::Normal)).collect();
        let batch = former.form_batch(reqs).unwrap();
