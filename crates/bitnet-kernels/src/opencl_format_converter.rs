...
impl AlignmentRequirements {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if !self.cache_line.is_power_of_two() {
            return Err(format!(
                "cache_line ({}) must be a power of two",
                self.cache_line
            ));
        }
        Ok(())
...
impl fmt::Display for AlignmentRequirements {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "align={}B, CL={}B",
            self.buffer_align, self.cache_line
        )
    }
}
...
impl LayoutConverter {
    ...
    pub fn required_len(
        ...
        match layout {
            ...
            | TensorLayout::Interleaved => rows * cols,
            ...
            TensorLayout::BlockedRC(br, bc) => {
                ...
                for r in 0..padded_r {
                    for c in 0..padded_c {
                        ...
                        let block_idx = block_row * blocks_per_row + block_col;
                        let elem_idx =
                            block_idx * (br * bc) + in_block_r * bc + in_block_c;
...
impl TileFormat {
    ...
    pub fn tile_count(&self, rows: usize, cols: usize) -> (usize, usize) {
        (
            rows.div_ceil(self.tile_rows),
            cols.div_ceil(self.tile_cols),
        )
    }
    ...
    pub fn untile(&self, tiled: &[f32], rows: usize, cols: usize) -> Vec<f32> {
        ...
        assert!(
            tiled.len() >= expected,
            "tiled buffer too short: {} < {expected}",
            tiled.len()
        );
...
impl PackedFormat {
    ...
    pub fn pack(&self, data: &[&[u8]]) -> Vec<u8> {
        assert_eq!(
            data.len(),
            self.entries.len(),
            "data slice count must match entry count"
        );
    ...
    pub fn dequant_qk256(packed: &[u8], block_count: usize) -> AdaptedQuant {
        ...
        for b in 0..block_count {
            ...
            let scale = if block_start + SCALE_BYTES <= packed.len() {
                let bytes: [u8; 4] = packed[block_start..block_start + SCALE_BYTES]
                    .try_into()
                    .unwrap();
...
impl ConversionPlan {
    ...
    pub fn pad(mut self, target_rows: usize, target_cols: usize) -> Self {
        self.steps
            .push(ConversionStep::Pad(target_rows, target_cols));
        self
    }
        ...
        for step in &self.steps {
            match step {
                ConversionStep::Dequantize(format) => {
                    ...
                    source_bytes = packed.len();
                    let adapted =
                        QuantFormatAdapter::adapt(packed, *format, rows * cols);
                    current = adapted.data;
                    current_rows = rows;
                    current_cols = cols;
                }
                ConversionStep::Reorder(from, to) => {
                    current = LayoutConverter::convert(
                        &current,
                        current_rows,
                        current_cols,
                        *from,
                    ...
                    let new_len =
                        LayoutConverter::required_len(current_rows, current_cols, *to);
                    let _ = new_len;
                }
...
mod tests {
    ...
    fn test_row_to_col_major_2x3() {
        ...
        let col = LayoutConverter::convert(
            &src, 2, 3, TensorLayout::RowMajor, TensorLayout::ColumnMajor,
        );
    ...
    fn test_col_to_row_major_2x3() {
        ...
        let row = LayoutConverter::convert(
            &col, 2, 3, TensorLayout::ColumnMajor, TensorLayout::RowMajor,
        );
    ...
    fn test_row_col_roundtrip() {
        ...
        let col = LayoutConverter::convert(
            &src, 4, 5, TensorLayout::RowMajor, TensorLayout::ColumnMajor,
        );
        let back = LayoutConverter::convert(
            &col, 4, 5, TensorLayout::ColumnMajor, TensorLayout::RowMajor,
        );
        assert_eq!(src, back);
    }
    ...
    fn test_row_col_roundtrip_1x1() {
        let src = vec![42.0];
        let col = LayoutConverter::convert(
            &src, 1, 1, TensorLayout::RowMajor, TensorLayout::ColumnMajor,
        );
        assert_eq!(col, vec![42.0]);
        let back = LayoutConverter::convert(
            &col, 1, 1, TensorLayout::ColumnMajor, TensorLayout::RowMajor,
        );
    ...
    fn test_same_layout_noop() {
        ...
        let out = LayoutConverter::convert(
            &src, 3, 4, TensorLayout::RowMajor, TensorLayout::RowMajor,
        );
        assert_eq!(src, out);
    }
    ...
    fn test_row_col_single_row() {
        ...
        let col = LayoutConverter::convert(
            &src, 1, 4, TensorLayout::RowMajor, TensorLayout::ColumnMajor,
        );
    ...
    fn test_row_col_single_col() {
        ...
        let col = LayoutConverter::convert(
            &src, 3, 1, TensorLayout::RowMajor, TensorLayout::ColumnMajor,
        );
    ...
    fn test_blocked_exact_fit() {
        ...
        let blocked = LayoutConverter::convert(
            &src, 4, 4, TensorLayout::RowMajor, TensorLayout::BlockedRC(2, 2),
        );
    ...
    fn test_blocked_roundtrip_exact() {
        ...
        let back = LayoutConverter::convert(
            &blocked, 4, 4, TensorLayout::BlockedRC(2, 2), TensorLayout::RowMajor,
        );
        assert_eq!(src, back);
    }
    ...
    fn test_blocked_non_divisible() {
        ...
        let blocked = LayoutConverter::convert(
            &src, 3, 3, TensorLayout::RowMajor, TensorLayout::BlockedRC(2, 2),
        );
        // Padded to 4×4 with zeros, then blocked
        assert_eq!(blocked.len(), 16); // 4×4
        // Roundtrip
        let back = LayoutConverter::convert(
            &blocked, 3, 3, TensorLayout::BlockedRC(2, 2), TensorLayout::RowMajor,
        );
        assert_eq!(back, src);
    }
    ...
    fn test_blocked_1x1_block() {
        ...
        let blocked = LayoutConverter::convert(
            &src, 2, 3, TensorLayout::RowMajor, TensorLayout::BlockedRC(1, 1),
        );
    ...
    fn test_blocked_large_roundtrip() {
        ...
        let blocked = LayoutConverter::convert(
            &src, 10, 10, TensorLayout::RowMajor, TensorLayout::BlockedRC(4, 4),
        );
        let back = LayoutConverter::convert(
            &blocked, 10, 10, TensorLayout::BlockedRC(4, 4), TensorLayout::RowMajor,
        );
        assert_eq!(src, back);
    }
    ...
    fn test_interleaved_2x3() {
        ...
        let inter = LayoutConverter::convert(
            &src, 2, 3, TensorLayout::RowMajor, TensorLayout::Interleaved,
        );
    ...
    fn test_interleaved_roundtrip() {
        ...
        let inter = LayoutConverter::convert(
            &src, 4, 6, TensorLayout::RowMajor, TensorLayout::Interleaved,
        );
        let back = LayoutConverter::convert(
            &inter, 4, 6, TensorLayout::Interleaved, TensorLayout::RowMajor,
        );
        assert_eq!(src, back);
    }
    ...
    fn test_col_to_blocked_roundtrip() {
        let src: Vec<f32> = (0..12).map(|i| i as f32).collect();
        let col = LayoutConverter::convert(
            &src, 3, 4, TensorLayout::RowMajor, TensorLayout::ColumnMajor,
        );
        let blocked = LayoutConverter::convert(
            &col, 3, 4, TensorLayout::ColumnMajor, TensorLayout::BlockedRC(2, 2),
        );
        let back = LayoutConverter::convert(
            &blocked, 3, 4, TensorLayout::BlockedRC(2, 2), TensorLayout::RowMajor,
        );
        assert_eq!(src, back);
    }
    ...
    fn test_interleaved_to_col_roundtrip() {
        let src: Vec<f32> = (0..20).map(|i| i as f32).collect();
        let inter = LayoutConverter::convert(
            &src, 4, 5, TensorLayout::RowMajor, TensorLayout::Interleaved,
        );
        let col = LayoutConverter::convert(
            &inter, 4, 5, TensorLayout::Interleaved, TensorLayout::ColumnMajor,
        );
        let back = LayoutConverter::convert(
            &col, 4, 5, TensorLayout::ColumnMajor, TensorLayout::RowMajor,
        );
    ...
    fn test_packed_plan_multiple() {
        let pf = PackedFormat::plan(
            &[("a", 50), ("b", 70), ("c", 10)],
            AlignmentRequirements::A770,
        );
    ...
    fn test_plan_execute_reorder_only() {
        let src: Vec<f32> = (0..6).map(|i| i as f32).collect();
        let plan = ConversionPlan::new(2, 3)
            .reorder(TensorLayout::RowMajor, TensorLayout::ColumnMajor);
    ...
    fn test_plan_execute_tile_only() {
        ...
        let plan = ConversionPlan::new(2, 2)
    ...
    fn test_all_layout_roundtrips() {
        ...
        for &layout in &layouts {
            let converted = LayoutConverter::convert(
                &src, rows, cols, TensorLayout::RowMajor, layout,
            );
            let back = LayoutConverter::convert(
                &converted, rows, cols, layout, TensorLayout::RowMajor,
            );
    ...
    fn test_tile_roundtrip_various_sizes() {
        let tiles = [
            TileFormat::TILE_8X8,
            TileFormat::TILE_16X16,
            TileFormat::TILE_32X32,
        ];
        ...
        for tf in &tiles {
            for &(rows, cols) in &shapes {
                ...
                assert_eq!(
                    src, back,
                    "tile roundtrip failed for {tf} on {rows}×{cols}"
                );
            }
        }
    }
    ...
    fn test_blocked_roundtrip_various_blocks() {
        ...
        for &(br, bc) in &block_sizes {
            ...
            let back = LayoutConverter::convert(
                &blocked, rows, cols, layout, TensorLayout::RowMajor,
            );
    ...
    fn test_layout_conversion_preserves_element_count() {
        ...
        for &layout in &layouts {
            let out = LayoutConverter::convert(
                &src, 5, 6, TensorLayout::RowMajor, layout,
            );
