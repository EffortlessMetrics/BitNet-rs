...
impl Default for GpuTracerConfig {
    fn default() -> Self {
        Self { max_traces: 0, capture_timing: true, device_name: "gpu:0".to_string() }
    }
}
...
impl GpuTracer {
    ...
    pub fn new(config: GpuTracerConfig) -> Self {
        Self {
            inner: Arc::new(Mutex::new(GpuTracerInner { traces: Vec::new(), enabled: true })),
            config,
        }
    }
    ...
    pub fn to_trace_records(&self) -> Vec<TraceRecord> {
        self.inner.lock().unwrap().traces.iter().map(|t| t.trace_record.clone()).collect()
    }
    ...
    pub fn total_execution_time(&self) -> Duration {
        self.inner.lock().unwrap().traces.iter().map(|t| t.execution_time).sum()
    }
}
...
mod tests {
    ...
    fn test_gpu_tracer_max_traces() {
        let config = GpuTracerConfig { max_traces: 2, ..Default::default() };
    ...
    fn test_gpu_tracer_timing_disabled() {
        let config = GpuTracerConfig { capture_timing: false, ..Default::default() };
        let tracer = GpuTracer::new(config);
