...
impl ModelCache {
    ...
    pub fn open(config: CacheConfig) -> Result<Self, CacheError> {
        ...
        let total_bytes = index.entries.values().map(|e| e.size).sum::<u64>();
        Ok(Self {
            config,
            index: RwLock::new(index),
            stats: RwLock::new(CacheStats { total_bytes, ..Default::default() }),
        })
    }
    ...
    pub fn put(&self, key: &str, data: &[u8]) -> Result<(), CacheError> {
        ...
        if size > self.config.max_size_bytes {
            return Err(CacheError::EntryTooLarge { size, max: self.config.max_size_bytes });
        }
    ...
    pub fn get(&self, key: &str) -> Result<Vec<u8>, CacheError> {
        let (file_path, expected_hash) = {
            let mut idx = self.index.write().unwrap();
            let entry =
                idx.entries.get_mut(key).ok_or_else(|| CacheError::EntryNotFound(key.into()))?;
        ...
        if self.config.integrity_check {
            ...
            if actual != expected_hash {
                ...
                return Err(CacheError::IntegrityMismatch { expected: expected_hash, actual });
            }
        }
    ...
    pub fn mmap(&self, key: &str) -> Result<Vec<u8>, CacheError> {
        let path = {
            let mut idx = self.index.write().unwrap();
            let entry =
                idx.entries.get_mut(key).ok_or_else(|| CacheError::EntryNotFound(key.into()))?;
    ...
    pub fn warm(&self, preload: &[(&str, Option<&[u8]>)]) -> Result<(), CacheError> {
    ...
    pub fn entry_metadata(&self, key: &str) -> Option<CacheEntry> {
    ...
    fn persist_index(dir: &Path, index: &CacheIndex) -> Result<(), CacheError> {
        let path = Self::index_path(dir);
    ...
    fn evict_until_fits(&self, needed: u64) -> Result<(), CacheError> {
        loop {
            let current: u64 = {
                let idx = self.index.read().unwrap();
    ...
    fn pick_victim(&self) -> Result<String, CacheError> {
        ...
        if idx.entries.is_empty() {
            return Err(CacheError::EntryNotFound("no entries to evict".into()));
        }
        let key = match self.config.eviction_policy {
            EvictionPolicy::LRU => {
                idx.entries.iter().min_by_key(|(_, e)| e.last_accessed).map(|(k, _)| k.clone())
            }
            EvictionPolicy::LFU => {
                idx.entries.iter().min_by_key(|(_, e)| e.access_count).map(|(k, _)| k.clone())
            }
            EvictionPolicy::FIFO => {
                idx.entries.iter().min_by_key(|(_, e)| e.created_at).map(|(k, _)| k.clone())
            }
            EvictionPolicy::SizeBased => {
                idx.entries.iter().max_by_key(|(_, e)| e.size).map(|(k, _)| k.clone())
            }
            EvictionPolicy::TTL => {
                let now = now_unix();
                ...
                idx.entries
                    ...
                    .or_else(|| {
                        ...
                        idx.entries.iter().min_by_key(|(_, e)| e.created_at)
                    })
                    .map(|(k, _)| k.clone())
            }
        };
        key.ok_or_else(|| CacheError::EntryNotFound("no victim found".into()))
    }
}
...
mod tests {
    ...
    fn warm_inserts_new_entries() {
        let tmp = TempDir::new().unwrap();
        let cache = open_cache(tmp.path());
        cache.warm(&[("a", Some(b"data_a")), ("b", Some(b"data_b"))]).unwrap();
        assert!(cache.contains("a"));
        assert!(cache.contains("b"));
    }
    ...
    fn warm_touches_existing_entries() {
        ...
        let before = cache.entry_metadata("a").unwrap().last_accessed;
        // Small sleep to ensure timestamp advances.
        std::thread::sleep(std::time::Duration::from_millis(50));
        cache.warm(&[("a", None)]).unwrap();
        let after = cache.entry_metadata("a").unwrap().last_accessed;
        assert!(after >= before);
    }
    ...
    fn stats_hit_rate_calculation() {
        let s = CacheStats { hits: 3, misses: 1, evictions: 0, total_bytes: 0 };
    ...
    fn index_file_is_valid_json() {
        ...
        let cache = open_cache(tmp.path());
        cache.put("m1", b"data").unwrap();
        let json = fs::read_to_string(tmp.path().join("cache_index.json")).unwrap();
    ...
    fn access_count_increments_on_get() {
        ...
        let cache = open_cache(tmp.path());
        cache.put("m1", b"data").unwrap();
        assert_eq!(cache.entry_metadata("m1").unwrap().access_count, 0);
        let _ = cache.get("m1").unwrap();
        assert_eq!(cache.entry_metadata("m1").unwrap().access_count, 1);
        let _ = cache.get("m1").unwrap();
        assert_eq!(cache.entry_metadata("m1").unwrap().access_count, 2);
    }
    ...
    fn last_accessed_updates_on_get() {
        ...
        let cache = open_cache(tmp.path());
        cache.put("m1", b"data").unwrap();
        let before = cache.entry_metadata("m1").unwrap().last_accessed;
        std::thread::sleep(std::time::Duration::from_millis(50));
        let _ = cache.get("m1").unwrap();
        let after = cache.entry_metadata("m1").unwrap().last_accessed;
        assert!(after >= before);
    }
    ...
    fn concurrent_reads_do_not_panic() {
        let tmp = TempDir::new().unwrap();
        let cache = std::sync::Arc::new(open_cache(tmp.path()));
    ...
    fn error_display_formats() {
        ...
        let e = CacheError::IntegrityMismatch { expected: "aaa".into(), actual: "bbb".into() };
