...
impl ApiVersion {
    ...
    pub fn parse(s: &str) -> Option<Self> {
        ...
        match parts.len() {
            ...
            1 => Some(Self { major: parts[0].parse().ok()?, minor: 0, patch: 0 }),
            _ => None,
        }
    }
...
impl VersionConstraint {
    ...
    pub fn matches(&self, v: &ApiVersion) -> bool {
        match self {
            ...
            Self::Caret(base) => v.major == base.major && v >= base,
...
impl fmt::Display for DeprecationWarning {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            self.endpoint, self.deprecated_since, self.sunset_date, self.migration_guide,
        )
    }
}
...
impl CompatibilityChecker {
    ...
    pub fn check(server: &ApiVersion, client: &ApiVersion) -> Compatibility {
        ...
        if server < client {
            return Compatibility::Incompatible {
                reason: format!("server version {server} is older than client {client}"),
            };
        }
        if server == client { Compatibility::Full } else { Compatibility::Backward }
    }
}
...
impl VersionNegotiator {
    ...
    pub fn best_compatible(
        ...
        server_versions.iter().filter(|sv| sv.is_compatible_with(client)).max().copied()
    }
}
...
impl MigrationGuide {
    ...
    pub fn add_step(&mut self, from: ApiVersion, to: ApiVersion, instructions: impl Into<String>) {
        self.steps.push(MigrationStep { from, to, instructions: instructions.into() });
    }

    /// Get the migration path from `from` to `to`.
    ///
    /// Returns an ordered list of steps, or `None` if no path exists.
    pub fn path(&self, from: &ApiVersion, to: &ApiVersion) -> Option<Vec<&MigrationStep>> {
        if from >= to {
            return None;
        }
...
impl ApiVersionRouter {
    ...
    pub fn add_route(&mut self, constraint: VersionConstraint, handler: impl Into<String>) {
    ...
    pub fn resolve(&self, version: &ApiVersion) -> Option<&str> {
        self.routes.iter().find(|(c, _)| c.matches(version)).map(|(_, h)| h.as_str())
    }

    /// Return all handlers matching a version.
    pub fn resolve_all(&self, version: &ApiVersion) -> Vec<&str> {
        self.routes.iter().filter(|(c, _)| c.matches(version)).map(|(_, h)| h.as_str()).collect()
    }
...
impl ApiCatalog {
    ...
    pub fn list_paths(&self) -> Vec<&str> {
        let mut paths: Vec<&str> = self.endpoints.keys().map(String::as_str).collect();
        paths.sort_unstable();
        paths
    }
    ...
    pub fn endpoints_for_version(&self, version: &ApiVersion) -> Vec<&VersionedEndpoint> {
        self.endpoints.values().filter(|ep| ep.supports_version(version)).collect()
    }
...
mod tests {
    ...
    fn constraint_display() {
        assert_eq!(VersionConstraint::Gte(ApiVersion::new(1, 0, 0)).to_string(), ">=1.0.0");
        assert_eq!(VersionConstraint::Lt(ApiVersion::new(2, 0, 0)).to_string(), "<2.0.0");
    }
    ...
    fn constraint_tilde_display() {
        assert_eq!(VersionConstraint::Tilde(ApiVersion::new(1, 2, 0)).to_string(), "~1.2.0");
    }

    #[test]
    fn constraint_caret_display() {
        assert_eq!(VersionConstraint::Caret(ApiVersion::new(1, 0, 0)).to_string(), "^1.0.0");
    }
    ...
    fn router_single_route() {
        ...
        router.add_route(VersionConstraint::Caret(ApiVersion::new(1, 0, 0)), "v1_handler");
        assert_eq!(router.resolve(&ApiVersion::new(1, 5, 0)), Some("v1_handler"));
    }
    ...
    fn router_multiple_routes() {
        let mut router = ApiVersionRouter::new();
        router.add_route(VersionConstraint::Caret(ApiVersion::new(1, 0, 0)), "v1_handler");
        router.add_route(VersionConstraint::Caret(ApiVersion::new(2, 0, 0)), "v2_handler");
        assert_eq!(router.resolve(&ApiVersion::new(1, 3, 0)), Some("v1_handler"));
        assert_eq!(router.resolve(&ApiVersion::new(2, 1, 0)), Some("v2_handler"));
    }
    ...
    fn router_resolve_all() {
        let mut router = ApiVersionRouter::new();
        router.add_route(VersionConstraint::Gte(ApiVersion::new(1, 0, 0)), "catch_all");
        router.add_route(VersionConstraint::Caret(ApiVersion::new(1, 0, 0)), "v1_specific");
    ...
    fn router_route_count() {
        ...
        router.add_route(VersionConstraint::Exact(ApiVersion::new(1, 0, 0)), "h1");
    ...
    fn router_first_match_wins() {
        let mut router = ApiVersionRouter::new();
        router.add_route(VersionConstraint::Gte(ApiVersion::new(1, 0, 0)), "first");
        router.add_route(VersionConstraint::Gte(ApiVersion::new(1, 0, 0)), "second");
        assert_eq!(router.resolve(&ApiVersion::new(1, 0, 0)), Some("first"));
    }
    ...
    fn tracker_warnings_for_endpoint() {
        let mut tracker = DeprecationTracker::new();
        tracker.deprecate("/v1/infer", ApiVersion::new(1, 5, 0), "2025-12-31", "Use /v2/infer");
        tracker.deprecate("/v1/embed", ApiVersion::new(1, 6, 0), "2026-03-31", "Use /v2/embed");
    ...
    fn tracker_all_warnings() {
        let mut tracker = DeprecationTracker::new();
        tracker.deprecate("/v1/a", ApiVersion::new(1, 0, 0), "2025-06-30", "migrate");
        tracker.deprecate("/v1/b", ApiVersion::new(1, 1, 0), "2025-09-30", "migrate");
    ...
    fn tracker_not_deprecated_unregistered() {
        let mut tracker = DeprecationTracker::new();
        tracker.deprecate("/v1/infer", ApiVersion::new(1, 5, 0), "2025-12-31", "migrate");
    ...
    fn compat_full() {
        let v = ApiVersion::new(1, 2, 3);
        assert_eq!(CompatibilityChecker::check(&v, &v), Compatibility::Full,);
    }
    ...
    fn compat_backward() {
        let server = ApiVersion::new(1, 3, 0);
        let client = ApiVersion::new(1, 2, 0);
        assert_eq!(CompatibilityChecker::check(&server, &client), Compatibility::Backward,);
    }
    ...
    fn negotiate_exact_overlap() {
        ...
        assert_eq!(VersionNegotiator::negotiate(&server, &client), Some(ApiVersion::new(2, 0, 0)),);
    }
    ...
    fn best_compatible_none() {
        let server = [ApiVersion::new(2, 0, 0)];
        let client = ApiVersion::new(1, 0, 0);
        assert!(VersionNegotiator::best_compatible(&server, &client).is_none());
    }
    ...
    fn migration_single_step() {
        ...
        let path = guide.path(&ApiVersion::new(1, 0, 0), &ApiVersion::new(1, 1, 0)).unwrap();
    ...
    fn migration_multi_step() {
        let mut guide = MigrationGuide::new();
        guide.add_step(ApiVersion::new(1, 0, 0), ApiVersion::new(1, 1, 0), "Step 1");
        guide.add_step(ApiVersion::new(1, 1, 0), ApiVersion::new(2, 0, 0), "Step 2");
        let path = guide.path(&ApiVersion::new(1, 0, 0), &ApiVersion::new(2, 0, 0)).unwrap();
    ...
    fn migration_no_path() {
        let guide = MigrationGuide::new();
        assert!(guide.path(&ApiVersion::new(1, 0, 0), &ApiVersion::new(2, 0, 0)).is_none());
    }

    #[test]
    fn migration_reverse_returns_none() {
        let mut guide = MigrationGuide::new();
        guide.add_step(ApiVersion::new(1, 0, 0), ApiVersion::new(1, 1, 0), "upgrade");
        assert!(guide.path(&ApiVersion::new(1, 1, 0), &ApiVersion::new(1, 0, 0)).is_none());
    }
    ...
    fn negotiate_picks_highest_common() {
        let server = [ApiVersion::new(1, 0, 0), ApiVersion::new(1, 1, 0), ApiVersion::new(1, 2, 0)];
        let client = [ApiVersion::new(1, 0, 0), ApiVersion::new(1, 2, 0)];
        assert_eq!(VersionNegotiator::negotiate(&server, &client), Some(ApiVersion::new(1, 2, 0)),);
    }
    ...
    fn router_with_and_constraint() {
        ...
        assert_eq!(router.resolve(&ApiVersion::new(1, 5, 0)), Some("v1_range"),);
        assert!(router.resolve(&ApiVersion::new(2, 0, 0)).is_none());
    }
    ...
    fn migration_step_count() {
        let mut guide = MigrationGuide::new();
        guide.add_step(ApiVersion::new(1, 0, 0), ApiVersion::new(1, 1, 0), "s1");
        guide.add_step(ApiVersion::new(1, 1, 0), ApiVersion::new(1, 2, 0), "s2");
