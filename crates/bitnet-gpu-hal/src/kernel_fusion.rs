...
pub enum FusionError {
    ...
    ShapeMismatch { src: usize, dst: usize, src_shape: Vec<usize>, dst_shape: Vec<usize> },
...
impl std::fmt::Display for FusionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::ShapeMismatch { src, dst, src_shape, dst_shape } => {
                write!(f, "shape mismatch: op {src} ({src_shape:?}) → op {dst} ({dst_shape:?})")
            }
            Self::MemoryBudgetExceeded { required, limit } => {
                write!(f, "memory budget exceeded: need {required} bytes, limit {limit} bytes")
            }
            Self::InvalidOpIndex(idx) => write!(f, "invalid op index {idx}"),
            Self::AutoFuseDisabled => write!(f, "auto-fusion is disabled"),
            Self::Other(msg) => write!(f, "fusion failed: {msg}"),
        }
    }
}
...
impl fmt::Debug for FusionRule {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("FusionRule").field("name", &self.name).finish_non_exhaustive()
    }
}
...
impl PatternMatcher {
        ...
        for (i, a) in graph.ops.iter().enumerate() {
            for (j, b) in graph.ops.iter().enumerate().skip(i + 1) {
                ...
                if let Some(pat) = self.match_pair(a, b) {
                    candidates.push(FusionCandidate { src: i, dst: j, pattern: pat });
                }
            }
        }
        candidates
    ...
    fn match_pair(&self, a: &OpNode, b: &OpNode) -> Option<FusionPattern> {
        ...
        if a.kind == OpKind::MatMul
            && b.kind == OpKind::BiasAdd
            && self.is_enabled(FusionPattern::MatMulBias)
        {
...
impl FusedKernel {
    ...
    pub fn compile(
        ...
        let id =
            format!("fused_{}_{}_{}_{}", candidate.pattern, candidate.src, candidate.dst, scratch);
...
impl FusionOptimizer {
    ...
    pub const fn new(config: FusionConfig) -> Self {
        Self { matcher: PatternMatcher::new(), config, min_speedup: 1.05 }
    }
    ...
    pub fn optimize(&self, graph: &ComputeGraph) -> FusionResult<Vec<FusedKernel>> {
        ...
        for c in &candidates {
            ...
            let rules_ok = self.config.fusion_rules.iter().all(|r| r.allows(src, dst));
            if !rules_ok {
                continue;
            }
...
mod tests {
    ...
    fn config_with_rules() {
        ...
        let cfg = FusionConfig { fusion_rules: vec![rule], ..FusionConfig::default() };
    ...
    fn error_memory_budget_display() {
        let e = FusionError::MemoryBudgetExceeded { required: 1024, limit: 512 };
        let msg = e.to_string();
    ...
    fn fusion_report_totals_correct() {
        let engine = KernelFusionEngine::new(FusionConfig::default(), 128);
        let report = engine.run(&mixed_graph()).unwrap();
        assert_eq!(report.kernels.len(), report.cache_hits + report.cache_misses,);
    }
