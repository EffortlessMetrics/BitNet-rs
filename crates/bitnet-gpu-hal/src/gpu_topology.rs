...
impl NumaAffinity {
    ...
    pub fn active_nodes(&self) -> Vec<u32> {
        let mut nodes: Vec<u32> =
            self.device_to_numa.values().copied().collect::<HashSet<_>>().into_iter().collect();
        nodes.sort_unstable();
        nodes
    }
...
impl BandwidthMatrix {
    ...
    pub fn set_symmetric(&mut self, a: &str, b: &str, gbps: f64) -> bool {
    ...
    pub fn min_bandwidth(&self) -> f64 {
        self.data.iter().copied().filter(|&v| v > 0.0).fold(f64::INFINITY, f64::min)
    }
...
impl PeerAccessTable {
    ...
    pub fn peers_of(&self, src: &str) -> Vec<&str> {
        self.pairs.iter().filter(|(s, _)| s == src).map(|(_, d)| d.as_str()).collect()
    }
...
impl GpuTopology {
    ...
    pub fn total_gpu_memory(&self) -> u64 {
        self.nodes.iter().filter(|n| n.kind == NodeKind::Gpu).map(|n| n.memory_bytes).sum()
    }

    /// Compute the [`BandwidthMatrix`] for GPU nodes only.
    #[must_use]
    pub fn bandwidth_matrix(&self) -> BandwidthMatrix {
        let gpu_ids: Vec<String> = self.gpus().iter().map(|g| g.id.clone()).collect();
        let mut mat = BandwidthMatrix::new(gpu_ids);
        for link in &self.links {
            // Only include links between GPUs.
            let src_gpu = self.nodes.iter().any(|n| n.id == link.source && n.kind == NodeKind::Gpu);
            let dst_gpu = self.nodes.iter().any(|n| n.id == link.target && n.kind == NodeKind::Gpu);
            if src_gpu && dst_gpu {
                mat.set(&link.source, &link.target, link.effective_bandwidth_gbps());
            }
        }
        mat
    }
...
impl TopologyDiscoverer for SysfsDiscoverer {
    fn discover(&self) -> Result<GpuTopology, String> {
        ...
        Ok(GpuTopology::new(Vec::new(), Vec::new(), NumaAffinity::new(), PeerAccessTable::new()))
    }
}
...
impl MockDiscoverer {
    ...
    pub fn two_gpu_nvlink() -> Self {
        let nodes = vec![
            ...
            TopologyNode::new("gpu1", NodeKind::Gpu, "GPU 1")
                .with_memory(80 * 1024 * 1024 * 1024)
                .with_numa(0),
            TopologyNode::new("cpu0", NodeKind::CpuSocket, "CPU 0").with_numa(0),
        ];
        let links = vec![
    ...
    pub fn four_gpu_pcie() -> Self {
        let nodes = vec![
            ...
            TopologyNode::new("cpu0", NodeKind::CpuSocket, "CPU 0").with_numa(0),
            TopologyNode::new("cpu1", NodeKind::CpuSocket, "CPU 1").with_numa(1),
        ];
        ...
        let mut numa = NumaAffinity::new();
        for (id, node) in [("gpu0", 0), ("gpu1", 0), ("gpu2", 1), ("gpu3", 1)] {
            numa.insert(id, node);
        }
...
impl<'a> PlacementOptimizer<'a> {
    ...
    pub fn place_by_memory(
        ...
        self.topology
            ...
            .filter_map(|g| {
                ...
                if free >= tensor_bytes { Some((&g.id, free)) } else { None }
            })
    ...
    #[must_use]
    pub fn place_near(&self, tensor_name: &str, anchor_device: &str) -> Option<PlacementDecision> {
        let bw = self.topology.bandwidth_matrix();
        let gpu_ids = bw.device_ids().to_vec();
        gpu_ids
            .iter()
            .filter(|id| id.as_str() != anchor_device)
            .filter_map(|id| bw.get(anchor_device, id).map(|b| (id.clone(), b)))
            ...
            .find_map(|g| {
                ...
                if free >= tensor_bytes {
                    Some(PlacementDecision {
                        ...
                        reason: format!("NUMA-local (node {anchor_numa}), {free} B free"),
                    })
                } else {
                    None
                }
...
impl<'a> TopologyVisualizer<'a> {
    ...
    pub fn render(&self) -> String {
        ...
        // Group GPUs by NUMA
        let mut numa_groups: BTreeMap<u32, Vec<&TopologyNode>> = BTreeMap::new();
...
mod tests {
    ...
    fn nvlink_latency_lower_than_pcie() {
        assert!(LinkType::NVLink4.typical_latency_ns() < LinkType::PCIeGen4.typical_latency_ns());
    }
    ...
    fn device_link_defaults() {
        ...
        assert!(
            (l.bandwidth_gbps - LinkType::PCIeGen4.theoretical_bandwidth_gbps()).abs()
                < f64::EPSILON
        );
    }
    ...
    fn device_link_with_bandwidth() {
        let l = DeviceLink::new("a", "b", LinkType::NVLink4).with_bandwidth(90.0);
        assert!((l.bandwidth_gbps - 90.0).abs() < f64::EPSILON);
    }

    #[test]
    fn device_link_with_latency() {
        let l = DeviceLink::new("a", "b", LinkType::NVLink4).with_latency(200);
        assert_eq!(l.latency_ns, 200);
    }

    #[test]
    fn device_link_with_lanes() {
        let l = DeviceLink::new("a", "b", LinkType::NVLink4).with_lanes(4);
    ...
    fn topology_node_with_memory() {
        let n =
            TopologyNode::new("gpu0", NodeKind::Gpu, "A100").with_memory(80 * 1024 * 1024 * 1024);
        assert_eq!(n.memory_bytes, 80 * 1024 * 1024 * 1024);
    }

    #[test]
    fn topology_node_with_numa() {
        let n = TopologyNode::new("gpu0", NodeKind::Gpu, "A100").with_numa(1);
    ...
    fn topology_empty() {
        let t =
            GpuTopology::new(Vec::new(), Vec::new(), NumaAffinity::new(), PeerAccessTable::new());
    ...
    fn topology_total_memory() {
        let mock = MockDiscoverer::two_gpu_nvlink();
        let topo = mock.discover().unwrap();
        assert_eq!(topo.total_gpu_memory(), 2 * 80 * 1024 * 1024 * 1024);
    }
    ...
    fn placement_numa_local() {
        ...
        let p = opt.place_numa_local("attn", "gpu0", 1024, &usage).unwrap();
        assert_eq!(p.device_id, "gpu1");
    }
    ...
    fn placement_numa_local_cross_node() {
        ...
        let opt = PlacementOptimizer::new(&topo);
        let usage = HashMap::new();
        let p = opt.place_numa_local("attn", "gpu2", 1024, &usage).unwrap();
