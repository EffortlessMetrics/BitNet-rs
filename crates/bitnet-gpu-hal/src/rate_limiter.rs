...
impl RateLimitResult {
    ...
    pub const fn allow(remaining: f64, limit: f64) -> Self {
        Self { allowed: true, retry_after_ms: 0, remaining_tokens: remaining, limit }
    }

    /// Convenience constructor for a denied result.
    pub const fn deny(retry_after_ms: u64, remaining: f64, limit: f64) -> Self {
        Self { allowed: false, retry_after_ms, remaining_tokens: remaining, limit }
    }
}
...
impl std::fmt::Display for RateLimitError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::QuotaExhausted { daily_remaining, monthly_remaining } => {
                write!(
                    f,
...
impl TokenBucketLimiter {
    ...
    pub const fn with_instant(capacity: f64, rate: f64, now: Instant) -> Self {
        Self { capacity, rate, tokens: capacity, last_refill: now }
    }
    ...
    pub fn try_acquire(&mut self, now: Instant) -> RateLimitResult {
        ...
        if self.tokens >= 1.0 {
            ...
            let wait_secs = if self.rate > 0.0 { deficit / self.rate } else { f64::INFINITY };
...
impl SlidingWindowLimiter {
    ...
    pub const fn new(window: Duration, max_requests: u64) -> Self {
        Self { window, max_requests, timestamps: Vec::new() }
    }
    ...
    pub fn try_acquire(&mut self, now: Instant) -> RateLimitResult {
        ...
        if count < self.max_requests {
            ...
            let wait = expires_at.checked_duration_since(now).unwrap_or(Duration::ZERO);
...
impl FixedWindowLimiter {
    ...
    pub fn new(window: Duration, max_requests: u64) -> Self {
        Self { window, max_requests, window_start: Instant::now(), count: 0 }
    }

    /// Create with a specific starting instant (for deterministic testing).
    pub const fn with_instant(window: Duration, max_requests: u64, now: Instant) -> Self {
        Self { window, max_requests, window_start: now, count: 0 }
    }
    ...
    pub fn try_acquire(&mut self, now: Instant) -> RateLimitResult {
        ...
        if self.count < self.max_requests {
            ...
            let wait = window_end.checked_duration_since(now).unwrap_or(Duration::ZERO);
            #[allow(clippy::cast_possible_truncation)]
            let retry_ms = wait.as_millis() as u64 + 1;
            RateLimitResult::deny(retry_ms, 0.0, limit)
        }
...
impl LeakyBucketLimiter {
    ...
    pub fn new(capacity: f64, drain_rate: f64) -> Self {
        Self { capacity, drain_rate, level: 0.0, last_drain: Instant::now() }
    }

    /// Create with a specific starting instant (for deterministic testing).
    pub const fn with_instant(capacity: f64, drain_rate: f64, now: Instant) -> Self {
        Self { capacity, drain_rate, level: 0.0, last_drain: now }
    }
    ...
    pub fn try_acquire(&mut self, now: Instant) -> RateLimitResult {
        ...
        if self.level + 1.0 <= self.capacity {
            ...
            let wait_secs =
                if self.drain_rate > 0.0 { excess / self.drain_rate } else { f64::INFINITY };
            #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
            let retry_ms = (wait_secs * 1000.0).ceil() as u64;
...
impl UserQuota {
    ...
    pub const fn new(daily_limit: u64, monthly_limit: u64) -> Self {
        Self { daily_limit, monthly_limit, daily_used: 0, monthly_used: 0 }
    }
...
impl QuotaManager {
    ...
    pub fn new(default_daily: u64, default_monthly: u64) -> Self {
        Self { quotas: HashMap::new(), default_daily, default_monthly }
    }
    ...
    pub fn get_quota(&mut self, user_id: &str) -> &mut UserQuota {
        ...
        self.quotas.entry(user_id.to_string()).or_insert_with(|| UserQuota::new(daily, monthly))
    }
...
impl RateLimiter {
    ...
    pub fn consume_quota(&mut self, user_id: &str, token_count: u64) -> Result<(), RateLimitError> {
...
mod tests {
    ...
    fn quota_exhaustion_error_contains_remaining() {
        ...
        match err {
            RateLimitError::QuotaExhausted { daily_remaining, monthly_remaining } => {
    ...
    fn error_display_quota_exhausted() {
        let e = RateLimitError::QuotaExhausted { daily_remaining: 0, monthly_remaining: 5 };
        let s = format!("{e}");
