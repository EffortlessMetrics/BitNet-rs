...
impl JitCache {
    ...
    pub fn get(&mut self, key: &SpecializationKey) -> Option<&CompiledKernel> {
        ...
        if self.entries.contains_key(&hash) {
            ...
            self.entries.get_mut(&hash).unwrap().last_access = self.access_counter;
    ...
    pub fn insert(
        ...
        self.access_counter += 1;
        let hash = key.hash_u64();
        self.entries.insert(hash, CacheEntry { kernel, last_access: self.access_counter });
        Ok(())
    }
    ...
    fn evict_lru(&mut self) {
        if let Some((&lru_key, _)) = self.entries.iter().min_by_key(|(_, e)| e.last_access) {
...
impl KernelSpecializer {
    ...
    pub fn specialize_node(node: &IrNode, target: BackendTarget) -> Result<String, JitError> {
        match &node.op {
            IrOp::Add => Ok(Self::gen_elementwise("add", "+", &node.output, target)),
            IrOp::Mul => Ok(Self::gen_elementwise("mul", "*", &node.output, target)),
            IrOp::MatMul => {
                if node.inputs.len() < 2 {
                    return Err(JitError::ShapeMismatch { expected: 2, actual: node.inputs.len() });
                }
                Ok(Self::gen_matmul(&node.inputs[0], &node.inputs[1], target))
            }
            IrOp::Reduce(op) => Ok(Self::gen_reduce(*op, &node.inputs[0], target)),
            IrOp::Activation(kind) => Ok(Self::gen_activation(*kind, &node.output, target)),
    ...
    fn gen_matmul(a: &TypedShape, b: &TypedShape, target: BackendTarget) -> String {
    ...
    fn gen_reduce(op: ReduceOp, input: &TypedShape, target: BackendTarget) -> String {
    ...
    fn gen_activation(kind: ActivationKind, shape: &TypedShape, target: BackendTarget) -> String {
        let n = shape.num_elements();
...
impl KernelJit {
    ...
    pub fn compile(
        ...
        let shapes: Vec<TypedShape> = ir.referenced_shapes().into_iter().cloned().collect();
        let key = SpecializationKey::new(&ir.name, shapes, target);
        ...
        for node in &ir.nodes {
            let frag = KernelSpecializer::specialize_node(node, target)?;
    ...
    fn optimize_source(src: &str) -> String {
        ...
        src.lines().filter(|l| !l.trim().is_empty()).collect::<Vec<_>>().join("\n") + "\n"
    }
...
mod tests {
    ...
    fn matmul_ir(m: usize, k: usize, n: usize) -> KernelIR {
        ...
        ir.push(IrNode { op: IrOp::MatMul, inputs: vec![a, b], output: c });
        ir
    }
    ...
    fn specialization_key_deterministic_hash() {
        let k1 = SpecializationKey::new("k", vec![f32_vec(vec![4])], BackendTarget::Cuda);
        let k2 = SpecializationKey::new("k", vec![f32_vec(vec![4])], BackendTarget::Cuda);
    ...
    fn specialization_key_differs_by_shape() {
        let k1 = SpecializationKey::new("k", vec![f32_vec(vec![4])], BackendTarget::Cuda);
        let k2 = SpecializationKey::new("k", vec![f32_vec(vec![8])], BackendTarget::Cuda);
    ...
    fn specialization_key_differs_by_backend() {
        let k1 = SpecializationKey::new("k", vec![f32_vec(vec![4])], BackendTarget::Cuda);
        let k2 = SpecializationKey::new("k", vec![f32_vec(vec![4])], BackendTarget::OpenCL);
        assert_ne!(k1.hash_u64(), k2.hash_u64());
    }

    #[test]
    fn specialization_key_differs_by_name() {
        let k1 = SpecializationKey::new("a", vec![f32_vec(vec![4])], BackendTarget::Cuda);
        let k2 = SpecializationKey::new("b", vec![f32_vec(vec![4])], BackendTarget::Cuda);
        assert_ne!(k1.hash_u64(), k2.hash_u64());
    }

    #[test]
    fn specialization_key_cache_filename() {
        let k = SpecializationKey::new("k", vec![f32_vec(vec![4])], BackendTarget::Cuda);
    ...
    fn cache_insert_and_get() {
        ...
        let key = SpecializationKey::new("k", vec![f32_vec(vec![8])], BackendTarget::Cuda);
    ...
    fn cache_miss_returns_none() {
        let mut c = JitCache::new(16);
        let key = SpecializationKey::new("missing", vec![], BackendTarget::Cuda);
    ...
    fn cache_lru_eviction() {
        ...
        let make_key = |name: &str| SpecializationKey::new(name, vec![], BackendTarget::Cuda);
    ...
    fn cache_clear() {
        let mut c = JitCache::new(16);
        let key = SpecializationKey::new("k", vec![], BackendTarget::Cuda);
        let kernel = CompiledKernel {
            source: String::new(),
            target: BackendTarget::Cuda,
            compilation_time: Duration::ZERO,
    ...
    fn gen_add_cuda() {
        ...
        let src = KernelSpecializer::specialize_node(&node, BackendTarget::Cuda).unwrap();
    ...
    fn gen_add_opencl() {
        ...
        let src = KernelSpecializer::specialize_node(&node, BackendTarget::OpenCL).unwrap();
    ...
    fn gen_add_vulkan() {
        ...
        let src = KernelSpecializer::specialize_node(&node, BackendTarget::Vulkan).unwrap();
    ...
    fn gen_matmul_insufficient_inputs() {
        ...
        let res = KernelSpecializer::specialize_node(&node, BackendTarget::Cuda);
        assert!(res.is_err());
        assert_eq!(res.unwrap_err(), JitError::ShapeMismatch { expected: 2, actual: 1 });
    }
    ...
    fn gen_custom_cuda() {
        let node =
            IrNode { op: IrOp::Custom("my_op".into()), inputs: vec![], output: f32_vec(vec![1]) };
        let src = KernelSpecializer::specialize_node(&node, BackendTarget::Cuda).unwrap();
    ...
    fn jit_specialization_disabled_no_cache() {
        let cfg = JitConfig { specialization_enabled: false, ..JitConfig::default() };
    ...
    fn jit_compile_reduce_sum() {
        ...
        ir.push(IrNode { op: IrOp::Reduce(ReduceOp::Sum), inputs: vec![input], output });
    ...
    fn end_to_end_all_backends() {
        let mut jit = KernelJit::with_defaults();
        let ir = simple_add_ir();
        for target in [BackendTarget::Cuda, BackendTarget::OpenCL, BackendTarget::Vulkan] {
    ...
    fn cache_eviction_under_small_limit() {
        let cfg = JitConfig { max_cache_entries: 3, ..JitConfig::default() };
        let mut jit = KernelJit::new(cfg);
    ...
    fn specialization_key_from_ir() {
        ...
        let shapes: Vec<TypedShape> = ir.referenced_shapes().into_iter().cloned().collect();
        let key = SpecializationKey::new(&ir.name, shapes, BackendTarget::Cuda);
    ...
    fn i8_element_type_codegen() {
        ...
        let node =
            IrNode { op: IrOp::Add, inputs: vec![shape.clone(), shape.clone()], output: shape };
        let src = KernelSpecializer::specialize_node(&node, BackendTarget::Cuda).unwrap();
    ...
    fn f16_element_type_codegen() {
        ...
        let node =
            IrNode { op: IrOp::Mul, inputs: vec![shape.clone(), shape.clone()], output: shape };
        let src = KernelSpecializer::specialize_node(&node, BackendTarget::Cuda).unwrap();
