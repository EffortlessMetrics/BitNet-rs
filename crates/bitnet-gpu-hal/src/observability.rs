...
pub enum MetricValue {
    Counter(u64),
    Gauge(f64),
    Histogram { observations: Vec<f64>, count: u64, sum: f64 },
}
...
impl MetricsCollector {
    pub fn new(prefix: impl Into<String>) -> Self {
        Self { definitions: HashMap::new(), values: HashMap::new(), prefix: prefix.into() }
    }
        ...
        for (name, labels) in &keys {
            ...
            match value {
                ...
                MetricValue::Histogram { observations, count, sum } => {
                    ...
                    let buckets =
                        defs.get(name.as_str()).map(|d| d.buckets.clone()).unwrap_or_default();
...
mod tests {
    ...
    fn definition_with_labels() {
        let d =
            MetricDefinition::counter("c", "d").with_labels(vec!["method".into(), "path".into()]);
    ...
    fn engine_prefix_propagated() {
        let cfg = ObservabilityConfig { metrics_prefix: "hal_".to_string(), ..Default::default() };
        let mut e = ObservabilityEngine::new(cfg);
