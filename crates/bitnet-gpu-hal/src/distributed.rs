...
impl DistributedConfig {
    ...
    pub fn validate(&self) -> Result<(), DistributedError> {
        if self.world_size == 0 {
            return Err(DistributedError::InvalidConfig("world_size must be >= 1".into()));
        }
        ...
        if let ParallelismStrategy::Hybrid { tensor_parallel_size, pipeline_parallel_size } =
            &self.strategy
        {
            let product = tensor_parallel_size * pipeline_parallel_size;
            if product != self.world_size {
                return Err(DistributedError::InvalidConfig(format!(
                    "TP({}) * PP({}) = {} != world_size({})",
                    tensor_parallel_size, pipeline_parallel_size, product, self.world_size,
                )));
            }
        }
        Ok(())
...
pub enum ParallelismStrategy {
    ...
    Hybrid { tensor_parallel_size: usize, pipeline_parallel_size: usize },
}
...
impl fmt::Display for ParallelismStrategy {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Hybrid { tensor_parallel_size: tp, pipeline_parallel_size: pp } => {
...
pub trait CommunicationBackend: fmt::Debug {
    ...
    fn all_reduce_sum(&self, data: &mut [f32]) -> Result<(), DistributedError>;
    ...
    fn all_gather(&self, send_buf: &[f32], recv_buf: &mut [f32]) -> Result<(), DistributedError>;
...
impl CommunicationBackend for MockCommunicationBackend {
    ...
    fn broadcast(&self, _data: &mut [f32], root: usize) -> Result<(), DistributedError> {
        if root >= self.world_size {
            return Err(DistributedError::CommunicationFailure(format!(
                "root {root} >= world_size {}",
                self.world_size
            )));
        }
...
impl ShardPlacement {
    ...
    pub fn element_range(&self, total_elements: usize) -> (usize, usize) {
        let chunk = total_elements / self.num_shards;
        let remainder = total_elements % self.num_shards;
        let start = chunk * self.shard_index + self.shard_index.min(remainder);
        let end = start + chunk + usize::from(self.shard_index < remainder);
        (start, end)
    }
}
...
impl FaultDetector {
    ...
    pub fn new(world_size: usize, timeout_count: u32, now_ms: u64) -> Self {
        Self {
            world_size,
    ...
    pub fn heartbeat(&mut self, rank: usize, now_ms: u64) -> Result<(), DistributedError> {
    ...
    pub fn tick(&mut self, now_ms: u64, interval_ms: u64) {
        for rank in 0..self.world_size {
            if now_ms.saturating_sub(self.last_heartbeat[rank]) >= interval_ms {
    ...
    pub fn dead_nodes(&self) -> Vec<usize> {
        (0..self.world_size).filter(|&r| self.health(r) == NodeHealth::Dead).collect()
    }
}
...
impl LoadBalancer {
    ...
    pub fn update(&mut self, rank: usize, free_memory: usize, queue_depth: u32, alive: bool) {
    ...
    pub fn select_node(&self) -> Option<usize> {
        self.capacities
            ...
            .filter(|c| c.alive)
            .min_by(|a, b| {
                a.queue_depth.cmp(&b.queue_depth).then(b.free_memory.cmp(&a.free_memory))
            })
            .map(|c| c.rank)
    }
    ...
    pub fn ranked_nodes(&self) -> Vec<usize> {
        let mut alive: Vec<&NodeCapacity> = self.capacities.iter().filter(|c| c.alive).collect();
...
impl<B: CommunicationBackend> DistributedEngine<B> {
    ...
    pub fn new(config: DistributedConfig, backend: B) -> Result<Self, DistributedError> {
        config.validate()?;

        let fault_detector =
            FaultDetector::new(config.world_size, config.heartbeat_timeout_count, 0);
        ...
        let pipeline_stages = match &config.strategy {
            ParallelismStrategy::PipelineParallel => partition_layers(32, config.world_size),
            ParallelismStrategy::Hybrid { pipeline_parallel_size, .. } => {
        ...
        let shard_map = match &config.strategy {
            ...
            ParallelismStrategy::Hybrid { tensor_parallel_size, .. } => {
    ...
    pub fn tick_fault_detector(&mut self, now_ms: u64) {
        self.fault_detector.tick(now_ms, self.config.heartbeat_interval_ms);
    }
    ...
    pub fn reshard_on_failure(
        ...
        if alive == 0 {
            return Err(DistributedError::NodeFailure { rank: failed_rank });
        }
    ...
    pub fn create_checkpoint(&mut self, kv_states: HashMap<usize, Vec<f32>>) -> Checkpoint {
        let ckpt = Checkpoint {
    ...
    pub fn restore_checkpoint(&mut self, checkpoint: &Checkpoint) -> Result<(), DistributedError> {
        #[allow(clippy::cast_possible_truncation)]
        if checkpoint.id as usize >= self.checkpoints.len() {
            return Err(DistributedError::CheckpointError(format!(
    ...
    pub fn all_reduce_sum(&self, data: &mut [f32]) -> Result<(), DistributedError> {
        self.backend.all_reduce_sum(data)
    }

    /// Broadcast data from `root`.
    pub fn broadcast(&self, data: &mut [f32], root: usize) -> Result<(), DistributedError> {
...
mod tests {
    ...
    fn mock_engine(world_size: usize) -> DistributedEngine<MockCommunicationBackend> {
    ...
    fn pipeline_engine(world_size: usize) -> DistributedEngine<MockCommunicationBackend> {
    ...
    fn config_rank_gte_world_size_rejected() {
        let cfg = DistributedConfig { world_size: 2, rank: 2, ..Default::default() };
        assert!(cfg.validate().is_err());
    }

    #[test]
    fn config_zero_heartbeat_rejected() {
        let cfg = DistributedConfig { heartbeat_interval_ms: 0, ..Default::default() };
        assert!(cfg.validate().is_err());
    }
    ...
    fn strategy_display_tensor_parallel() {
        assert_eq!(ParallelismStrategy::TensorParallel.to_string(), "TensorParallel");
    }

    #[test]
    fn strategy_display_pipeline_parallel() {
        assert_eq!(ParallelismStrategy::PipelineParallel.to_string(), "PipelineParallel");
    }

    #[test]
    fn strategy_display_data_parallel() {
        assert_eq!(ParallelismStrategy::DataParallel.to_string(), "DataParallel");
    }

    #[test]
    fn strategy_display_hybrid() {
        let s = ParallelismStrategy::Hybrid { tensor_parallel_size: 2, pipeline_parallel_size: 4 };
    ...
    fn error_display_coverage() {
        let cases: Vec<(DistributedError, &str)> = vec![
            (DistributedError::InvalidConfig("bad".into()), "invalid distributed config: bad"),
            (DistributedError::CommunicationFailure("oops".into()), "communication failure: oops"),
            (DistributedError::NodeFailure { rank: 3 }, "node 3 failed"),
            (DistributedError::CheckpointError("gone".into()), "checkpoint error: gone"),
            (
                DistributedError::LengthMismatch { expected: 10, actual: 5 },
    ...
    fn load_balancer_selects_lowest_queue_depth() {
        let caps = vec![
            NodeCapacity { rank: 0, free_memory: 1000, queue_depth: 5, alive: true },
            NodeCapacity { rank: 1, free_memory: 1000, queue_depth: 2, alive: true },
            NodeCapacity { rank: 2, free_memory: 1000, queue_depth: 8, alive: true },
        ];
    ...
    fn load_balancer_breaks_ties_by_memory() {
        let caps = vec![
            NodeCapacity { rank: 0, free_memory: 500, queue_depth: 1, alive: true },
            NodeCapacity { rank: 1, free_memory: 900, queue_depth: 1, alive: true },
        ];
        let lb = LoadBalancer::new(caps);
        assert_eq!(lb.select_node(), Some(1));
    }
    ...
    fn load_balancer_skips_dead_nodes() {
        let caps = vec![
            NodeCapacity { rank: 0, free_memory: 1000, queue_depth: 0, alive: false },
            NodeCapacity { rank: 1, free_memory: 100, queue_depth: 5, alive: true },
        ];
        let lb = LoadBalancer::new(caps);
        assert_eq!(lb.select_node(), Some(1));
    }
    ...
    fn load_balancer_no_alive_returns_none() {
        let caps = vec![NodeCapacity { rank: 0, free_memory: 1000, queue_depth: 0, alive: false }];
        let lb = LoadBalancer::new(caps);
    ...
    fn load_balancer_mark_dead() {
        let caps = vec![
            NodeCapacity { rank: 0, free_memory: 1000, queue_depth: 0, alive: true },
            NodeCapacity { rank: 1, free_memory: 1000, queue_depth: 0, alive: true },
        ];
    ...
    fn load_balancer_update_capacity() {
        let caps = vec![
            NodeCapacity { rank: 0, free_memory: 500, queue_depth: 3, alive: true },
            NodeCapacity { rank: 1, free_memory: 500, queue_depth: 3, alive: true },
        ];
        let mut lb = LoadBalancer::new(caps);
    ...
    fn load_balancer_ranked_nodes_by_memory() {
        let caps = vec![
            NodeCapacity { rank: 0, free_memory: 100, queue_depth: 0, alive: true },
            NodeCapacity { rank: 1, free_memory: 300, queue_depth: 0, alive: true },
            NodeCapacity { rank: 2, free_memory: 200, queue_depth: 0, alive: true },
        ];
        let lb = LoadBalancer::new(caps);
    ...
    fn checkpoint_round_trip() {
        ...
        e.advance(10);
        let kv = HashMap::from([(0, vec![1.0, 2.0]), (1, vec![3.0, 4.0])]);
    ...
    fn restore_invalid_checkpoint_errors() {
        let mut e = mock_engine(2);
        let bad =
            Checkpoint { id: 99, ranks: vec![0, 1], kv_states: HashMap::new(), token_position: 0 };
