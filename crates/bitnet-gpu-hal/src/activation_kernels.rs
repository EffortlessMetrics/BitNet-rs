...
pub trait ActivationKernel {
    ...
    fn swish(&self, input: &[f32], output: &mut [f32], beta: f32) -> Result<usize, HalError>;

    /// Logistic sigmoid: `1 / (1 + exp(-x))`.
    fn sigmoid(&self, input: &[f32], output: &mut [f32]) -> Result<usize, HalError>;

    /// Hyperbolic tangent activation.
    fn tanh_activation(&self, input: &[f32], output: &mut [f32]) -> Result<usize, HalError>;

    /// Leaky `ReLU`: `x` if `x ≥ 0`, else `alpha·x`.
    fn leaky_relu(&self, input: &[f32], output: &mut [f32], alpha: f32) -> Result<usize, HalError>;

    /// ELU: `x` if `x ≥ 0`, else `alpha·(exp(x) − 1)`.
    fn elu(&self, input: &[f32], output: &mut [f32], alpha: f32) -> Result<usize, HalError>;
}
...
const fn check_len(input: &[f32], output: &[f32]) -> Result<usize, HalError> {
    let n = input.len();
    if output.len() < n {
        return Err(HalError::ShapeMismatch { expected: n, actual: output.len() });
    }
    Ok(n)
}
...
impl ActivationKernel for CpuActivationKernels {
    ...
    fn elu(&self, input: &[f32], output: &mut [f32], alpha: f32) -> Result<usize, HalError> {
        let n = check_len(input, output)?;
        for i in 0..n {
            output[i] = if input[i] >= 0.0 { input[i] } else { alpha * (safe_exp(input[i]) - 1.0) };
        }
        Ok(n)
    }
}
...
pub fn activate_inplace(
    ...
    match kind {
        ...
        ActivationKind::LeakyRelu => kernel.leaky_relu(&tmp, data, config.leaky_relu_alpha),
...
pub fn activate_batched(
    ...
    for start in (0..n).step_by(batch_size) {
        ...
        let w = match kind {
            ...
            ActivationKind::Tanh => kernel.tanh_activation(chunk_in, chunk_out)?,
            ActivationKind::LeakyRelu => {
                kernel.leaky_relu(chunk_in, chunk_out, config.leaky_relu_alpha)?
            }
            ActivationKind::Elu => kernel.elu(chunk_in, chunk_out, config.elu_alpha)?,
        };
        written += w;
    }
    Ok(written)
...
pub fn fused_activation_bias(
    ...
    let n = check_len(input, output)?;
    if n % bias.len() != 0 {
        return Err(HalError::ShapeMismatch { expected: n, actual: bias.len() });
    }
    ...
    match kind {
        ...
        ActivationKind::LeakyRelu => kernel.leaky_relu(&biased, output, config.leaky_relu_alpha),
    ...
    pub fn run(
        ...
        for _ in 0..iters {
            let _ = match kind {
                ...
                ActivationKind::Swish => kernel.swish(&input, &mut output, 1.0),
                ActivationKind::Sigmoid => kernel.sigmoid(&input, &mut output),
                ActivationKind::Tanh => kernel.tanh_activation(&input, &mut output),
                ActivationKind::LeakyRelu => {
                    kernel.leaky_relu(&input, &mut output, config.leaky_relu_alpha)
                }
                ActivationKind::Elu => kernel.elu(&input, &mut output, config.elu_alpha),
            };
        }
...
mod tests {
    ...
    fn tanh_inf_saturates() {
        let k = kern();
        let mut out = [0.0_f32; 2];
        k.tanh_activation(&[f32::INFINITY, f32::NEG_INFINITY], &mut out).unwrap();
        assert!((out[0] - 1.0).abs() < 1e-6);
    ...
    fn batched_relu_full() {
        ...
        let mut out = vec![0.0; 4];
        let n = activate_batched(&k, ActivationKind::Relu, &input, &mut out, &c, 2).unwrap();
        assert_eq!(n, 4);
    ...
    fn batched_empty_input() {
        ...
        let c = cfg();
        let mut out = [];
        let n = activate_batched(&k, ActivationKind::Relu, &[], &mut out, &c, 64).unwrap();
        assert_eq!(n, 0);
    }
    ...
    fn batched_zero_batch_size_errors() {
        ...
        assert!(activate_batched(&k, ActivationKind::Relu, &input, &mut out, &c, 0,).is_err());
    }
    ...
    fn fused_relu_bias() {
        ...
        fused_activation_bias(&k, ActivationKind::Relu, &input, &bias, &mut out, &c).unwrap();
    ...
    fn fused_sigmoid_bias_broadcast() {
        ...
        let bias = [0.0, 100.0];
        let mut out = [0.0_f32; 4];
        fused_activation_bias(&k, ActivationKind::Sigmoid, &input, &bias, &mut out, &c).unwrap();
        assert!((out[0] - 0.5).abs() < 1e-6);
        assert!((out[1] - 1.0).abs() < 1e-6);
    ...
    fn fused_empty_bias_errors() {
        ...
        let mut out = [0.0_f32; 1];
        assert!(
            fused_activation_bias(&k, ActivationKind::Relu, &input, &[], &mut out, &c,).is_err()
        );
    }
    ...
    fn fused_mismatched_bias_errors() {
        ...
        // 3 % 2 != 0 → error
        assert!(
            fused_activation_bias(&k, ActivationKind::Relu, &input, &bias, &mut out, &c,).is_err()
        );
    }
    ...
    fn fused_all_kinds() {
        ...
        for kind in kinds {
            let mut out = [0.0_f32; 2];
            fused_activation_bias(&k, kind, &input, &bias, &mut out, &c).unwrap();
        }
    }
    ...
    fn benchmark_basic() {
        let k = kern();
        let c = cfg();
        let b = ActivationBenchmark::run(&k, ActivationKind::Relu, 1024, 10, &c);
    ...
    fn tanh_bounded() {
        ...
        for &v in &out {
            assert!((-1.0..=1.0).contains(&v), "tanh in [-1,1], got {v}");
        }
    }
    ...
    fn large_vector_all_activations() {
        ...
        for kind in kinds {
            ...
            activate_batched(&k, kind, &input, &mut out, &c, 512).unwrap();
            // Verify all outputs are finite (no NaN/Inf from stable math)
            for &v in &out {
                assert!(v.is_finite(), "{kind:?} produced non-finite value");
            }
        }
    }
}
