...
impl FfiGuard {
    ...
    pub fn new(library_name: &str) -> Self {
        Self { library_name: library_name.to_string(), loaded: false, symbol_checks: Vec::new() }
    }
    ...
    pub fn check_symbol(&mut self, name: &str, required: bool) -> bool {
        ...
        self.symbol_checks.push(SymbolCheck { name: name.to_string(), required, found });
        found
    }
    ...
    pub fn all_required_found(&self) -> bool {
        self.symbol_checks.iter().filter(|s| s.required).all(|s| s.found)
    }
...
impl<T> FfiResult<T> {
    ...
    pub fn err(code: i32, message: &str) -> Self {
        Self { value: None, error_code: code, error_message: Some(message.to_string()) }
    }
    ...
    pub fn into_result(self) -> Result<T, FfiError> {
        if self.error_code != 0 {
            return Err(FfiError::RuntimeError {
                ...
                message: self.error_message.unwrap_or_else(|| "unknown error".to_string()),
            });
        }
...
impl LibraryLoader {
    ...
    pub fn find_library(&self, name: &str) -> Option<&str> {
        self.search_paths.iter().find(|p| p.contains(name)).map(String::as_str)
    }
...
impl NullCheckGuard {
    ...
    pub const fn check_ptr<T>(ptr: *const T) -> Result<(), FfiError> {
        if ptr.is_null() { Err(FfiError::NullPointer) } else { Ok(()) }
    }
...
impl BoundsCheckGuard {
    ...
    pub const fn check(&self, offset: usize, size: usize) -> Result<(), FfiError> {
    ...
    pub const fn check_index(&self, index: usize) -> Result<(), FfiError> {
        if index >= self.max_size {
            Err(FfiError::BufferOverflow { expected: self.max_size, actual: index })
        } else {
            Ok(())
        }
    }
...
mod tests {
    ...
    fn display_buffer_overflow() {
        ...
        assert_eq!(e.to_string(), "buffer overflow: expected at most 100, got 120");
    }
    ...
    fn display_runtime_error() {
        let e = FfiError::RuntimeError { code: -1, message: "device lost".into() };
    ...
    fn guard_add_symbol_check_manually() {
        let mut g = FfiGuard::new("lib");
        g.add_symbol_check(SymbolCheck { name: "myFunc".into(), required: true, found: true });
        assert!(g.all_required_found());
    }
    ...
    fn result_err_with_code_and_message() {
        ...
        assert_eq!(e, FfiError::RuntimeError { code: -5, message: "out of memory".into() });
    }

    #[test]
    fn result_err_default_message() {
        let r: FfiResult<()> = FfiResult { value: None, error_code: 1, error_message: None };
    ...
    fn result_ok_no_value_gives_null_pointer() {
        let r: FfiResult<i32> = FfiResult { value: None, error_code: 0, error_message: None };
    ...
    fn result_err_zero_code_but_no_value() {
        let r: FfiResult<u32> = FfiResult { value: None, error_code: 0, error_message: None };
        assert!(!r.is_ok());
    }
    ...
    fn loader_find_library_first_match_wins() {
        ...
        assert_eq!(loader.find_library("libcuda"), Some("/first/libcuda.so"));
    }
    ...
    fn null_check_null() {
        ...
        assert_eq!(NullCheckGuard::check_ptr(ptr).unwrap_err(), FfiError::NullPointer,);
    }
    ...
    fn null_check_mut_null() {
        ...
        assert_eq!(NullCheckGuard::check_mut_ptr(ptr).unwrap_err(), FfiError::NullPointer,);
    }
    ...
    fn bounds_overflow() {
        ...
        assert_eq!(err, FfiError::BufferOverflow { expected: 100, actual: 110 },);
    }
    ...
    #[test]
    fn error_clone() {
        let e = FfiError::RuntimeError { code: 7, message: "fail".into() };
        let e2 = e.clone();
        assert_eq!(e, e2);
    }
