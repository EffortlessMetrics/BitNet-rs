...
impl FragmentationAnalyzer {
    ...
    pub fn analyze(blocks: &[MemoryBlock]) -> FragmentationMetrics {
        let free_blocks: Vec<&MemoryBlock> = blocks.iter().filter(|b| b.is_free).collect();
        ...
        let total_waste: usize =
            free_blocks.iter().filter(|b| b.size < largest_free).map(|b| b.size).sum();
...
impl OomRecovery {
    ...
    pub fn suggest(shortfall: usize, blocks: &[MemoryBlock]) -> Vec<OomAction> {
        let evictable: usize =
            blocks.iter().filter(|b| !b.is_free && b.pin_count == 0).map(|b| b.size).sum();
...
impl AllocationHistory {
    ...
    pub fn record(&mut self, is_alloc: bool, size: usize, address: usize) {
        ...
        self.events.push_back(AllocEvent { seq: self.next_seq, is_alloc, size, address });
        self.next_seq += 1;
    }
    ...
    pub fn net_allocated(&self) -> i64 {
        self.events.iter().fold(0i64, |acc, e| {
            ...
            if e.is_alloc { acc + e.size as i64 } else { acc - e.size as i64 }
        })
    }
}
...
impl MemoryDefragmenter {
    ...
    /// address 0.
    pub fn plan_compaction(&self, blocks: &[MemoryBlock]) -> Result<CompactionPlan, HalError> {
        if blocks.is_empty() {
        ...
        for blk in blocks {
            ...
            if blk.address != cursor {
                moves.push(BlockMove { src: blk.address, dst: cursor, size: blk.size });
            }
            cursor += blk.size;
        }
    ...
    pub fn run(&mut self, blocks: &[MemoryBlock]) -> Result<CompactionPlan, HalError> {
        let plan = match self.config.strategy {
            DefragStrategy::Compaction | DefragStrategy::Incremental => {
                self.plan_compaction(blocks)?
            }
            DefragStrategy::BuddyRebalance | DefragStrategy::GenerationalCollect => {
...
mod tests {
    ...
    fn block(address: usize, size: usize, is_free: bool) -> MemoryBlock {
        MemoryBlock { address, size, is_free, generation: 0, pin_count: 0 }
    }

    fn pinned_block(address: usize, size: usize, pin_count: u32) -> MemoryBlock {
        MemoryBlock { address, size, is_free: false, generation: 0, pin_count }
    }

    fn gen_block(address: usize, size: usize, generation: u32) -> MemoryBlock {
        MemoryBlock { address, size, is_free: false, generation, pin_count: 0 }
    }
    ...
    fn no_fragmentation_when_single_free_block() {
        let blocks = vec![block(0, 256, false), block(256, 256, true)];
    ...
    fn many_small_free_blocks_high_fragmentation() {
        let blocks: Vec<MemoryBlock> = (0..10)
            .flat_map(|i| vec![block(i * 200, 100, false), block(i * 200 + 100, 100, true)])
            .collect();
        let m = FragmentationAnalyzer::analyze(&blocks);
    ...
    fn pressure_monitor_window_eviction() {
        let mut pm = PressureMonitor::with_watermarks(1000, WatermarkConfig::default());
    ...
    fn defrag_should_defrag_on_high_fragmentation() {
        let cfg = DefragConfig { fragmentation_threshold: 0.2, ..Default::default() };
        let d = MemoryDefragmenter::new(cfg);
        // Two equal free blocks → ratio = 0.5 > 0.2
        let blocks = vec![block(0, 100, true), block(100, 100, false), block(200, 100, true)];
        assert!(d.should_defrag(&blocks));
    }

    #[test]
    fn defrag_no_defrag_low_fragmentation() {
        let cfg = DefragConfig { fragmentation_threshold: 0.9, ..Default::default() };
        let d = MemoryDefragmenter::new(cfg);
        let blocks = vec![block(0, 100, false), block(100, 900, true)];
        assert!(!d.should_defrag(&blocks));
    }
    ...
    fn plan_compaction_no_fragmentation() {
        let d = MemoryDefragmenter::new(DefragConfig::default());
        let blocks = vec![block(0, 128, false), block(128, 128, false), block(256, 256, true)];
    ...
    fn run_compaction_resets_failures() {
        ...
        let blocks = vec![block(0, 64, true), block(64, 128, false)];
    ...
    fn run_buddy_rebalance_falls_back_to_compaction() {
        let cfg = DefragConfig { strategy: DefragStrategy::BuddyRebalance, ..Default::default() };
        let mut d = MemoryDefragmenter::new(cfg);
        let blocks = vec![block(0, 32, true), block(32, 64, false)];
    ...
    fn run_generational_collect_falls_back() {
        let cfg =
            DefragConfig { strategy: DefragStrategy::GenerationalCollect, ..Default::default() };
        let mut d = MemoryDefragmenter::new(cfg);
        let blocks = vec![block(0, 16, true), block(16, 32, false)];
        let plan = d.run(&blocks).unwrap();
        assert_eq!(plan.moves.len(), 1);
    }
    ...
    fn run_incremental_uses_compaction() {
        let cfg = DefragConfig { strategy: DefragStrategy::Incremental, ..Default::default() };
        let mut d = MemoryDefragmenter::new(cfg);
        let blocks = vec![block(0, 8, true), block(8, 16, false)];
        let plan = d.run(&blocks).unwrap();
        assert_eq!(plan.moves.len(), 1);
    }
    ...
    fn scenario_pressure_to_oom_recovery() {
        ...
        let blocks = vec![block(0, 200, false), pinned_block(200, 300, 1), block(500, 500, true)];
