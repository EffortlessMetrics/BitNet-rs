...
impl BenchmarkCase {
    pub fn new(name: impl Into<String>, input_sizes: Vec<usize>, expected_op: OpKind) -> Self {
...
impl LatencyHistogram {
    ...
    pub fn from_samples(samples: &[u64]) -> Option<Self> {
        ...
        let mean_ns = {
            ...
            let v = sorted.iter().copied().sum::<u64>() as f64 / n as f64;
            v
        };
        Some(Self {
...
impl BenchComparison {
    pub fn from_results(baseline: &BenchResult, candidate: &BenchResult) -> Self {
        #[allow(clippy::cast_precision_loss)]
...
impl RegressionDetector {
    ...
    pub fn set_baseline(&mut self, backend: &str, case_name: &str, elapsed_ns: u64) {
        let key = format!("{backend}::{case_name}");
        self.baselines.insert(key, elapsed_ns);
    }

    /// Load baselines from JSON bytes.
    pub fn load_baselines(&mut self, json: &[u8]) -> Result<usize, serde_json::Error> {
    ...
    pub fn check(&self, backend: &str, case_name: &str, elapsed_ns: u64) -> RegressionStatus {
        let key = format!("{backend}::{case_name}");
        ...
        #[allow(clippy::cast_precision_loss)]
        let delta_pct = (elapsed_ns as f64 - baseline_ns as f64) / baseline_ns as f64;
...
impl BenchmarkReport {
    ...
    fn render_csv(&self) -> String {
        let mut out = String::from("backend,case,elapsed_ns,throughput_gops\n");
    ...
    fn render_table(&self) -> String {
        ...
        let _ = writeln!(out, "{:<12} {:<24} {:>14} {:>12}", "Backend", "Case", "Elapsed", "GOPS",);
        ...
        if !self.comparisons.is_empty() {
            ...
            for c in &self.comparisons {
                let _ = writeln!(
                    out,
                    "{:<24} {:<12} {:<12} {:>10.3}",
                    c.case_name, c.baseline_backend, c.candidate_backend, c.speedup_ratio,
                );
            }
        }
        out
...
pub fn standard_scenarios() -> Vec<BenchmarkCase> {
    vec![
        BenchmarkCase::new("matmul_128x128", vec![128, 128], OpKind::Matmul),
        BenchmarkCase::new("matmul_512x512", vec![512, 512], OpKind::Matmul),
        BenchmarkCase::new("matmul_1024x1024", vec![1024, 1024], OpKind::Matmul),
        BenchmarkCase::new("matmul_2048x2048", vec![2048, 2048], OpKind::Matmul),
        BenchmarkCase::new("attention_seq64_dim64", vec![64, 64], OpKind::Attention),
        BenchmarkCase::new("attention_seq256_dim128", vec![256, 128], OpKind::Attention),
        BenchmarkCase::new("softmax_1024", vec![1024], OpKind::Softmax),
        BenchmarkCase::new("softmax_32768", vec![32768], OpKind::Softmax),
        BenchmarkCase::new("quantize_4096", vec![4096], OpKind::Quantize),
        BenchmarkCase::new("quantize_16384", vec![16384], OpKind::Quantize),
        BenchmarkCase::new("tokenize_128", vec![128], OpKind::Tokenize),
        BenchmarkCase::new("elementwise_8192", vec![8192], OpKind::Elementwise),
        BenchmarkCase::new("reduce_8192", vec![8192], OpKind::Reduce),
        BenchmarkCase::new("rms_norm_4096", vec![4096], OpKind::RmsNorm),
...
fn simulate_softmax(sizes: &[usize]) -> u64 {
    ...
    let mut logits: Vec<f32> = (0..n).map(|i| (i as f32) * 0.01).collect();
    let start = Instant::now();
...
fn simulate_rms_norm(sizes: &[usize]) -> u64 {
    ...
    let w = vec![1.0_f32; n];
    let start = Instant::now();
    let n_f32 = x.len() as f32;
    let rms = (x.iter().map(|v| v * v).sum::<f32>() / n_f32 + 1e-5).sqrt();
...
fn simulate_rope(sizes: &[usize]) -> u64 {
    ...
    let start = Instant::now();
    let half = dim / 2;
    let mut cos_table = vec![0.0_f32; seq_len * half];
    let mut sin_table = vec![0.0_f32; seq_len * half];
...
fn run_workload(op: OpKind, sizes: &[usize]) -> u64 {
    match op {
        ...
        OpKind::Elementwise | OpKind::Tokenize => simulate_elementwise(sizes),
        OpKind::Reduce => simulate_reduce(sizes),
        OpKind::Quantize | OpKind::Dequantize => simulate_quantize(sizes),
...
impl BenchmarkSuite {
    ...
    pub fn new(config: BenchConfig) -> Self {
        Self { config, cases: Vec::new(), detector: RegressionDetector::new(0.10) }
    }

    /// Create a suite with default standard scenarios.
    pub fn with_standard_scenarios(config: BenchConfig) -> Self {
        Self { config, cases: standard_scenarios(), detector: RegressionDetector::new(0.10) }
    }
    ...
    pub fn run(&self) -> BenchmarkReport {
        ...
        for case in &self.cases {
            for backend in &self.config.backends_to_test {
                let result = self.run_single(backend, case);

                // Build histogram
                if let Some(hist) = LatencyHistogram::from_samples(&result.latency_samples_ns) {
                    let key = format!("{}::{}", result.backend, result.case_name);
                    report.histograms.insert(key, hist);
                }

                // Check regression
                let status =
                    self.detector.check(&result.backend, &result.case_name, result.elapsed_ns);
                let key = format!("{}::{}", result.backend, result.case_name);
        ...
        if self.config.backends_to_test.len() >= 2 {
            ...
            for case in &self.cases {
                let baseline_result = report
                    .results
                    .iter()
                    .find(|r| r.backend == *baseline_backend && r.case_name == case.name);
                for other in &self.config.backends_to_test[1..] {
                    let cand_result = report
                        .results
                        .iter()
                        .find(|r| r.backend == *other && r.case_name == case.name);
                    if let (Some(b), Some(c)) = (baseline_result, cand_result) {
                        report.comparisons.push(BenchComparison::from_results(b, c));
                    }
                }
            }
        }
    ...
    fn run_single(&self, backend: &str, case: &BenchmarkCase) -> BenchResult {
        // Warm-up
        ...
        let throughput_gops = if case.expected_op == OpKind::Matmul {
            let n = case.input_sizes.first().copied().unwrap_or(1) as f64;
            let flops = 2.0 * n * n * n;
...
mod tests {
    ...
    fn case_construction() {
        let c = BenchmarkCase::new("test_case", vec![64, 64], OpKind::Matmul);
    ...
    fn case_serialization_roundtrip() {
        let c = BenchmarkCase::new("matmul_64", vec![64], OpKind::Matmul);
    ...
    fn standard_scenarios_have_unique_names() {
        let s = standard_scenarios();
        let mut names: Vec<&str> = s.iter().map(|c| c.name.as_str()).collect();
    ...
    fn standard_scenarios_sizes_nonempty() {
        let s = standard_scenarios();
        for case in &s {
            assert!(!case.input_sizes.is_empty(), "case {} has empty sizes", case.name,);
        }
    }
    ...
    fn suite_with_standard_scenarios() {
        let s = BenchmarkSuite::with_standard_scenarios(BenchConfig::default());
    ...
    fn suite_add_case() {
        ...
        s.add_case(BenchmarkCase::new("custom", vec![32], OpKind::Reduce));
    ...
    fn suite_run_single_case() {
        ...
        s.add_case(BenchmarkCase::new("softmax_small", vec![64], OpKind::Softmax));
        let r = s.run();
    ...
    fn suite_run_produces_histograms() {
        ...
        let mut s = BenchmarkSuite::new(cfg);
        s.add_case(BenchmarkCase::new("reduce_small", vec![128], OpKind::Reduce));
        let r = s.run();
    ...
    fn suite_run_multi_backend() {
        ...
        let mut s = BenchmarkSuite::new(cfg);
        s.add_case(BenchmarkCase::new("elem_tiny", vec![16], OpKind::Elementwise));
        let r = s.run();
    ...
    fn suite_detector_integration() {
        ...
        let r = s.run();
        assert!(r.regressions.iter().all(|(_, st)| { matches!(st, RegressionStatus::NoBaseline) }));
    }
    ...
    fn suite_matmul_throughput_positive() {
        ...
        let mut s = BenchmarkSuite::new(cfg);
        s.add_case(BenchmarkCase::new("matmul_tiny", vec![16, 16], OpKind::Matmul));
        let r = s.run();
    ...
    fn suite_non_matmul_throughput_zero() {
        ...
        let mut s = BenchmarkSuite::new(cfg);
        s.add_case(BenchmarkCase::new("reduce_tiny", vec![64], OpKind::Reduce));
        let r = s.run();
