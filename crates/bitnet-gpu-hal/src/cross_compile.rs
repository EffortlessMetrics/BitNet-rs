...
impl TargetPlatform {
    ...
    pub const fn is_posix(self) -> bool {
        matches!(
            self,
            Self::X86_64Linux | Self::X86_64MacOS | Self::Aarch64Linux | Self::Aarch64MacOS
        )
    }
...
impl GpuBackend {
    pub const ALL: &[Self] = &[Self::Cpu, Self::Cuda, Self::Metal, Self::Vulkan];
...
pub const fn backend_availability(platform: TargetPlatform, backend: GpuBackend) -> Availability {
    match (platform, backend) {
        ...
        (TargetPlatform::Aarch64MacOS, GpuBackend::Cuda) => Availability::Unavailable,
        (_, GpuBackend::Cuda) => Availability::Optional,

        // Metal: only on macOS.
        (TargetPlatform::X86_64MacOS, GpuBackend::Metal) => Availability::Available,
        (TargetPlatform::Aarch64MacOS, GpuBackend::Metal) => Availability::Available,
        (_, GpuBackend::Metal) => Availability::Unavailable,

        // Vulkan: unavailable on wasm32, optional elsewhere.
        (TargetPlatform::Wasm32, GpuBackend::Vulkan) => Availability::Unavailable,
...
impl BuildConfig {
    ...
    pub fn generate(platform: TargetPlatform, backend: GpuBackend) -> Option<Self> {
...
fn linker_flags_for(platform: TargetPlatform, backend: GpuBackend) -> Vec<String> {
...
fn env_vars_for(platform: TargetPlatform, backend: GpuBackend) -> Vec<(String, String)> {
...
pub enum CompileCheckError {
    ...
    UnavailableBackend { platform: TargetPlatform, backend: GpuBackend },
    /// Target triple does not match the platform enum.
    TripleMismatch { expected: String, actual: String },
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TripleMismatch { expected, actual } => {
                write!(f, "triple mismatch: expected {expected}, got {actual}")
            }
...
pub fn compile_check(config: &BuildConfig) -> Result<(), Vec<CompileCheckError>> {
    let mut errors = Vec::new();

    // Backend must not be unavailable.
    if backend_availability(config.platform, config.backend) == Availability::Unavailable {
    ...
    if !matches!(config.backend, GpuBackend::Cpu) && !config.features.iter().any(|f| f == "gpu") {
...
pub fn resolve_features(platform: TargetPlatform, backend: GpuBackend) -> Option<ResolvedFeatures> {
    if backend_availability(platform, backend) == Availability::Unavailable {
        return None;
    }
...
mod tests {
    ...
    fn platform_triples_are_unique() {
        let triples: Vec<_> = TargetPlatform::ALL.iter().map(|p| p.target_triple()).collect();
    ...
    fn x86_64_linux_triple() {
        assert_eq!(TargetPlatform::X86_64Linux.target_triple(), "x86_64-unknown-linux-gnu");
    }

    #[test]
    fn x86_64_windows_triple() {
        assert_eq!(TargetPlatform::X86_64Windows.target_triple(), "x86_64-pc-windows-msvc");
    }

    #[test]
    fn x86_64_macos_triple() {
        assert_eq!(TargetPlatform::X86_64MacOS.target_triple(), "x86_64-apple-darwin");
    }

    #[test]
    fn aarch64_linux_triple() {
        assert_eq!(TargetPlatform::Aarch64Linux.target_triple(), "aarch64-unknown-linux-gnu");
    }

    #[test]
    fn aarch64_macos_triple() {
        assert_eq!(TargetPlatform::Aarch64MacOS.target_triple(), "aarch64-apple-darwin");
    }

    #[test]
    fn wasm32_triple() {
        assert_eq!(TargetPlatform::Wasm32.target_triple(), "wasm32-unknown-unknown");
    }
    ...
    fn cpu_available_everywhere() {
        for &p in TargetPlatform::ALL {
            assert_eq!(backend_availability(p, GpuBackend::Cpu), Availability::Available,);
        }
    }
    ...
    fn cuda_unavailable_on_wasm() {
        assert_eq!(
            backend_availability(TargetPlatform::Wasm32, GpuBackend::Cuda,),
            Availability::Unavailable,
        );
    }
    ...
    fn cuda_unavailable_on_aarch64_macos() {
        assert_eq!(
            backend_availability(TargetPlatform::Aarch64MacOS, GpuBackend::Cuda,),
            Availability::Unavailable,
        );
    }
    ...
    fn cuda_optional_on_x86_linux() {
        assert_eq!(
            backend_availability(TargetPlatform::X86_64Linux, GpuBackend::Cuda,),
    ...
    fn cuda_optional_on_x86_windows() {
        assert_eq!(
            backend_availability(TargetPlatform::X86_64Windows, GpuBackend::Cuda,),
            Availability::Optional,
        );
    }
    ...
    fn cuda_optional_on_aarch64_linux() {
        assert_eq!(
            backend_availability(TargetPlatform::Aarch64Linux, GpuBackend::Cuda,),
            Availability::Optional,
        );
    }
    ...
    fn metal_available_on_macos() {
        assert_eq!(
            backend_availability(TargetPlatform::X86_64MacOS, GpuBackend::Metal,),
            Availability::Available,
        );
        assert_eq!(
            backend_availability(TargetPlatform::Aarch64MacOS, GpuBackend::Metal,),
            Availability::Available,
        );
    }
    ...
    fn metal_unavailable_off_macos() {
        assert_eq!(
            backend_availability(TargetPlatform::X86_64Linux, GpuBackend::Metal,),
            Availability::Unavailable,
        );
        assert_eq!(
            backend_availability(TargetPlatform::X86_64Windows, GpuBackend::Metal,),
            Availability::Unavailable,
        );
        assert_eq!(
            backend_availability(TargetPlatform::Wasm32, GpuBackend::Metal,),
            Availability::Unavailable,
        );
    }
    ...
    fn vulkan_unavailable_on_wasm() {
        assert_eq!(
            backend_availability(TargetPlatform::Wasm32, GpuBackend::Vulkan,),
            Availability::Unavailable,
        );
    }
    ...
    fn vulkan_optional_on_linux() {
        assert_eq!(
            backend_availability(TargetPlatform::X86_64Linux, GpuBackend::Vulkan,),
            Availability::Optional,
        );
    }
    ...
    fn wasm_only_has_cpu() {
        let caps = PlatformCapabilities::for_platform(TargetPlatform::Wasm32);
    ...
    fn aarch64_macos_has_cpu_and_metal() {
        let caps = PlatformCapabilities::for_platform(TargetPlatform::Aarch64MacOS);
    ...
    fn x86_linux_optional_includes_cuda_and_vulkan() {
        let caps = PlatformCapabilities::for_platform(TargetPlatform::X86_64Linux);
    ...
    fn generate_returns_none_for_unavailable() {
        assert!(BuildConfig::generate(TargetPlatform::Wasm32, GpuBackend::Cuda,).is_none());
    }

    #[test]
    fn generate_cpu_on_linux() {
        let cfg = BuildConfig::generate(TargetPlatform::X86_64Linux, GpuBackend::Cpu).unwrap();
    ...
    fn generate_cuda_includes_gpu_umbrella() {
        let cfg = BuildConfig::generate(TargetPlatform::X86_64Linux, GpuBackend::Cuda).unwrap();
        assert!(cfg.features.contains(&"gpu".to_owned()));
    ...
    fn metal_config_has_framework_flags() {
        let cfg = BuildConfig::generate(TargetPlatform::Aarch64MacOS, GpuBackend::Metal).unwrap();
        assert!(cfg.linker_flags.iter().any(|f| f == "-framework Metal"));
    }
    ...
    fn windows_config_no_posix_flags() {
        let cfg = BuildConfig::generate(TargetPlatform::X86_64Windows, GpuBackend::Cpu).unwrap();
        assert!(!cfg.linker_flags.contains(&"-lpthread".to_owned()));
    }
    ...
    fn windows_config_has_cl_compiler() {
        let cfg = BuildConfig::generate(TargetPlatform::X86_64Windows, GpuBackend::Cpu).unwrap();
        assert!(cfg.env_vars.iter().any(|(k, v)| k == "CC" && v == "cl.exe"));
    }
    ...
    fn matrix_has_no_unavailable_combos() {
        ...
        for cfg in &matrix.configs {
            assert_ne!(backend_availability(cfg.platform, cfg.backend), Availability::Unavailable,);
        }
    }
    ...
    fn wrong_triple_fails_check() {
        let mut cfg = BuildConfig::generate(TargetPlatform::X86_64Linux, GpuBackend::Cpu).unwrap();
        cfg.target_triple = "wrong-triple".into();
        let errs = compile_check(&cfg).unwrap_err();
        assert!(errs.iter().any(|e| matches!(e, CompileCheckError::TripleMismatch { .. })));
    }
    ...
    fn missing_feature_fails_check() {
        ...
        cfg.features.clear();
        let errs = compile_check(&cfg).unwrap_err();
        assert!(errs.iter().any(|e| matches!(e, CompileCheckError::MissingFeature(_))));
    }

    #[test]
    fn gpu_missing_umbrella_fails() {
        let mut cfg = BuildConfig::generate(TargetPlatform::X86_64Linux, GpuBackend::Cuda).unwrap();
    ...
    fn unavailable_backend_fails_check() {
        ...
        let errs = compile_check(&cfg).unwrap_err();
        assert!(errs.iter().any(|e| matches!(e, CompileCheckError::UnavailableBackend { .. })));
    }
    ...
    fn resolve_cpu_features() {
        let r = resolve_features(TargetPlatform::X86_64Linux, GpuBackend::Cpu).unwrap();
    ...
    fn resolve_cuda_features() {
        let r = resolve_features(TargetPlatform::X86_64Linux, GpuBackend::Cuda).unwrap();
    ...
    fn resolve_unavailable_returns_none() {
        assert!(resolve_features(TargetPlatform::Wasm32, GpuBackend::Cuda,).is_none());
    }

    #[test]
    fn resolve_metal_on_macos() {
        let r = resolve_features(TargetPlatform::Aarch64MacOS, GpuBackend::Metal).unwrap();
    ...
    fn resolve_aarch64_cpu_includes_neon() {
        let r = resolve_features(TargetPlatform::Aarch64Linux, GpuBackend::Cpu).unwrap();
    ...
    fn cuda_optional_on_x86_macos_egpu() {
        ...
        assert_eq!(
            backend_availability(TargetPlatform::X86_64MacOS, GpuBackend::Cuda,),
            Availability::Optional,
        );
    }
    ...
    fn vulkan_optional_on_all_non_wasm() {
        for &p in TargetPlatform::ALL {
            ...
            assert_eq!(backend_availability(p, GpuBackend::Vulkan), Availability::Optional,);
        }
    }
    ...
    fn vulkan_linker_flags() {
        let cfg = BuildConfig::generate(TargetPlatform::X86_64Linux, GpuBackend::Vulkan).unwrap();
