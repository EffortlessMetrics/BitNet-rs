...
// ── Local helper types & functions (not part of the crate public API) ─────
...
impl EmbeddingTable {
    fn new(vocab: u32, dim: usize, init: f32) -> Self {
        Self { vocab, dim, init }
    }
...
fn top_k(logits: &mut [f32], k: usize) {
    ...
    indices
        .sort_by(|&a, &b| logits[b].partial_cmp(&logits[a]).unwrap_or(std::cmp::Ordering::Equal));
    for &i in &indices[k..] {
...
fn compression_ratio(n: usize) -> f32 {
    ...
    // Ternary = ~1.585 bits/value vs 32-bit float
    let original_bits = n as f32 * 32.0;
    let compressed_bits = n as f32 * 1.585;
...
fn attention_forward(
    ...
    if q.len() != n || v.len() != n {
        return Err(HalError::InvalidArgument { index: 0, reason: "shape mismatch".into() });
    }
    Ok(vec![0.0f32; n])
}
...
fn build_rope_tables(
    ...
    let half = dim / 2;
    let total = seq_len * half;
    let mut cos_t = Vec::with_capacity(total);
    let mut sin_t = Vec::with_capacity(total);
...
fn apply_rope(x: &[f32], cos: &[f32], sin: &[f32]) -> Vec<f32> {
    let half = cos.len();
    ...
    for i in 0..half {
        ...
        out[i + half] = x0 * sin[i] + x1 * cos[i];
    }
    out
}

fn apply_rope_inverse(x: &[f32], cos: &[f32], sin: &[f32]) -> Vec<f32> {
    let half = cos.len();
    let mut out = x.to_vec();
    for i in 0..half {
        let x0 = x[i];
...
fn ffn_forward(x: &[f32]) -> Vec<f32> {
    // SiLU-gated identity: output same shape as input
