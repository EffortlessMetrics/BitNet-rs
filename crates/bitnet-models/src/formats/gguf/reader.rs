...
impl<'a> GgufReader<'a> {
    ...
    pub fn get_tensor_data_by_name(&self, name: &str) -> Result<&[u8]> {
        let info = self.get_tensor_info_by_name(name)
    ...
    pub fn get_quantization_type(&self) -> Option<QuantizationType> {
        ...
        for tensor_info in &self.tensor_infos {
            match tensor_info.tensor_type {
                ...
                GgufTensorType::Q6_K | GgufTensorType::Q8_K => has_tl2 = true,
                _ => continue,
            }
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for tensor_info in &self.tensor_infos {
            if !tensor_names.insert(&tensor_info.name) {
                return Err(BitNetError::Validation(
                    format!("Duplicate tensor name: {}", tensor_info.name)
                ));
            }
        }
        ...
        for tensor_info in &self.tensor_infos {
            if tensor_info.shape.is_empty() {
                return Err(BitNetError::Validation(
                    format!("Tensor '{}' has empty shape", tensor_info.name)
                ));
            }
            
            if tensor_info.shape.iter().any(|&dim| dim == 0) {
                return Err(BitNetError::Validation(
                    format!("Tensor '{}' has zero dimension", tensor_info.name)
                ));
            }
        }
