...
impl GgufLoader {
    ...
    fn load_tensors(
        ...
        for i in 0..tensor_count {
            ...
            debug!("Loading tensor '{}' with shape {:?} and type {:?}", 
    ...
    fn validate_tensor_data(&self, info: &crate::formats::gguf::TensorInfo, data: &[u8]) -> Result<()> {
        // Check data size matches expected size
        let expected_size = info.size as usize;
        if data.len() != expected_size {
            return Err(BitNetError::Validation(
                format!("Tensor '{}' data size mismatch: expected {}, got {}", 
                        info.name, expected_size, data.len())
            ));
        }
        ...
        if info.tensor_type.is_quantized() {
            ...
            if total_elements % block_size != 0 {
                return Err(BitNetError::Validation(
                    format!("Tensor '{}' elements ({}) not aligned to block size ({})", 
