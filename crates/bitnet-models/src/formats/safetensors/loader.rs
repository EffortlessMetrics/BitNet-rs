...
impl FormatLoader for SafeTensorsLoader {
    ...
    fn detect_format(&self, path: &Path) -> Result<bool> {
        ...
        let header_len = u64::from_le_bytes([
            header_len_bytes[0], header_len_bytes[1], header_len_bytes[2], header_len_bytes[3],
            header_len_bytes[4], header_len_bytes[5], header_len_bytes[6], header_len_bytes[7],
        ]);
        {
    ...
    fn extract_metadata(&self, path: &Path) -> Result<ModelMetadata> {
        ...
        let mmap = MmapFile::open(path)?;
        let safetensors = SafeTensors::deserialize(mmap.as_slice())
            .map_err(|e| BitNetError::Model(ModelError::InvalidFormat {
        ...
        let header_metadata: std::collections::HashMap<String, String> = std::collections::HashMap::new();
        
        let name = header_metadata.get("name")
        ...
        let version = header_metadata.get("version")
            .or_else(|| header_metadata.get("model_version"))
            .map(|s| s.to_string())
            .unwrap_or_else(|| "unknown".to_string());
        
        let architecture = header_metadata.get("architecture")
...
impl SafeTensorsLoader {
    fn extract_config(&self, safetensors: &SafeTensors) -> Result<BitNetConfig> {
        ...
        if config.model.vocab_size == BitNetConfig::default().model.vocab_size {
            let (vocab_size, context_length) = self.infer_model_dimensions(safetensors);
            config.model.vocab_size = vocab_size;
            if config.model.max_position_embeddings == BitNetConfig::default().model.max_position_embeddings {
    ...
    fn load_tensors(
        ...
        for (i, tensor_name) in tensor_names.iter().enumerate() {
            ...
            debug!("Loading tensor '{}' with shape {:?} and dtype {:?}", 
                   tensor_name, tensor_view.shape(), tensor_view.dtype());
    ...
    fn infer_model_dimensions(&self, safetensors: &SafeTensors) -> (usize, usize) {
        ...
        for tensor_name in safetensors.names() {
            if let Ok(tensor_view) = safetensors.tensor(tensor_name) {
                ...
                if tensor_name.contains("embed") || tensor_name.contains("lm_head") || tensor_name.contains("output") {
                    if shape.len() >= 2 {
