...
mod tests {
    ...
    #[test]
    fn create_causal_mask_masks_future_positions() -> Result<()> {
        ...
        let attn = MultiHeadAttention::new(&config, vb)?;

        // Simulate two past tokens by setting k_len > q_len
        let mask = attn.create_causal_mask(4, 6, &device)?;
        let mask_vals = mask.to_vec2::<f32>()?;
        let past_len = 6 - 4;

        for i in 0..4 {
            for j in 0..6 {
                if j > past_len + i {
                    assert!(mask_vals[i][j].is_infinite() && mask_vals[i][j].is_sign_negative());
                } else {
                    assert_eq!(mask_vals[i][j], 0.0);
                }
            }
        }
