...
pub enum SafeTensorsGpuError {
    ...
    UnsupportedConversion { from: TensorDtype, to: TensorDtype },
}
...
pub fn parse_header<R: Read + Seek>(
    reader: &mut R,
) -> Result<SafeTensorsHeader, SafeTensorsGpuError> {
    // Read header length (first 8 bytes, LE u64).
    let mut len_buf = [0u8; 8];
    reader.read_exact(&mut len_buf).map_err(|e| {
        SafeTensorsGpuError::InvalidHeader(format!("cannot read header length: {e}"))
    })?;
    ...
    let header_json: serde_json::Value = serde_json::from_slice(&header_buf)
        .map_err(|e| SafeTensorsGpuError::InvalidHeader(format!("JSON parse error: {e}")))?;

    let obj = header_json
        .as_object()
        .ok_or_else(|| SafeTensorsGpuError::InvalidHeader("header is not a JSON object".into()))?;
    ...
    for (key, value) in obj {
        ...
        let dtype_str = tensor_obj.get("dtype").and_then(|v| v.as_str()).ok_or_else(|| {
        ...
        let offsets =
            tensor_obj.get("data_offsets").and_then(|v| v.as_array()).ok_or_else(|| {
    ...
    Ok(SafeTensorsHeader { header_size, tensors, data_offset })
}
...
impl<G: GpuTransfer> SafeTensorsGpuLoader<G> {
    ...
    pub fn new(gpu: Arc<G>) -> Self {
        Self { gpu, target_dtype: None, progress_cb: None }
    }
    ...
    fn try_mmap_load(
        ...
        let mmap = unsafe {
            memmap2::MmapOptions::new()
                .map(&file)
                .map_err(|e| SafeTensorsGpuError::MmapUnavailable(format!("mmap failed: {e}")))?
        };
    ...
    fn maybe_convert(
        ...
        if let Some(target) = self.target_dtype {
            if target != info.dtype {
                return self.gpu.convert_dtype(buf, info.dtype, target, info.num_elements());
            }
        }
        Ok(buf.clone())
    }
...
mod tests {
    ...
    fn test_load_file_host_staged_fallback() {
        let st_data =
            build_safetensors(&[("w1", TensorDtype::F32, &[2, 2]), ("w2", TensorDtype::F16, &[4])]);
    ...
    fn test_progress_callback_invoked() {
        let st_data =
            build_safetensors(&[("a", TensorDtype::F32, &[2]), ("b", TensorDtype::F32, &[3])]);
        ...
        let gpu = Arc::new(MockGpu::new(false, 1 << 30));
        let loader = SafeTensorsGpuLoader::new(gpu).with_progress(Arc::new(move |p| {
