...
fn test_i2s_quantization_deterministic(
    ...
    // Note: I2S quantization is mathematically deterministic by design,
    // but we set these for consistency with BitNet.rs determinism patterns
    unsafe {
        std::env::set_var("BITNET_DETERMINISTIC", "1");
        std::env::set_var("BITNET_SEED", seed.to_string());
    }
    ...
    // Extract dequantized data as Vec<f32>
    let dequantized_data = bitnet_quantization::utils::extract_f32_data(&dequantized)?;
...
fn test_tl2_extreme_value_handling(weight_data: &[f32], shape: &[usize]) -> Result<(f32, bool)> {
    ...
    // Quantize using TL2 (8-bit table lookup quantization)
    ...
    // Check if overflow is handled (all outputs are finite)
    let overflow_handled = dequantized_data.iter().all(|x| x.is_finite());

    // Calculate MSE and signal power for accuracy
...
fn test_tl2_block_size_effects(
    ...
    use bitnet_quantization::tl2::{TL2Config, TL2Quantizer};
...
fn test_distribution_preservation(
    ...
    // 1. Filter non-finite values before processing
    ...
    let original_mean = calculate_mean(&finite_values);
    let original_variance = calculate_variance(&finite_values);
    ...
    // 5. Filter non-finite values from dequantized data
    let finite_dequantized: Vec<f32> =
        dequantized_data.iter().copied().filter(|x| x.is_finite()).collect();

    if finite_dequantized.is_empty() {
        // All dequantized values are non-finite
        return Ok((false, false, 0.0));
    }

    // 6. Calculate dequantized statistics
    let dequantized_mean = calculate_mean(&finite_dequantized);
    let dequantized_variance = calculate_variance(&finite_dequantized);

    // 7. Calculate correlation between original and dequantized
    let correlation = calculate_correlation(weight_data, &dequantized_data);

    // 8. Check if mean preserved (within 10% relative error)
    let mean_error = if original_mean.abs() > 1e-10 {
        ((original_mean - dequantized_mean) / original_mean).abs()
    } else {
        // For near-zero means, use absolute difference
        (original_mean - dequantized_mean).abs()
    };
    let mean_preserved = mean_error < 0.10; // 10% relative tolerance

    // 9. Check if variance preserved (within 20% relative error)
    let variance_error = if original_variance > 1e-10 {
        ((original_variance - dequantized_variance) / original_variance).abs()
    } else {
        // For near-zero variance, use absolute difference
        (original_variance - dequantized_variance).abs()
    };
    let variance_preserved = variance_error < 0.20; // 20% relative tolerance

    // 10. Return (mean_preserved, variance_preserved, correlation)
    Ok((mean_preserved, variance_preserved, correlation))
}
