...
use bitnet_ffi::{
    bitnet_init, bitnet_cleanup, bitnet_version, bitnet_abi_version,
    bitnet_model_load, bitnet_model_free, bitnet_model_is_loaded, bitnet_model_get_info,
    bitnet_inference, bitnet_inference_with_config, bitnet_get_last_error, bitnet_clear_last_error,
    bitnet_set_num_threads, bitnet_get_num_threads, bitnet_set_gpu_enabled, bitnet_is_gpu_available,
    bitnet_batch_inference, bitnet_start_streaming, bitnet_stop_streaming, bitnet_stream_next_token,
    bitnet_get_performance_metrics, bitnet_reset_performance_metrics,
    bitnet_set_memory_limit, bitnet_get_memory_usage, bitnet_garbage_collect,
    BitNetCModel, BitNetCInferenceConfig, BitNetCPerformanceMetrics, BitNetCStreamConfig,
    BITNET_SUCCESS, BITNET_ERROR_INVALID_ARGUMENT, BITNET_ERROR_MODEL_NOT_FOUND,
    BITNET_ERROR_MODEL_LOAD_FAILED, BITNET_ERROR_INFERENCE_FAILED, BITNET_ERROR_OUT_OF_MEMORY,
    BITNET_ERROR_THREAD_SAFETY, BITNET_ERROR_INVALID_MODEL_ID, BITNET_ERROR_CONTEXT_LENGTH_EXCEEDED,
    BITNET_ERROR_UNSUPPORTED_OPERATION, BITNET_ERROR_INTERNAL, BITNET_ABI_VERSION
};
...
fn test_library_initialization() {
    ...
    // Test cleanup
    let result = unsafe { bitnet_cleanup() };
    assert_eq!(result, BITNET_SUCCESS);
...
fn test_version_and_abi() {
    // Test version string
    let version_ptr = unsafe { bitnet_version() };
    ...
    // Test ABI version
    let abi_version = unsafe { bitnet_abi_version() };
...
fn test_memory_management() {
    unsafe {
        ...
        // Usage should be non-negative (might be 0 initially)
        assert!(usage >= 0);
...
fn test_batch_inference_invalid_cases() {
    unsafe {
        ...
        let output_ptrs: Vec<*mut c_char> = outputs.iter_mut().map(|v| v.as_mut_ptr() as *mut c_char).collect();
...
fn test_streaming_invalid_cases() {
    unsafe {
        ...
        // Test invalid model ID
        let result = bitnet_start_streaming(
            -1,
            prompt.as_ptr(),
            &config,
            &stream_config,
        ...
        let result = bitnet_stream_next_token(
            -1,
...
fn test_performance_characteristics() {
    unsafe {
        ...
        assert!(elapsed < Duration::from_millis(100), 
                "Basic operations took too long: {:?}", elapsed);
