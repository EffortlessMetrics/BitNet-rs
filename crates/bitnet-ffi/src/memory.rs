...
impl TrackingAllocator {
    ...
    pub fn get_stats(&self) -> Result<MemoryStats, BitNetCError> {
        self.stats.lock().map(|stats| stats.clone()).map_err(|_| {
            BitNetCError::ThreadSafety("Failed to acquire memory stats lock".to_string())
        })
    }
...
impl MemoryPool {
    ...
    pub fn deallocate(&self, ptr: *mut u8, size: usize) -> Result<(), BitNetCError> {
        ...
        let layout = Layout::from_size_align(size, std::mem::align_of::<u8>()).map_err(|_| {
            BitNetCError::Internal("Invalid memory layout for deallocation".to_string())
        })?;
...
impl MemoryManager {
    ...
    pub fn set_memory_limit(&self, limit: Option<usize>) -> Result<(), BitNetCError> {
        let mut memory_limit = self.memory_limit.write().map_err(|_| {
            BitNetCError::ThreadSafety("Failed to acquire memory limit write lock".to_string())
        })?;
        *memory_limit = limit;
        Ok(())
    }
    ...
    pub fn get_memory_limit(&self) -> Result<Option<usize>, BitNetCError> {
        let memory_limit = self.memory_limit.read().map_err(|_| {
            BitNetCError::ThreadSafety("Failed to acquire memory limit read lock".to_string())
        })?;
        Ok(*memory_limit)
    }
    ...
    pub fn allocate(&self, size: usize) -> Result<*mut u8, BitNetCError> {
        ...
        if !self.check_memory_limit(size)? {
            return Err(BitNetCError::OutOfMemory(format!(
                "Allocation of {} bytes would exceed memory limit",
                size
            )));
        }
    ...
    pub fn set_tracking_enabled(&self, enabled: bool) -> Result<(), BitNetCError> {
        let mut tracking_enabled = self.tracking_enabled.write().map_err(|_| {
            BitNetCError::ThreadSafety("Failed to acquire tracking enabled write lock".to_string())
        })?;
