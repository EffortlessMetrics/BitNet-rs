...
use bitnet_common::{QuantizationType, BitNetError, QuantizationError, MockTensor, ConcreteTensor, DType, Device};
...
mod error_handling {
    ...
    fn test_extreme_values() {
        ...
        let small_values = vec![f32::MIN_POSITIVE, f32::MIN_POSITIVE * 2.0, f32::MIN_POSITIVE * 4.0, f32::MIN_POSITIVE * 8.0];
...
mod algorithm_comprehensive {
    ...
    fn test_tl1_comprehensive() {
        ...
        for block_size in block_sizes {
            ...
            let data: Vec<f32> = (0..block_size * 4).map(|i| (i as f32 - block_size as f32 * 2.0) / 10.0).collect();
            ...
            assert!(max_error < 0.5, "Max error too high for block size {}: {}", block_size, max_error);
        }
    }
    ...
    fn test_quantization_compression_ratios() {
        ...
        println!("Compression ratios - I2S: {:.2}x, TL1: {:.2}x, TL2: {:.2}x", 
...
mod performance_tests {
    ...
    fn test_memory_usage() {
        ...
        let quantized_bytes = quantized.data.len() + quantized.scales.len() * std::mem::size_of::<f32>();
        
        println!("Original: {} bytes, Quantized: {} bytes, Ratio: {:.2}x", 
                 original_bytes, quantized_bytes, original_bytes as f32 / quantized_bytes as f32);
...
mod integration_tests {
    ...
    fn test_full_quantization_pipeline() {
        ...
        let methods = vec![
            ("I2S", Box::new(I2SQuantizer::new().unwrap()) as Box<dyn Quantize>),
            ("TL1", Box::new(TL1Quantizer::new().unwrap()) as Box<dyn Quantize>),
            ("TL2", Box::new(TL2Quantizer::new().unwrap()) as Box<dyn Quantize>),
        ];
        ...
        for (name, quantizer) in methods {
            ...
            // Calculate metrics
            let mse: f32 = model_weights.iter()
                .zip(dequantized.iter())
                .map(|(orig, deq)| (orig - deq).powi(2))
                .sum::<f32>() / model_weights.len() as f32;
            
            let max_error = model_weights.iter()
                .zip(dequantized.iter())
                .map(|(orig, deq)| (orig - deq).abs())
                .fold(0.0f32, f32::max);
    ...
    fn test_cross_algorithm_compatibility() {
        ...
        // Compare accuracy
        let i2s_mse: f32 = data.iter().zip(i2s_deq.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f32>() / data.len() as f32;
        let tl1_mse: f32 = data.iter().zip(tl1_deq.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f32>() / data.len() as f32;
        let tl2_mse: f32 = data.iter().zip(tl2_deq.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f32>() / data.len() as f32;
        
        println!("Cross-algorithm MSE comparison - I2S: {:.6}, TL1: {:.6}, TL2: {:.6}", i2s_mse, tl1_mse, tl2_mse);
