...
use bitnet_common::{BitNetTensor, Device as BDevice, Tensor};
...
fn calculate_relative_error(original: &[f32], reconstructed: &[f32]) -> f32 {
    ...
    let original_variance: f32 =
        original.iter().map(|x| x.powi(2)).sum::<f32>() / original.len() as f32;

    if original_variance > 0.0 { (mse / original_variance).sqrt() * 100.0 } else { 0.0 }
}
...
fn test_i2s_accuracy() {
    ...
    for (i, pattern) in patterns.iter().enumerate() {
        let candle_tensor =
            CandleTensor::from_vec(pattern.clone(), &[pattern.len()], &Device::Cpu).unwrap();
        let tensor = BitNetTensor::new(candle_tensor);

        // Quantize and dequantize
        let quantized = quantizer.quantize_tensor(&tensor).unwrap();
        let reconstructed = quantizer.dequantize_tensor(&quantized, &BDevice::Cpu).unwrap();

        // Extract data
        let reconstructed_candle = reconstructed.to_candle().unwrap();
        let reconstructed_data =
            reconstructed_candle.flatten_all().unwrap().to_vec1::<f32>().unwrap();
    ...
    for (name, quantizer) in quantizers.iter() {
        ...
        let results: Vec<_> = (0..5)
            .map(|_| {
                ...
                let d = quantizer.dequantize_tensor(&q, &BDevice::Cpu).unwrap();
...
fn test_edge_cases() {
    ...
    for (name, quantizer) in quantizers.iter() {
        for (i, case) in edge_cases.iter().enumerate() {
            let candle_tensor =
                CandleTensor::from_vec(case.clone(), &[case.len()], &Device::Cpu).unwrap();
            let tensor = BitNetTensor::new(candle_tensor);

            // Should not panic
            let quantized = quantizer.quantize_tensor(&tensor).unwrap();
            let _reconstructed = quantizer.dequantize_tensor(&quantized, &BDevice::Cpu).unwrap();
