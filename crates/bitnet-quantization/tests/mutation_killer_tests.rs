...
mod i2s_arithmetic_mutation_killers {
    ...
    fn test_kill_i2s_multiplication_mutation_line_43() {
        ...
        for (block_size, expected_bytes) in test_cases {
            ...
            assert_ne!(layout.data_bytes_per_block, wrong_add_result,
                "Addition mutation detected for block_size={}", block_size);

            // Kill * -> - mutation: (block_size - 2).div_ceil(8) would give wrong result
            if block_size >= 2 {
                let wrong_sub_result = (block_size - 2).div_ceil(8);
                assert_ne!(layout.data_bytes_per_block, wrong_sub_result,
                    "Subtraction mutation detected for block_size={}", block_size);
            }

            // Kill * -> / mutation: (block_size / 2).div_ceil(8) would give wrong result
            let wrong_div_result = (block_size / 2).div_ceil(8);
            assert_ne!(layout.data_bytes_per_block, wrong_div_result,
                "Division mutation detected for block_size={}", block_size);

            // Verify correct calculation
            assert_eq!(layout.data_bytes_per_block, expected_bytes,
                "Incorrect data_bytes calculation for block_size={}", block_size);
        }
    }
    ...
    fn test_kill_i2s_addition_mutation_line_46() {
        ...
        for (block_size, data_bytes, expected_total) in test_cases {
            ...
            assert_ne!(layout.bytes_per_block, wrong_sub_result,
                "Subtraction mutation detected for block_size={}", block_size);

            // Kill + -> * mutation: data_bytes * 2 would be wrong
            let wrong_mul_result = data_bytes * 2;
            assert_ne!(layout.bytes_per_block, wrong_mul_result,
                "Multiplication mutation detected for block_size={}", block_size);

            // Kill + -> / mutation: data_bytes / 2 would be wrong
            let wrong_div_result = data_bytes / 2;
            assert_ne!(layout.bytes_per_block, wrong_div_result,
                "Division mutation detected for block_size={}", block_size);

            // Verify correct calculation
            assert_eq!(layout.bytes_per_block, expected_total,
                "Incorrect bytes_per_block calculation for block_size={}", block_size);

            assert_eq!(layout.data_bytes_per_block, data_bytes,
                "data_bytes_per_block mismatch for block_size={}", block_size);

            assert_eq!(layout.scale_bytes_per_block, 2,
                "scale_bytes_per_block should always be 2");
        }
    }
    ...
    fn test_i2s_layout_bit_manipulation_edge_cases() {
        ...
        for block_size in edge_cases {
            ...
            assert_eq!(layout.bytes_per_block, expected_bytes + 2,
                "Incorrect total bytes for block_size={}: expected {}, got {}",
                block_size, expected_bytes + 2, layout.bytes_per_block);

            // Verify that we need exactly ceil((block_size * 2) / 8) bytes
            let manual_calc = (block_size * 2 + 7) / 8; // Manual div_ceil
            assert_eq!(layout.data_bytes_per_block, manual_calc,
                "Manual calculation mismatch for block_size={}", block_size);
        }
    }
    ...
    fn test_i2s_layout_systematic_verification() {
        ...
        for (block_size, expected_data_bytes) in test_cases {
            ...
            assert!(layout.bytes_per_block > layout.data_bytes_per_block,
                "Total bytes must be greater than data bytes");
            assert!(layout.bytes_per_block >= 2,
                "Total bytes must account for scale");
            assert!(layout.data_bytes_per_block > 0 || block_size == 0,
...
mod device_aware_comparison_mutation_killers {
    ...
    fn test_kill_length_comparison_mutation_line_147() {
        ...
        let mut report = AccuracyReport::new(QuantizationType::I2S, bitnet_common::Device::Cpu, 1e-5);
        ...
        for (original, quantized) in test_cases {
            ...
            if original.len() != quantized.len() {
                ...
                let max_errors_equal = (report.max_absolute_error.is_nan() && initial_max_error.is_nan()) ||
                                       report.max_absolute_error == initial_max_error;
                let mean_errors_equal = (report.mean_absolute_error.is_nan() && initial_mean_error.is_nan()) ||
                                        report.mean_absolute_error == initial_mean_error;

                assert!(max_errors_equal,
                    "Max error changed when lengths differ - possible == mutation");
                assert!(mean_errors_equal,
    ...
    fn test_kill_arithmetic_mutation_line_156() {
        ...
        for (original, quantized) in test_cases {
            ...
            if (expected_abs_error - wrong_add_error).abs() > 1e-6 {
                assert!((report.max_absolute_error - wrong_add_error as f64).abs() > 1e-6,
                    "Addition mutation detected for orig={}, quant={}", orig_val, quant_val);
            }

            // Kill * mutation: (orig * quant).abs() would be wrong
            let wrong_mul_error = (orig_val * quant_val).abs();
            if (expected_abs_error - wrong_mul_error).abs() > 1e-6 {
                assert!((report.max_absolute_error - wrong_mul_error as f64).abs() > 1e-6,
                    "Multiplication mutation detected for orig={}, quant={}", orig_val, quant_val);
            }
            ...
            if quant_val.abs() > 1e-10 {
                ...
                if (expected_abs_error - wrong_div_error).abs() > 1e-6 {
                    assert!((report.max_absolute_error - wrong_div_error as f64).abs() > 1e-6,
                        "Division mutation detected for orig={}, quant={}", orig_val, quant_val);
                }
            }

            // Verify correct calculation
            assert!((report.max_absolute_error - expected_abs_error as f64).abs() < 1e-10,
                "Incorrect abs_error calculation for orig={}, quant={}", orig_val, quant_val);
        }
    }
    ...
    fn test_kill_threshold_comparison_mutation_line_159() {
        ...
        for (original, quantized) in test_cases {
            ...
            if should_calc_relative {
                ...
                assert!(report.relative_error != initial_relative_error ||
                       (report.relative_error == 0.0 && quantized[0] == orig_val),
                       "Relative error should be updated for orig.abs()={} > 1e-10", orig_val.abs());
            } else {
    ...
    fn test_kill_division_mutation_line_160() {
        ...
        for (original, quantized) in test_cases {
            ...
            if (expected_rel_error - wrong_sub_result).abs() > 1e-6 {
                assert!((report.relative_error - wrong_sub_result as f64).abs() > 1e-6,
                    "Subtraction mutation detected for abs_error={}, orig.abs()={}",
                    abs_error, orig_val.abs());
            }

            // Verify correct calculation within tolerance
            assert!((report.relative_error - expected_rel_error as f64).abs() < 1e-10,
                "Incorrect relative error calculation: expected {}, got {}",
                expected_rel_error, report.relative_error);
        }
    }
    ...
    fn test_comprehensive_comparison_mutations() {
        ...
        let tolerance_configs = vec![
            ToleranceConfig { i2s_tolerance: 1e-5, tl_tolerance: 1e-4, perplexity_tolerance: 0.001, strict_validation: true },
            ToleranceConfig { i2s_tolerance: 1e-6, tl_tolerance: 1e-5, perplexity_tolerance: 0.0001, strict_validation: false },
        ];

        for config in tolerance_configs {
            let mut report = AccuracyReport::new(QuantizationType::I2S, bitnet_common::Device::Cpu, config.i2s_tolerance);
            ...
            for (original, quantized) in test_data {
                ...
                assert_eq!(report.passed, within_tolerance,
                ...
                if report.relative_error == report.tolerance {
                    ...
                    assert_ne!(within_tolerance, wrong_less_than,
                        "At exact tolerance, <= and < should differ");
                    assert_ne!(within_tolerance, wrong_greater_than,
                        "At exact tolerance, <= and > should differ");
                }
            }
        }
    ...
    fn test_i2s_gpu_cpu_parity() {
        ...
        for data in test_data {
            ...
            if bitnet_kernels::gpu::cuda::is_cuda_available() {
                ...
                for (i, (&cpu_val, &gpu_val)) in cpu_recovered.iter().zip(gpu_recovered.iter()).enumerate() {
                    let abs_diff = (cpu_val - gpu_val).abs();
                    let rel_diff = if cpu_val.abs() > 1e-10 {
                        abs_diff / cpu_val.abs()
                    } else {
                        abs_diff
                    };

                    assert!(rel_diff < parity_tolerance,
                        "GPU/CPU parity violation at index {}: CPU={}, GPU={}, rel_diff={}",
                        i, cpu_val, gpu_val, rel_diff);
                }
...
mod quantization_boundary_tests {
        ...
        for quantizer in quantizers {
            for (case_idx, extreme_values) in extreme_cases.iter().enumerate() {
                ...
                if let Ok(quantized) = quantize_result {
                    ...
                    assert!(!quantized.data.is_empty() || extreme_values.iter().all(|x| !x.is_finite()),
                        "Quantized data empty for case {} with finite inputs", case_idx);

                    // Scales should be finite and positive
                    for (i, &scale) in quantized.scales.iter().enumerate() {
                        if scale.is_finite() {
                            assert!(scale > 0.0, "Scale {} should be positive at index {}", scale, i);
                        }
                    }
                    ...
                    if let Ok(dequantized) = quantizer.dequantize_tensor(&quantized) {
                        ...
                        for (orig, rec) in extreme_values.iter().zip(recovered.iter()) {
                            if orig.is_finite() && orig.abs() < 1e10 {
                                // Only check for reasonable finite values
                                assert!(rec.is_finite() || orig.abs() < 1e-10,
        ...
        for (case_idx, values) in special_values.iter().enumerate() {
            ...
            match result {
                Ok(quantized) => {
                    ...
                    assert!(deq_result.is_ok(),
                        "Dequantization failed for case {} after successful quantization", case_idx);
    ...
    fn test_zero_and_near_zero_handling() {
        ...
        for (name, quantizer) in quantizers {
            for (case_idx, values) in near_zero_cases.iter().enumerate() {
                ...
                let quantized = quantizer.quantize_tensor(&tensor)
                    .expect(&format!("{} quantization failed for near-zero case {}", name, case_idx));

                // Should produce valid quantized representation
                assert!(!quantized.data.is_empty() || values.iter().all(|&x| x == 0.0),
                    "{} produced empty data for non-zero case {}", name, case_idx);

                // Scales should handle near-zero values appropriately
                for &scale in &quantized.scales {
                    assert!(scale.is_finite() && scale >= 0.0,
                        "{} produced invalid scale {} for case {}", name, scale, case_idx);
                }

                let dequantized = quantizer.dequantize_tensor(&quantized)
                    .expect(&format!("{} dequantization failed for case {}", name, case_idx));
                ...
                if values.iter().all(|&x| x.abs() < 1e-20) {
                    ...
                    assert!(max_recovered < 10.0,
                        "{} failed to preserve near-zero property: max_recovered={}", name, max_recovered);
                }
            }
        }
    }
    ...
    fn test_block_size_boundary_conditions() {
        ...
        for (data, expected_len) in test_cases {
            ...
            let quantized = quantizer.quantize_tensor(&tensor)
                .expect(&format!("Quantization failed for data_len={}, block_size={}", expected_len, block_size));

            // Verify data integrity
            assert_eq!(data.len(), expected_len, "Test data length mismatch");

            let dequantized = quantizer.dequantize_tensor(&quantized)
                .expect(&format!("Dequantization failed for data_len={}, block_size={}", expected_len, block_size));

            let recovered = dequantized.to_vec().unwrap();
            assert_eq!(recovered.len(), expected_len,
                "Length mismatch after round-trip: expected {}, got {}", expected_len, recovered.len());
            ...
            if data.len() % block_size == 0 {
                ...
                assert_eq!(num_blocks, data.len() / block_size,
                    "div_ceil mutation detected: {} vs {} for data_len={}, block_size={}",
                    num_blocks, data.len() / block_size, data.len(), block_size);
            } else {
                // Non-exact division - div_ceil should be one more than regular division
                assert_eq!(num_blocks, data.len() / block_size + 1,
                    "div_ceil mutation detected: {} vs {} for data_len={}, block_size={}",
                    num_blocks, data.len() / block_size + 1, data.len(), block_size);
            }

                // Verify scales count matches expected blocks
                assert_eq!(quantized.scales.len(), expected_scales,
                    "Scales count mismatch for data_len={}, block_size={}: expected {}, got {}",
                    data.len(), block_size, expected_scales, quantized.scales.len());
            }
        }
    }
}
...
mod mathematical_validation_tests {
        ...
        for (original, quantized) in test_cases {
            ...
            if noise_power > 1e-10 && signal_power > 1e-10 {
                ...
                if wrong_mul_ratio != signal_power / noise_power {
                    ...
                    assert!((snr_db - wrong_snr).abs() > 1.0,
                        "Multiplication mutation detected in SNR: correct={}, wrong={}", snr_db, wrong_snr);
                }

                // Kill * -> / mutation in 10 * log10(...)
                let wrong_div_coefficient = 10.0 / (signal_power / noise_power).log10();
                if wrong_div_coefficient != snr_db {
                    assert!((snr_db - wrong_div_coefficient).abs() > 1.0,
                        "Division mutation detected in SNR coefficient: correct={}, wrong={}", snr_db, wrong_div_coefficient);
                }

                // Kill + -> - mutation in power calculations
                let wrong_signal_power = original.iter().map(|x| x * x).sum::<f32>() - original.len() as f32;
                if wrong_signal_power > 1e-10 {
                    let wrong_snr = 10.0 * (wrong_signal_power / noise_power).log10();
                    assert!((snr_db - wrong_snr).abs() > 1.0,
                        "Subtraction mutation detected in signal power: correct={}, wrong={}", snr_db, wrong_snr);
                }

                // SNR should be reasonable for small quantization errors
                assert!(snr_db > 0.0, "SNR should be positive for small quantization errors: {}", snr_db);
    ...
    fn test_mse_calculation_arithmetic_mutations() {
        ...
        for (original, quantized) in test_cases {
            ...
            let squared_errors: Vec<f32> = original.iter()
                .zip(quantized.iter())
            ...
            let wrong_add_errors: Vec<f32> = original.iter()
                .zip(quantized.iter())
                .map(|(orig, quant)| (*orig as f32 + *quant as f32).powi(2))
                .collect();
            ...
            if (mse - wrong_add_mse).abs() > 1e-6 {
                assert!((mse - wrong_add_mse).abs() > 1e-6,
                    "Addition mutation detected in error calculation: correct MSE={}, wrong MSE={}", mse, wrong_add_mse);
            }

            // Kill / -> * mutation in sum / n
            let wrong_mul_mse = sum_squared_errors * n;
            if (mse - wrong_mul_mse).abs() > 1e-6 {
                assert!((mse - wrong_mul_mse).abs() > 1e-6,
                    "Multiplication mutation detected in MSE division: correct MSE={}, wrong MSE={}", mse, wrong_mul_mse);
            }

            // Kill powi(2) -> powi(1) mutation
            let wrong_power_errors: Vec<f32> = original.iter()
                .zip(quantized.iter())
            ...
            if (mse - wrong_power_mse).abs() > 1e-6 {
                assert!((mse - wrong_power_mse).abs() > 1e-6,
                    "Power mutation detected in squared error: correct MSE={}, wrong MSE={}", mse, wrong_power_mse);
            }
    ...
    fn test_standard_deviation_arithmetic_mutations() {
        ...
        for values in test_cases {
            ...
            let wrong_add_variance = values.iter()
                .map(|x| (x + mean).powi(2))
                .sum::<f32>() / (n - 1.0);
            let wrong_add_std = wrong_add_variance.sqrt();

            if (std_dev - wrong_add_std).abs() > 1e-6 && n > 1.0 {
                assert!((std_dev - wrong_add_std).abs() > 1e-6,
                    "Addition mutation detected in variance: correct std={}, wrong std={}", std_dev, wrong_add_std);
            }

            // Kill / -> * mutation in sum / (n - 1)
            let wrong_mul_variance = values.iter()
                .map(|x| (x - mean).powi(2))
                .sum::<f32>() * (n - 1.0);
            let wrong_mul_std = wrong_mul_variance.sqrt();

            if (std_dev - wrong_mul_std).abs() > 1e-6 && n > 1.0 {
                assert!((std_dev - wrong_mul_std).abs() > 1e-6,
                    "Multiplication mutation detected in variance: correct std={}, wrong std={}", std_dev, wrong_mul_std);
            }

            // Kill - -> + mutation in (n - 1)
            let wrong_denom_variance = values.iter()
                .map(|x| (x - mean).powi(2))
                .sum::<f32>() / (n + 1.0);
            let wrong_denom_std = wrong_denom_variance.sqrt();

            if (std_dev - wrong_denom_std).abs() > 1e-6 && n > 1.0 {
                assert!((std_dev - wrong_denom_std).abs() > 1e-6,
                    "Denominator mutation detected in variance: correct std={}, wrong std={}", std_dev, wrong_denom_std);
            }
            ...
            if values.iter().all(|&x| (x - values[0]).abs() < 1e-10) {
                assert!(std_dev < 1e-6, "Standard deviation should be near zero for constant values: {}", std_dev);
            }
        }
    }
    ...
    fn test_statistical_calculations_comprehensive() {
        ...
        for (dataset_idx, original_values) in datasets.iter().enumerate() {
            ...
            let quantized_values: Vec<f32> = original_values.iter()
                .enumerate()
            ...
            let orig_variance = original_values.iter()
                .map(|x| (x - orig_mean).powi(2))
                .sum::<f32>() / (n - 1.0);

            let quant_variance = quantized_values.iter()
                .map(|x| (x - quant_mean).powi(2))
                .sum::<f32>() / (n - 1.0);
            ...
            let quant_centered: Vec<f32> = quantized_values.iter().map(|x| x - quant_mean).collect();

            let covariance = orig_centered.iter()
            ...
            if correlation.abs() > 1e-10 && (correlation - wrong_mul_correlation).abs() > 1e-6 {
                assert!((correlation - wrong_mul_correlation).abs() > 1e-6,
                    "Multiplication mutation in correlation for dataset {}: correct={}, wrong={}",
                    dataset_idx, correlation, wrong_mul_correlation);
            }

            // Verify statistical properties
            assert!(correlation >= -1.0 && correlation <= 1.0,
                "Correlation should be in [-1, 1] for dataset {}: {}", dataset_idx, correlation);

            assert!(orig_variance >= 0.0 && quant_variance >= 0.0,
                "Variances should be non-negative for dataset {}: orig={}, quant={}",
                dataset_idx, orig_variance, quant_variance);
