...
pub fn gemv_qk256(
    ...
    // Allow deterministic scalar fallback in CI/debug via env override.
    if std::env::var("BITNET_FORCE_SCALAR").as_deref() == Ok("1") {
        return gemv_qk256_scalar(qs_data, x, y_out, rows, cols, row_stride_bytes);
    }
...
mod tests {
    ...
    fn qk256_stride_mismatch_panics() {
        ...
        // Now returns Err instead of panicking, thanks to row_stride_bytes validation
        let result = gemv_qk256(&qs_data, &x, &mut y_out, rows, cols, wrong_stride);
        assert!(result.is_err(), "Should error on mismatched row_stride_bytes");
    }
}
