...
pub enum PipelineError {
    ...
    #[error("validation failed: max error {max_error:.6} exceeds tolerance {tolerance:.6}")]
...
impl GpuQuantizationPipeline {
    ...
    pub fn quantize(
        ...
        let (max_error, mean_error) = if self.config.validate {
            let (max_e, mean_e) = self.compute_validation_errors(data, &quantized, quant_type);
    ...
    pub fn quantize_batch(
        ...
        let total_output_bytes: usize = results.iter().map(|r| r.data.len()).sum();
        let max_error = results
            .iter()
            .filter_map(|r| r.max_error)
            .fold(None, |acc, e| Some(acc.map_or(e, |a: f64| a.max(e))));
        let mean_error = {
            let errors: Vec<f64> = results.iter().filter_map(|r| r.mean_error).collect();
            if errors.is_empty() {
                None
            } else {
    ...
    fn validate_quant_type(qt: QuantizationType) -> Result<(), PipelineError> {
        // All current QuantizationType variants are supported.
        match qt {
            QuantizationType::I2S | QuantizationType::TL1 | QuantizationType::TL2 => Ok(()),
        }
    }
    ...
    fn cpu_reference_quantize(&self, data: &[f32], quant_type: QuantizationType) -> Vec<u8> {
        match quant_type {
            ...
            QuantizationType::TL1 | QuantizationType::TL2 => {
                ...
                data.iter().map(|&val| self.quantize_tl_scalar(val)).collect()
            }
        }
    }
    ...
    fn compute_validation_errors(
        ...
        let mean_err = if original.is_empty() { 0.0 } else { sum_err / original.len() as f64 };
        (max_err, mean_err)
    }
    ...
    fn dequantize_reference(
        ...
        match quant_type {
            ...
            QuantizationType::TL1 | QuantizationType::TL2 => {
                quantized.iter().take(element_count).map(|&b| (b as f64 / 127.5) - 1.0).collect()
            }
        }
    }
}
...
mod tests {
    ...
    fn pipeline_no_validate() -> GpuQuantizationPipeline {
        GpuQuantizationPipeline::new(PipelineConfig { validate: false, ..Default::default() })
    }
    ...
    fn empty_batch_returns_error() {
        let p = pipeline();
        let err = p.quantize_batch(&[], QuantizationType::I2S).unwrap_err();
    ...
    fn batch_too_large_returns_error() {
        ...
        let t2 = [2.0f32];
        let t3 = [3.0f32];
        let err = p.quantize_batch(&[&t1, &t2, &t3], QuantizationType::I2S).unwrap_err();
        assert_eq!(err, PipelineError::BatchTooLarge { actual: 3, max: 2 });
    }
    ...
    fn batch_quantize_returns_correct_count() {
        ...
        let t2 = [-0.1f32, -0.2];
        let (results, summary) = p.quantize_batch(&[&t1, &t2], QuantizationType::I2S).unwrap();
    ...
    fn pipeline_default_matches_with_defaults() {
        ...
        assert_eq!(p1.config().qk256_block_size, p2.config().qk256_block_size);
    }
