...
mod tests {
    ...
    fn property_quantization_idempotency() -> Result<()> {
        ...
        for (case_id, test_data) in test_cases.iter().enumerate() {
            ...
            match quantizer.quantize_tensor(&tensor) {
                Ok(quantized1) => {
                    match quantizer.dequantize_tensor(&quantized1) {
                        Ok(dequantized) => {
                            match quantizer.quantize_tensor(&dequantized) {
                                Ok(quantized2) => {
    ...
    fn property_continuity_approximation() -> Result<()> {
        ...
        for (base_idx, base_data) in base_values.iter().enumerate() {
            ...
            match quantizer.quantize_tensor(&base_tensor) {
                Ok(base_quantized) => {
                    for &perturbation in &perturbation_magnitudes {
                        ...
                        match quantizer.quantize_tensor(&perturbed_tensor) {
    ...
    fn property_scale_relationship() -> Result<()> {
        ...
        for &scale in &scales {
            ...
            match quantizer.quantize_tensor(&scaled_tensor) {
                Ok(quantized) => {
    ...
    fn property_error_bounds() -> Result<()> {
        ...
        for (quantizer_name, quantizer, max_expected_error) in &quantizers {
            ...
            for test_data in test_cases {
                ...
                match quantizer.quantize_tensor(&tensor) {
                    Ok(quantized) => match quantizer.dequantize_tensor(&quantized) {
                        Ok(dequantized) => {
    ...
    fn tensor_to_vec(tensor: &BitNetTensor) -> Result<Vec<f32>> {
        tensor.to_vec()
    }
