...
impl BenchmarkCommand {
    ...
    /// Load model and tokenizer
    async fn load_model_and_tokenizer(&self, config: &CliConfig) -> Result<(BitNetInferenceEngine, std::sync::Arc<dyn bitnet_tokenizers::Tokenizer>)> {
        ...
        let model = loader.load(&self.model)
    ...
    fn determine_device(&self, config: &CliConfig) -> Result<Device> {
        let device_str = self.device.as_ref()
            .unwrap_or(&config.default_device);
    ...
    /// Run all benchmarks
    async fn run_benchmarks(&self, _engine: &mut BitNetInferenceEngine) -> Result<BenchmarkResults> {
    ...
    async fn run_single_benchmark(&self, test_name: &str, batch_size: usize, seq_len: usize) -> Result<BenchmarkResult> {
    ...
    async fn run_single_iteration(&self, iteration: usize, batch_size: usize, seq_len: usize, is_warmup: bool) -> Result<IterationResult> {
        let start_time = Instant::now();
        
        // Simulate inference work
        let work_duration = Duration::from_millis(
            (50 + batch_size * 10 + seq_len / 10) as u64
        );
        ...
        if !is_warmup {
            debug!("Iteration {}: {:.2}ms, {:.2} tok/s", 
                iteration + 1, latency_ms, tokens_per_second);
        }
    ...
    fn calculate_statistics(&self, iterations: &[IterationResult]) -> Statistics {
        ...
        let peak_memory = iterations.iter()
    ...
    fn calculate_summary(&self, results: &[BenchmarkResult], total_duration: Duration) -> BenchmarkSummary {
        // Find best performance
        let best = results.iter()
            .max_by(|a, b| a.statistics.mean_tokens_per_second.partial_cmp(&b.statistics.mean_tokens_per_second).unwrap())
            .unwrap();
        ...
        if best.batch_size > 1 {
            recommendations.push(format!("Best performance achieved with batch size {}", best.batch_size));
        }
        
        if best.statistics.mean_tokens_per_second > 100.0 {
            recommendations.push("Good throughput achieved. Consider GPU acceleration for even better performance.".to_string());
        } else {
            recommendations.push("Consider optimizing model or using GPU acceleration for better performance.".to_string());
        }
    ...
    fn write_text_results(&self, mut output: Box<dyn Write>, results: &BenchmarkResults) -> Result<()> {
        ...
        for result in &results.results {
            ...
            writeln!(output, "    Mean Throughput: {:.2} tokens/sec", result.statistics.mean_tokens_per_second)?;
        ...
        writeln!(output, "  Best Performance: {} ({:.2} tokens/sec)", 
    ...
    fn write_csv_results(&self, mut output: Box<dyn Write>, results: &BenchmarkResults) -> Result<()> {
        ...
        for result in &results.results {
            writeln!(output, "{},{},{},{:.2},{:.2},{:.2},{:.2},{}", 
                ...
                result.statistics.peak_memory_mb.map(|m| format!("{:.2}", m)).unwrap_or_else(|| "".to_string())
            )?;
        }
