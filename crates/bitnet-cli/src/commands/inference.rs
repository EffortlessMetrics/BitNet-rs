...
use bitnet_inference::{BitNetInferenceEngine, InferenceConfig, SamplingConfig, InferenceEngine};
...
impl InferenceCommand {
    ...
    pub async fn execute(&self, config: &CliConfig) -> Result<()> {
        ...
        match (self.interactive, self.input_file.as_ref(), self.prompt.as_ref()) {
            ...
            (false, None, Some(prompt)) => self.run_single_inference(engine, tokenizer, prompt).await,
            (false, None, None) => {
    ...
    fn validate_args(&self) -> Result<()> {
        ...
        match self.format.as_str() {
            "text" | "json" | "jsonl" => {}
            _ => anyhow::bail!("Invalid format: {}. Must be one of: text, json, jsonl", self.format),
        }
    ...
    /// Load model and tokenizer
    async fn load_model_and_tokenizer(&self, config: &CliConfig) -> Result<(BitNetInferenceEngine, Arc<dyn bitnet_tokenizers::Tokenizer>)> {
        let model_path = self.model.as_ref()
    ...
    /// Load tokenizer
    async fn load_tokenizer(&self, model_path: &PathBuf) -> Result<Arc<dyn bitnet_tokenizers::Tokenizer>> {
        ...
        if let Some(tokenizer_path) = tokenizer_path {
            ...
            TokenizerBuilder::from_pretrained("gpt2")
                .context("Failed to load default tokenizer")
        }
    }
    ...
    async fn run_single_inference(
        ...
        if self.stream {
            ...
            let result = self.run_batch_inference(&mut engine, &[prompt.to_string()], &config).await?;
    ...
    async fn run_streaming_inference(
        ...
        // Placeholder implementation - would use actual streaming
        let result = "This is a placeholder response. Streaming inference not yet fully implemented.".to_string();
        ...
        match self.format.as_str() {
            ...
            _ => {
                ...
                for (i, result) in results.iter().enumerate() {
                    ...
                    if self.metrics {
                        writeln!(writer, "Tokens: {}, Time: {:.2}s, Speed: {:.2} tok/s", 
