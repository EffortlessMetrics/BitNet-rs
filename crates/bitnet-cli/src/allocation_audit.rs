    ...
    serde_json::json!({
        ...
        "prompt_prefill_breakdown": {
            ...
            "forward_boundary": {
                ...
                "classification": "the typed transformer forward workspace now owns the feed_forward.down_proj output handle while preserving the existing Candle math path",
                "reuse_status": "workspace_owned_feed_forward_output_candle_storage_reuse_not_enabled",
                "workspace_owned_tensor_output": {
                    "boundary": "FeedForward::forward_with_workspace down_proj output",
                    "status": "workspace_owned_handle_only",
                    "storage_reuse_enabled": false,
                    "claim_scope": "one transformer feed-forward output handle is workspace-owned; Candle tensor storage reuse and speedup are not claimed",
                },
                "no_reuse_reason": "TransformerModel::forward_with_workspace, TransformerBlock::forward_with_workspace, and FeedForward::forward_with_workspace thread a typed workspace through the model and keep the feed-forward output handle in the workspace, but Candle operations still allocate their result tensors before any reusable storage API is enabled",
                "required_api_boundary": "typed_transformer_forward_workspace",
                "next_safe_change": "replace another owned transformer tensor output or prove a narrower no-reuse boundary behind generated-ID and strict-receipt preservation tests",
