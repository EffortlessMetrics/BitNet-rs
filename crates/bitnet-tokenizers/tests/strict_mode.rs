...
fn strict_mode_disallows_bpe_mock_fallback() {
    with_var("BITNET_STRICT_TOKENIZERS", Some("1"), || {
        ...
        assert!(err.to_string().contains("disabled"), "expected strict mode error, got: {}", err);
    });
}
...
fn strict_mode_disallows_unknown_tokenizer_fallback() {
    with_var("BITNET_STRICT_TOKENIZERS", Some("1"), || {
        let cfg =
            TokenizerConfig { model_type: "unknown_tokenizer_type".into(), ..Default::default() };
...
fn normal_mode_allows_mock_fallback() {
    ...
    with_var("BITNET_STRICT_TOKENIZERS", None::<&str>, || {
        ...
        let tokenizer =
            UniversalTokenizer::new(cfg).expect("mock tokenizer should be created in normal mode");
