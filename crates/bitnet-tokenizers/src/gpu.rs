...
impl VocabLookupTable {
    ...
    pub fn get_token(&self, id: TokenId) -> Option<&str> {
        self.id_to_token.get(id as usize).filter(|s| !s.is_empty()).map(|s| s.as_str())
    }
...
impl Default for GpuBatchTokenizerConfig {
    fn default() -> Self {
        Self { min_batch_for_gpu: 2, max_tokens_per_prompt: 4096 }
    }
}
...
impl<T: CpuTokenizer> GpuBatchTokenizer<T> {
    ...
    pub fn new(vocab: VocabLookupTable, cpu_tokenizer: T, config: GpuBatchTokenizerConfig) -> Self {
        Self { vocab: Arc::new(vocab), cpu_tokenizer, config, gpu_buffer_ready: false }
    }
    ...
    pub fn tokenize_batch(&self, prompts: &[&str]) -> Vec<TokenizedPrompt> {
        if prompts.len() < self.config.min_batch_for_gpu || !self.gpu_buffer_ready {
    ...
    fn tokenize_batch_cpu(&self, prompts: &[&str]) -> Vec<TokenizedPrompt> {
        prompts
            ...
            .map(|text| {
                ...
                TokenizedPrompt { text: text.to_string(), token_ids, duration: start.elapsed() }
            })
            .collect()
    }
...
mod tests {
    ...
    impl WhitespaceCpuTokenizer {
        fn new(vocab: &VocabLookupTable) -> Self {
            Self { vocab: vocab.token_to_id.clone() }
        }
    }

    impl CpuTokenizer for WhitespaceCpuTokenizer {
        fn encode(&self, text: &str) -> Vec<TokenId> {
            text.split_whitespace().map(|w| self.vocab.get(w).copied().unwrap_or(0)).collect()
        }
    }
    ...
    fn test_vocab_gpu_buffer() {
        let vocab = VocabLookupTable::from_pairs(vec![("ab".to_string(), 1)]);
    ...
    fn test_single_request_uses_cpu() {
        ...
        let mut tokenizer = GpuBatchTokenizer::new(vocab, cpu, GpuBatchTokenizerConfig::default());
    ...
    fn test_batch_below_threshold_uses_cpu() {
        let vocab = test_vocab();
        let cpu = WhitespaceCpuTokenizer::new(&vocab);
        let config = GpuBatchTokenizerConfig { min_batch_for_gpu: 3, ..Default::default() };
        let mut tokenizer = GpuBatchTokenizer::new(vocab, cpu, config);
        tokenizer.upload_vocab();

        // Batch of 2 < threshold of 3, should use CPU path
        let results = tokenizer.tokenize_batch(&["hello world", "foo bar"]);
    ...
    fn test_batch_gpu_path() {
        let vocab = test_vocab();
        let cpu = WhitespaceCpuTokenizer::new(&vocab);
        let config = GpuBatchTokenizerConfig { min_batch_for_gpu: 2, ..Default::default() };
        let mut tokenizer = GpuBatchTokenizer::new(vocab, cpu, config);
        tokenizer.upload_vocab();

        let results = tokenizer.tokenize_batch(&["hello", "world", "foo"]);
    ...
    fn test_gpu_not_ready_falls_back() {
        let vocab = test_vocab();
        let cpu = WhitespaceCpuTokenizer::new(&vocab);
        let tokenizer = GpuBatchTokenizer::new(vocab, cpu, GpuBatchTokenizerConfig::default());
    ...
    fn test_tokenized_prompt_json_roundtrip() {
        ...
        let restored: TokenizedPrompt = serde_json::from_str(&json).unwrap();
