//! Core startup contract primitives shared by binaries and service layers.
//!
//! This crate owns the canonical startup profile contract model:
//! - component defaults (scenario/environment)
//! - profile/state evaluation
//! - enforcement behavior with diagnostics
...
impl RuntimeComponent {
    const fn default_scenario(self) -> TestingScenario {
        match self {
    ...
    const fn default_environment(self) -> ExecutionEnvironment {
        match self {
...
/// Resolved startup contract for a component.
#[derive(Debug)]
...
impl ProfileContract {
    ...
    pub fn with_context(
        ...
        let required = profile.cell.map(|cell| cell.required_features.labels()).unwrap_or_default();
        let optional = profile.cell.map(|cell| cell.optional_features.labels()).unwrap_or_default();
        let forbidden_cell =
            profile.cell.map(|cell| cell.forbidden_features.labels()).unwrap_or_default();
        let mut missing = Vec::new();
        let mut forbidden = Vec::new();
        let state = if profile.cell.is_some() {
    ...
    pub fn summary(&self) -> String {
        ...
        let required =
            if self.required.is_empty() { "none".to_string() } else { self.required.join(", ") };
        let optional =
            if self.optional.is_empty() { "none".to_string() } else { self.optional.join(", ") };
        let forbidden =
            if self.forbidden.is_empty() { "none".to_string() } else { self.forbidden.join(", ") };
...
#[cfg(test)]
mod tests {
    use super::{ContractPolicy, ProfileContract, RuntimeComponent};
