...
//! Policy snapshot and compatibility helpers for testing policy orchestration.

use bitnet_testing_profile::{
    ActiveContext, ActiveProfile, ExecutionEnvironment, FeatureSet, TestingScenario,
    validate_active_profile_for, validate_profile_for_context,
};
use bitnet_testing_scenarios_core::{
    ConfigurationContext, ScenarioConfigManager, TestConfigProfile,
};
...
impl PolicySnapshot {
    ...
    pub fn from_configuration_context(context: ConfigurationContext) -> Self {
        ...
        let active_profile =
            bitnet_testing_profile::active_profile_for(context.scenario, context.environment);
