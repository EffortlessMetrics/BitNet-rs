...
pub use bitnet_bdd_grid_core::{
    ...
    feature_set_from_names, feature_set_from_names_strict,
};
...
fn build_curated_rows() -> Box<[BddCell]> {
    fn curated_features(features: &[&str]) -> FeatureSet {
        feature_set_from_names_strict(features)
            .unwrap_or_else(|e| panic!("curated grid has unknown feature labels: {e}"))
    }
