...
impl RepetitionPenaltyConfig {
    ...
    pub fn apply(&self, logits: &mut [f32], token_counts: &[(u32, usize)]) {
        // PERF: Pre-calculate the inverse penalty base outside the loop to avoid division later.
        ...
        for &(token_id, count) in token_counts {
            ...
            if apply_count_penalty {
                ...
                // PERF: Compute powi lazily per branch and multiply by the pre-calculated inverse to eliminate division overhead.
                if logits[idx] > 0.0 {
