...
impl RepetitionPenaltyConfig {
    ...
    pub fn apply(&self, logits: &mut [f32], token_counts: &[(u32, usize)]) {
        ...
        // ⚡ Bolt: Pre-calculate the inverse base outside the loop to avoid redundant divisions
        ...
        for &(token_id, count) in token_counts {
            ...
            if has_count_penalty {
                ...
                if logits[idx] > 0.0 {
                    // ⚡ Bolt: Compute inverse power lazily and multiply to eliminate division overhead
                    logits[idx] *= inv_count_penalty.powi(count);
                } else {
                    // ⚡ Bolt: Compute normal power lazily inside the branch
