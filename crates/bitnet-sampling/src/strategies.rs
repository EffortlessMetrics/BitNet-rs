...
impl RepetitionPenaltyConfig {
    ...
    pub fn apply(&self, logits: &mut [f32], token_counts: &[(u32, usize)]) {
        // ⚡ Bolt: Pre-calculate the inverse penalty outside the hot loop to replace division with multiplication.
        // Division is significantly slower than multiplication on modern CPUs.
        let has_count_penalty = self.count_penalty.to_bits() != 1.0f32.to_bits();
        let inv_count_penalty = if has_count_penalty { 1.0 / self.count_penalty } else { 1.0 };
        ...
        for &(token_id, count) in token_counts {
            ...
            if has_count_penalty {
                ...
                // ⚡ Bolt: Compute `powi` lazily inside branches to avoid redundant calculations.
                if logits[idx] > 0.0 {
