...
impl OpenClQueue {
    ...
    pub fn new(ctx: &OpenClContext) -> Result<Self> {
        let inner =
            CommandQueue::create_default_with_properties(&ctx.inner, CL_QUEUE_PROFILING_ENABLE, 0)
                .map_err(|e| OpenClError::QueueCreation { reason: e.to_string() })?;
    ...
    pub fn flush(&self) -> Result<()> {
        self.inner
            .flush()
            .map_err(|e| OpenClError::QueueCreation { reason: format!("flush failed: {e}") })
    }
    ...
    pub fn finish(&self) -> Result<()> {
        self.inner
            .finish()
            .map_err(|e| OpenClError::QueueCreation { reason: format!("finish failed: {e}") })
    }
}
