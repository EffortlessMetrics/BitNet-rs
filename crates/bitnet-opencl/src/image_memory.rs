...
impl ImageFormat {
    ...
    pub const fn new(channel_order: ImageChannelOrder, channel_type: ImageChannelType) -> Self {
        Self { channel_order, channel_type }
    }
...
impl ImageCapabilities {
    ...
    pub fn unsupported() -> Self {
        Self { images_supported: false, max_width: 0, max_height: 0, supported_formats: Vec::new() }
    }
    ...
    pub fn best_format(&self) -> Option<ImageFormat> {
        ...
        PREFERRED_FORMATS.iter().find(|pf| self.supported_formats.contains(pf)).copied()
    }
...
impl ImageMemoryManager {
    ...
    pub fn new(capabilities: ImageCapabilities) -> Self {
        ...
        Self { capabilities, selected_format, descriptors: Vec::new() }
    }
    ...
    pub fn allocate(
        ...
        rows: usize,
        cols: usize,
    ) -> WeightImageDescriptor {
        let name = name.into();

        let (backing, image_width, image_height, size_bytes) = if let Some(fmt) =
            self.selected_format
        {
            if self.capabilities.matrix_fits(rows, cols, &fmt) {
                ...
                log::debug!("Weight '{name}' ({rows}×{cols}) exceeds image limits, using buffer");
...
mod tests {
    ...
    fn test_allocate_uses_image_when_possible() {
        ...
        assert_eq!(
            desc.backing,
            MemoryBacking::Image(ImageFormat::new(ImageChannelOrder::Rgba, ImageChannelType::Half))
        );
    }
    ...
    fn test_descriptor_display() {
        let desc = WeightImageDescriptor {
            ...
            rows: 4096,
            cols: 4096,
            backing: MemoryBacking::Image(ImageFormat::new(
                ImageChannelOrder::Rgba,
                ImageChannelType::Half,
            )),
            image_width: 1024,
            image_height: 4096,
