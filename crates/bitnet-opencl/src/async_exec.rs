...
impl AsyncKernelExecution {
    ...
    pub fn new() -> Self {
        Self { next_id: 1, events: HashMap::new(), callbacks: HashMap::new(), deps: HashMap::new() }
    }
    ...
    pub fn enqueue_after(&mut self, _kernel_name: &str, wait_list: &[KernelId]) -> ClEvent {
        let id = self.next_id;
        self.next_id += 1;
    ...
    {
        self.callbacks.entry(kernel_id).or_default().push(Box::new(cb));
    }
        ...
        self.deps
            ...
            .map(|dep_ids| {
                dep_ids.iter().all(|dep| {
                    self.events.get(dep).is_some_and(|e| e.status() == EventStatus::Complete)
                })
            })
            .unwrap_or(true)
    }
...
impl DoubleBuffer {
    ...
    pub fn both_complete(&self) -> bool {
        let t_done = self.transfer_event.as_ref().is_none_or(|e| e.is_finished());
        let c_done = self.compute_event.as_ref().is_none_or(|e| e.is_finished());
        t_done && c_done
    }
...
impl KernelPipeline {
    ...
    pub fn new() -> Self {
        Self { exec: AsyncKernelExecution::new(), stages: Vec::new() }
    }
    ...
    pub fn add_stage(&mut self, kernel_name: &str) -> KernelId {
        let wait_list: Vec<KernelId> =
            self.stages.last().map(|(_, ev)| vec![ev.id()]).unwrap_or_default();
