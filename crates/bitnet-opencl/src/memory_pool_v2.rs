...
impl MemoryPoolV2 {
    ...
    pub fn free(&mut self, handle: PoolHandle) {
        ...
        self.deferred.push_back(DeferredEntry { handle, freed_at: Instant::now() });
    ...
    pub fn reclaim(&mut self) {
        ...
        while let Some(entry) = self.deferred.pop_front() {
            if now.duration_since(entry.freed_at) >= min_age {
                ...
                self.deferred_bytes =
                    self.deferred_bytes.saturating_sub(entry.handle.allocated_bytes as u64);
            } else {
    ...
    pub fn statistics(&self) -> PoolStatistics {
        ...
        stats.cache_misses =
            stats.total_allocations - stats.cache_hits - stats.oversized_allocations;
        stats
    }
