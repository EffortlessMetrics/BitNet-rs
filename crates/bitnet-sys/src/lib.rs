...
pub mod disabled {
    //! Disabled stubs when crossval feature is not enabled
    //!
    //! This module provides stub implementations that return errors
    //! when the crossval feature is not enabled.

    /// Error returned when crossval feature is disabled
    #[derive(Debug, thiserror::Error)]
    #[error("BitNet C++ bindings not available (compile with --features crossval)")]
    pub struct DisabledError;

    /// Check if C++ implementation is available (always false without crossval)
    pub fn is_available() -> bool {
        false
    }

    /// Get version (returns error without crossval)
