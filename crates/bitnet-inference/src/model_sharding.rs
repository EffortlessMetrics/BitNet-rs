...
pub enum CommOp {
    ...
    ReduceScatter { shard_elements: usize, num_devices: usize },
    /// Point-to-point transfer between two devices (pipeline).
    SendRecv { src_device: usize, dst_device: usize, num_elements: usize },
...
pub fn assign_layers(topology: &ModelTopology, config: &ShardingConfig) -> Vec<LayerAssignment> {
    ...
    for (i, &mem) in topology.layer_memory.iter().enumerate() {
        ...
        let target =
            device_mem.iter().enumerate().min_by_key(|(_, m)| *m).map(|(idx, _)| idx).unwrap_or(0);
    ...
    match strategy {
        ShardingStrategy::ColumnParallel => {
            ...
            ShardDescriptor {
                ...
                comm_op: CommOp::AllGather { shard_elements: rows * shard_cols, num_devices },
            }
        }
        ShardingStrategy::RowParallel => {
            ...
            ShardDescriptor {
                ...
                comm_op: CommOp::ReduceScatter { shard_elements: shard_rows * cols, num_devices },
            }
        }
...
pub fn pipeline_comm(
    ...
    CommOp::SendRecv { src_device, dst_device, num_elements: hidden_dim * seq_len }
}
...
mod tests {
    ...
    fn test_pipeline_comm_descriptor() {
        ...
        assert_eq!(op, CommOp::SendRecv { src_device: 0, dst_device: 1, num_elements: 2048 * 128 });
    }
