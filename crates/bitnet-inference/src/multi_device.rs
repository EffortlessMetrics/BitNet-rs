...
// Even round-robin across all devices
    ...
    pub fn memory_plan(&self, model_size: usize) -> Vec<(Device, usize)> {
        ...
        match &self.strategy {
            ...
            SchedulingStrategy::LayerParallel { .. } => {
                let total_mem: usize =
                    self.devices.iter().map(|d| d.memory_available).sum();
                if total_mem == 0 {
                ...
                self.devices
                    .iter()
                    .map(|slot| {
                        let share = (slot.memory_available as f64
                            / total_mem as f64
                            * model_size as f64)
                            as usize;
                        (slot.device, share)
...
fn default_memory_for_device(device: &Device) -> usize {
    match device {
        ...
        Device::OpenCL(_) | Device::Vulkan(_) | Device::Hip(_) => 8 * 1024 * 1024 * 1024,
        Device::Metal => 8 * 1024 * 1024 * 1024,
        Device::Npu => 4 * 1024 * 1024 * 1024,
    }
}
...
fn default_capability_for_device(device: &Device) -> f64 {
    match device {
        Device::Cpu => 1.0,
        Device::Cuda(_) => 10.0,
        Device::OpenCL(_) | Device::Vulkan(_) | Device::Hip(_) => 6.0,
        Device::Metal => 7.0,
        Device::Npu => 3.0,
    }
}
...
mod tests {
    ...
    fn test_assign_layer_with_split_points() {
        ...
        let strategy = SchedulingStrategy::LayerParallel {
            split_points: vec![16],
        };
    ...
    fn test_pipeline_strategy_uses_best_device() {
        ...
        let scheduler =
            MultiDeviceScheduler::new(slots, SchedulingStrategy::Pipeline);
        assert_eq!(scheduler.assign_layer(0), Device::Cuda(0));
    ...
    fn test_best_device_selection() {
        ...
        let scheduler =
            MultiDeviceScheduler::new(slots, SchedulingStrategy::SingleDevice);
        assert_eq!(scheduler.assign_layer(0), Device::Cuda(0));
