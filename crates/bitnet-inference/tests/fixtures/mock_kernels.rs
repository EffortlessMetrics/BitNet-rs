...
pub struct MockKernel {
    ...
    pub available: bool,
    pub min_compute_capability: Option<(u32, u32)>, // GPU only
    pub required_simd_features: Vec<SimdFeature>,    // CPU only
...
pub enum QuantizationType {
    I2S,     // 2-bit signed
    TL1,     // Table lookup 1 (ARM NEON)
    TL2,     // Table lookup 2 (x86 AVX)
    FP32,    // Fallback (full precision)
}
...
impl MockKernelRegistry {
    ...
    pub fn new() -> Self {
        Self {
            kernels: vec![
                // GPU Kernels
                gpu_kernel_gemm_fp16(),
                gpu_kernel_gemm_bf16(),
                gpu_kernel_wmma_matmul_fp16(),
                gpu_kernel_wmma_matmul_bf16(),
                gpu_kernel_i2s_quantize(),
                gpu_kernel_i2s_pack(),
                gpu_kernel_i2s_matmul(),
                gpu_kernel_tl1_pack(),
                gpu_kernel_tl2_pack(),
                gpu_kernel_cuda_sync(),

                // CPU Kernels
                cpu_kernel_i2s_gemv(),
                cpu_kernel_tl1_neon_pack(),
                cpu_kernel_tl1_neon_matmul(),
                cpu_kernel_tl2_avx_matmul(),
                cpu_kernel_tl2_avx512_pack(),
                cpu_kernel_quantized_matmul_i2s(),

                // Fallback Kernels
                fallback_kernel_dequant_i2s(),
                fallback_kernel_fp32_matmul(),
                fallback_kernel_scalar_matmul(),
            ],
        }
    }
    ...
    pub fn has_native_quantized_kernel(&self, qtype: QuantizationType, device: DeviceType) -> bool {
        self.kernels.iter().any(|k| {
            k.quantization_type == qtype
                && k.device_type == device
                && k.available
...
// ============================================================================
// GPU Kernel Definitions (ADR-012 Naming)
// ============================================================================
...
fn gpu_kernel_gemm_fp16() -> MockKernel {
    MockKernel {
        ...
        available: true,
        min_compute_capability: Some((7, 0)), // Volta+
...
fn gpu_kernel_gemm_bf16() -> MockKernel {
    MockKernel {
        ...
        available: true,
        min_compute_capability: Some((8, 0)), // Ampere+
        required_simd_features: vec![],
...
fn gpu_kernel_wmma_matmul_fp16() -> MockKernel {
    MockKernel {
        ...
        precision: KernelPrecision::FP16,
        available: true,
        min_compute_capability: Some((7, 0)), // Volta+ Tensor Cores
        required_simd_features: vec![],
...
fn gpu_kernel_wmma_matmul_bf16() -> MockKernel {
    MockKernel {
        ...
        precision: KernelPrecision::BF16,
        available: true,
        min_compute_capability: Some((8, 0)), // Ampere+ BF16 Tensor Cores
        required_simd_features: vec![],
...
fn gpu_kernel_i2s_quantize() -> MockKernel {
    MockKernel {
        ...
        available: true,
        min_compute_capability: Some((6, 1)), // Pascal+
        required_simd_features: vec![],
...
fn gpu_kernel_cuda_sync() -> MockKernel {
    MockKernel {
        ...
        available: true,
        min_compute_capability: Some((6, 0)), // All modern GPUs
        required_simd_features: vec![],
...
// ============================================================================
// CPU Kernel Definitions (ADR-012 Naming)
// ============================================================================
...
// ============================================================================
// Kernel ID Pattern Helpers (ADR-012)
// ============================================================================

/// Check if kernel ID represents a GPU kernel
pub fn is_gpu_kernel(kernel_id: &str) -> bool {
    kernel_id.starts_with("gemm_")
        || kernel_id.starts_with("wmma_")
        || kernel_id.starts_with("cuda_")
        || kernel_id.starts_with("i2s_gpu_")
        || kernel_id.starts_with("tl1_gpu_")
        || kernel_id.starts_with("tl2_gpu_")
}

/// Check if kernel ID represents a quantized kernel
pub fn is_quantized_kernel(kernel_id: &str) -> bool {
    is_gpu_kernel(kernel_id)
        || kernel_id.starts_with("i2s_")
        || kernel_id.starts_with("tl1_")
        || kernel_id.starts_with("tl2_")
...
pub fn is_fallback_kernel(kernel_id: &str) -> bool {
    kernel_id.starts_with("dequant_")
...
pub fn generate_kernel_id(qtype: QuantizationType, device: DeviceType, precision: KernelPrecision) -> String {
    match (qtype, device, precision) {
        (QuantizationType::I2S, DeviceType::Gpu, KernelPrecision::FP16) => "gemm_fp16".to_string(),
        (QuantizationType::I2S, DeviceType::Gpu, KernelPrecision::BF16) => "gemm_bf16".to_string(),
        (QuantizationType::I2S, DeviceType::Gpu, KernelPrecision::INT2) => "i2s_gpu_quantize".to_string(),
...
mod tests {
    ...
    fn test_has_native_quantized_kernel() {
        ...
        #[cfg(feature = "cpu")]
        {
            assert!(registry.has_native_quantized_kernel(QuantizationType::I2S, DeviceType::Cpu));
            assert!(registry.has_native_quantized_kernel(QuantizationType::TL1, DeviceType::Cpu));
            assert!(registry.has_native_quantized_kernel(QuantizationType::TL2, DeviceType::Cpu));
        }
    }
}
