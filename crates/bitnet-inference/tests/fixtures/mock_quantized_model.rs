...
impl MockQuantizedLinear {
    ...
    pub async fn forward(&self, _input: &MockTensor) -> Result<MockTensor> {
        if !self.has_native_quantized_kernel() {
            ...
            panic!(
                "fallback to FP32 in debug mode: layer={}, qtype={:?}, device={:?}", self
                .name, self.quantization_type, self.device
            );
            return Err(
                BitNetError::StrictMode(
                        format!(
                            "FP32 fallback rejected - qtype={:?}, device={:?}, layer_dims=[{}, {}], reason=kernel_unavailable",
                            self.quantization_type, self.device, self.in_features, self
                            .out_features
                        ),
                    )
                    .into(),
...
impl MockBitNetAttention {
    ...
    pub fn new(
        ...
        Self {
            q_proj: MockQuantizedLinear::new(
                "q_proj",
                hidden_size,
                hidden_size,
                qtype,
            ...
            k_proj: MockQuantizedLinear::new(
                "k_proj",
                hidden_size,
                hidden_size,
                qtype,
            ...
            v_proj: MockQuantizedLinear::new(
                "v_proj",
                hidden_size,
                hidden_size,
                qtype,
            ...
            o_proj: MockQuantizedLinear::new(
                "o_proj",
                hidden_size,
                hidden_size,
                qtype,
    ...
    pub fn validate_projections_quantized(&self) -> Result<()> {
        ...
        for (name, proj) in &projections {
            ...
            if !has_native_kernel {
                #[cfg(debug_assertions)]
                panic!(
                    "fallback to FP32 in debug mode: {} projection would fall back", name
                );
                return Err(
                    BitNetError::StrictMode(
                            format!(
...
impl MockReceipt {
    ...
    pub fn with_native_quantized(backend: &str) -> Self {
        Self {
            ...
            kernels: vec![
                "gemm_fp16".to_string(), "i2s_gpu_quantize".to_string(), "wmma_matmul"
                .to_string(),
            ],
    ...
    pub fn with_fp32_fallback(backend: &str) -> Self {
        Self {
            ...
            kernels: vec![
                "dequant_i2s".to_string(), "fp32_matmul".to_string(), "cuda_sync"
                .to_string(),
            ],
...
pub fn create_test_model_with_fallback() -> MockBitNetModel {
    let mut model = MockBitNetModel::new(2, 768, 12, QuantizationType::I2S, Device::Cpu);
...
pub fn create_test_model_quantized() -> MockBitNetModel {
    MockBitNetModel::new(2, 768, 12, QuantizationType::I2S, Device::Cpu)
}
#[cfg(test)]
mod tests {
    use super::*;
