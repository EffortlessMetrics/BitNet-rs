...
pub struct InferenceResult {
    pub tokens: Vec<u32>,
    pub text: String,
    pub logits: Vec<Vec<f32>>,        // [step][vocab_size]
    pub probabilities: Vec<Vec<f32>>, // [step][vocab_size]
    pub duration_ms: u64,
}
...
pub fn compare_inference_outputs(
    ...
    // 2. Calculate logit correlation (Pearson correlation coefficient)
    let logit_correlation = calculate_logit_correlation(&bitnet_result.logits, &cpp_result.logits)?;

    // 3. Calculate MSE (mean squared error) across all logits
    let mse = calculate_mse(&bitnet_result.logits, &cpp_result.logits)?;

    // 4. Calculate perplexity for both implementations
...
fn calculate_logit_correlation(logits_a: &[Vec<f32>], logits_b: &[Vec<f32>]) -> Result<f32> {
    // Flatten logits for correlation calculation
...
fn calculate_perplexity(probabilities: &[Vec<f32>]) -> Result<f32> {
    ...
    for step_probs in probabilities {
        // Find the max probability (greedy decoding) and use it for perplexity
...
pub fn aggregate_validation_metrics(
    ...
    // Calculate perplexity degradation
    // Degradation = |reference_perplexity - perplexity| / reference_perplexity
    ...
    Ok(AggregatedMetrics {
        ...
        lookup_performance_metrics: None, // Populated separately for table lookup tests
    })
}
...
pub fn parse_crossval_output(output: &std::process::Output) -> Result<CrossvalResults> {
    ...
    // Parse stdout for JSON or structured output
    ...
    // Fallback: Parse text-based output with regex patterns
...
fn parse_json_crossval_output(stdout: &str) -> Result<CrossvalResults> {
    ...
    // Find JSON block in output (may be mixed with log messages)
...
fn parse_text_crossval_output(stdout: &str) -> Result<CrossvalResults> {
    // Simple text parsing without regex for MVP
    // Extract numeric values following keywords
    let text = stdout.to_lowercase();

    // Helper function to extract float value after a pattern
...
mod tests {
    ...
    fn test_calculate_mse_different() {
        ...
        let mse = calculate_mse(&logits_a, &logits_b).unwrap();
        assert_eq!(mse, 1.0); // Each element differs by 1, so MSE = 1
    }
