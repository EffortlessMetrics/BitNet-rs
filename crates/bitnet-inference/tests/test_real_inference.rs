...
async fn test_real_transformer_forward_pass() -> Result<()> {
    ...
    // Convert to ConcreteTensor for testing
    ...
    // Check that logits have reasonable distribution (not all zeros)
...
async fn test_multi_head_attention() -> Result<()> {
    // For now, just test that we can access the attention module
    // The real test will be through the full transformer
...
async fn test_autoregressive_generation() -> Result<()> {
    ...
    // Verify we get actual text generation
...
async fn test_performance_targets() -> Result<()> {
    ...
    // For small test models, we expect reasonable performance
    // Note: Real BitNet 2B models would be different, but for unit tests this validates the pipeline
    println!(
        "✅ AC5: Performance - {:.2} tok/sec ({} tokens in {:?})",
        tokens_per_second, tokens_generated, total_time
    );

    // Just verify we get measurable performance (not testing exact targets in unit tests)
...
fn create_real_model_with_weights(config: &BitNetConfig) -> Result<Arc<BitNetModel>> {
    ...
    // Create minimal tensor set for a working model
...
fn create_mock_quantized_weights(
    ...
    // Create BitNetTensor from the data
    let candle_device = candle_core::Device::Cpu;
