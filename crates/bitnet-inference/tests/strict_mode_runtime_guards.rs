...
async fn test_strict_blocks_fp32_fallback_i2s() -> Result<()> {
    ...
    // This test no longer uses environment variables; strict mode is implicit in layer behavior
...
fn test_strict_mode_config_from_env() {
    // Test with strict mode enabled (inline unsafe usage)
    let config = unsafe {
...
fn test_non_strict_mode_skips_validation() {
    // Use with_config API with explicit non-strict mode configuration
    let config = bitnet_common::strict_mode::StrictModeConfig {
        enabled: false,
        fail_on_mock: false,
...
async fn test_strict_mode_end_to_end() -> Result<()> {
    // Test 1: Strict mode blocks fallback (implicit in layer behavior)
    ...
    // Test 2: Non-strict mode allows everything (implicit behavior)
    let layer = create_fallback_layer(100, 200, QuantizationType::I2S)?;
    let input = create_mock_tensor(2, 8, 100)?;
