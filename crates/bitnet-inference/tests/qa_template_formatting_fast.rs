...
mod qa_formatting_fast {
    ...
    fn test_raw_template_preserves_math_question() -> Result<()> {
        ...
        // Math question should pass through unchanged
    ...
    fn test_instruct_template_includes_system_prompt() -> Result<()> {
        ...
        // Should include system prompt at the beginning
        assert!(
    ...
    fn test_llama3_chat_template_special_tokens() -> Result<()> {
        ...
        // Must start with begin_of_text token
        assert!(
        ...
        // Should contain EOT token after user message
        assert!(
            output.contains("<|eot_id|>"),
            "Should contain <|eot_id|> token after user message"
        );

        // Should end with assistant header ready for response
        assert!(
    ...
    fn test_llama3_chat_template_with_system_prompt() -> Result<()> {
        ...
        // Should contain system header and content
        assert!(
        ...
        // System message should be terminated with EOT
    ...
    fn test_instruct_template_stop_sequences() {
        ...
        // Should stop on new questions to prevent runaway generation
        assert!(
            stops.contains(&"\n\nQ:".to_string()),
    ...
    fn test_special_token_parsing_control() {
        // Raw should NOT parse special tokens (treat as literal text)
        assert!(!TemplateType::Raw.parse_special(), "Raw template should NOT parse special tokens");

        // Instruct should NOT parse special tokens (no special tokens used)
        assert!(
            !TemplateType::Instruct.parse_special(),
            "Instruct template should NOT parse special tokens"
        );

        // LLaMA-3 SHOULD parse special tokens (<|begin_of_text|>, <|eot_id|>, etc.)
        assert!(
    ...
    fn test_math_completion_prompt_raw() -> Result<()> {
        ...
        // Completion-style math prompt (no question, just pattern)
        let input = "2+2=";
        let output = template.apply(input, None);
    ...
    fn test_math_completion_prompt_instruct() -> Result<()> {
        ...
        // Completion-style prompt with Instruct template (suboptimal but valid)
        let input = "2+2=";
        let output = template.apply(input, None);

        // Instruct will format it as Q&A even for completion
    ...
    fn test_complex_question_formatting() -> Result<()> {
        ...
        // Verify full complex question formatting
    ...
    fn test_special_characters_in_input() -> Result<()> {
        ...
        // Input with special characters that might break formatting
...
mod qa_template_parsing_fast {
    ...
    fn test_parse_invalid_template_types() {
        // Invalid template types should return errors
...
mod qa_template_detection_fast {
    ...
    fn test_detection_priority_gguf_over_tokenizer() {
        // GGUF says LLaMA-3, tokenizer says Mistral (Instruct)
