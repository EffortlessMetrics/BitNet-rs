...
fn test_qk256_dequant_correctness() {
    ...
    // Test with random data (multiple blocks)
...
fn test_qk256_dequant_performance_baseline() {
    ...
    // Medium-sized tensor for performance validation
    const QK256_PACKED_BYTES: usize = 64;
    let num_blocks = 64; // 16384 elements total
    ...
    // Benchmark scalar (10 iterations)
    ...
    // Assert minimum 1.2× speedup (MVP baseline)
    // This threshold will increase as optimizations are added (target ≥3×)
...
fn test_qk256_lut_code_patterns() {
    ...
    // Test each code pattern
    for code in 0u8..4 {
        // Create packed data with uniform code
        ...
        // Verify all values match expected LUT value * scale
...
fn test_qk256_edge_case_scales() {
    ...
    const QK256_PACKED_BYTES: usize = 64;
    let quantized = vec![0xAAu8 as i8; QK256_PACKED_BYTES]; // code=2 → +1.0

    // Test cases: very small, very large, zero, negative scales
    let test_cases = vec![
