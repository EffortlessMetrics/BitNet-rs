...
// ==============================================================================
// Priority 1: QuantizedLinear Integration Tests (1,530 lines uncovered)
// ==============================================================================
...
async fn test_i2s_quantized_linear_forward_real_inference() -> Result<()> {
    // Create realistic input tensor (batch=2, seq_len=16, hidden_size=256)
    let batch_size = 2;
    let seq_len = 16;
    let hidden_size = 256;
    let out_features = 512;

    // Generate non-trivial input data with Gaussian-like distribution
    let input_data: Vec<f32> = (0..batch_size * seq_len * hidden_size)
        .map(|i| {
            let x = (i as f32) / 100.0;
            (x.sin() * 0.5 + x.cos() * 0.3).tanh() // Bounded non-trivial values
        })
        .collect();
    ...
    let weight_data: Vec<f32> = (0..hidden_size * out_features)
        .map(|i| {
            ...
            x * 0.5 // Centered around 0 with variance
        })
        .collect();
    ...
    // Execute forward pass (covers forward_i2s kernel path)
    ...
    // Validate output has non-zero variance (actual computation occurred)
...
async fn test_tl1_quantized_linear_forward_real_inference() -> Result<()> {
    ...
    // Generate input with controlled range for TL1
    ...
    // Create and quantize weights with TL1
    ...
    // Create TL1 lookup table (16 entries for TL1)
    ...
    // Execute forward pass (covers forward_tl1 kernel path)
    ...
    // Validate table lookup occurred (non-trivial computation)
...
async fn test_tl2_quantized_linear_forward_real_inference() -> Result<()> {
    ...
    // Create and quantize weights with TL2
    let weight_data: Vec<f32> =
    ...
    // Create TL2 lookup table (256 entries for TL2)
    ...
    // Execute forward pass (covers forward_tl2 kernel path)
...
async fn test_quantized_linear_with_bias() -> Result<()> {
    ...
    // Validate computation occurred
    let output_candle = output.to_candle()?;
    let output_data = output_candle.flatten_all()?.to_vec1::<f32>()?;

    // Check that output has non-trivial values from matrix multiplication
...
async fn test_quantized_linear_input_validation() -> Result<()> {
    ...
    // Test 2: Wrong feature dimension
...
// ==============================================================================
// Priority 2: KV-Cache Integration Tests (717 lines uncovered in attention.rs)
// ==============================================================================
...
async fn test_kv_cache_error_handling() -> Result<()> {
    ...
    // Test out-of-bounds layer index
...
// ==============================================================================
// Integration Test: Multi-Layer Quantized Forward Pass
// ==============================================================================
...
async fn test_multi_layer_quantized_transformer_block() -> Result<()> {
    ...
    // Simulate 3-layer transformer with quantized linears
    for layer_idx in 0..3 {
        // Create and quantize weights for this layer
        ...
        // Apply activation (simulating residual connection + activation)
...
async fn test_quantized_linear_extreme_values() -> Result<()> {
    ...
    // Test with large values (within valid range)
