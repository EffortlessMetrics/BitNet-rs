...
// Smoke tests for GGUF header parsing
...
async fn opens_model_when_env_is_set() -> anyhow::Result<()> {
    ...
    // Async open (sanity) + parse header for real format validation
    let header = bitnet_inference::gguf::read_header(&path).await?;
    // Basic sanity checks: version present, counts look plausible
    assert!(header.version >= 1);
    // n_tensors/n_kv are data dependent; just ensure parser didn't garbage out
    Ok(())
}
