...
use bitnet_inference::production_engine::{
    ...
    TimingMetrics, ThroughputMetrics,
};
use bitnet_inference::receipts::{
    InferenceReceipt, ModelInfo, PerformanceBaseline, TestResults, RECEIPT_SCHEMA_VERSION,
};
...
fn sampling_strategy_reproducible_with_seed() {
    ...
    let cfg = SamplingConfig { temperature: 0.8, top_k: 5, top_p: 0.9, seed: Some(42), ..Default::default() };
...
fn inference_config_serialization_roundtrip() {
    ...
    let restored: InferenceConfig = serde_json::from_str(&json).expect("deserialize must succeed");
...
fn generation_config_builder_stop_token_ids() {
    let cfg = GenerationConfig::greedy()
        .with_max_tokens(64)
        .with_stop_token_ids(vec![128009, 50256]);
...
fn generation_config_serialization_roundtrip() {
    ...
    let json = serde_json::to_string(&cfg).expect("serialize must succeed");
    let restored: GenerationConfig = serde_json::from_str(&json).expect("deserialize must succeed");
...
fn receipt_generate_detects_mock_kernels() {
    let receipt =
        InferenceReceipt::generate("cpu", vec!["mock_gemv".to_string()], None)
...
fn receipt_with_model_info_builder() {
    ...
    assert_eq!(
        receipt.model_info.model_path.as_deref(),
...
fn receipt_with_test_results_builder() {
    let results =
        TestResults { total_tests: 10, passed: 9, failed: 1, ..Default::default() };
    let receipt = InferenceReceipt::generate("cpu", vec!["i2s_gemv".to_string()], None)
        .expect("must generate")
...
fn performance_metrics_efficiency_ratio_nonzero() {
    ...
    assert!(
        (ratio - 0.1).abs() < 1e-6,
        "100 tokens / 1000 ms = 0.1 tokens/ms, got {ratio}"
    );
}
...
fn inference_result_zero_tps_is_unacceptable() {
    let perf = PerformanceMetrics { tokens_per_second: 0.0, total_latency_ms: 100, ..Default::default() };
    let result = InferenceResult::new("".to_string(), 0, 100, 0.0, perf);
    assert!(
        !result.is_performance_acceptable(),
        "zero tokens_per_second must be unacceptable"
    );
}
