...
impl PerformanceMeasurer {
    ...
    fn get_metrics(&self) -> PerformanceMetrics {
        ...
        PerformanceMetrics { latency: elapsed, throughput, memory_peak: 0, cpu_usage: 0.0 }
    }
}
mod latency_tests {
    use super::*;
...
mod memory_tests {
    use super::*;
    async fn create_test_engine() -> InferenceEngine {
        let model = Arc::new(MockModel::new().with_memory_usage(1024 * 1024));
        let tokenizer = Arc::new(MockTokenizer::new());
        let device = Device::Cpu;
        InferenceEngine::new(model, tokenizer, device).unwrap()
    }
    ...
    async fn test_memory_usage_with_different_context_lengths() {
        ...
        for context_length in context_lengths {
            let config = InferenceConfig::default().with_memory_pool_size(context_length * 1024);
            let model = Arc::new(MockModel::new());
            let tokenizer = Arc::new(MockTokenizer::new());
            let device = Device::Cpu;
            let engine = InferenceEngine::with_config(model, tokenizer, device, config).unwrap();
            let prompt = "a".repeat(context_length / 4);
            let result = engine.generate(&prompt).await;
            assert!(result.is_ok());
            let stats = engine.get_stats().await;
