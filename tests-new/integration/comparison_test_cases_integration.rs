...
async fn test_comparison_test_case_registry() {
    ...
    assert!(registry.get("regression_tokenization_consistency").is_some());
...
async fn test_edge_case_suite() {
    ...
    assert!(edge_suite.iter().any(|tc| tc.name == "edge_very_long_input"));
    assert!(edge_suite.iter().any(|tc| tc.name == "edge_multilingual"));

    // Test empty input case specifically
    let empty_test = edge_suite.iter().find(|tc| tc.name == "edge_empty_input").unwrap();
    assert_eq!(empty_test.input, "");
    assert_eq!(empty_test.expected_min_tokens, Some(0));

    // Test very long input case
    let long_test = edge_suite.iter().find(|tc| tc.name == "edge_very_long_input").unwrap();
...
async fn test_performance_suite() {
    ...
    assert!(perf_suite.iter().any(|tc| tc.name == "perf_long_generation"));
    assert!(perf_suite.iter().any(|tc| tc.name == "perf_batch_simulation"));
    assert!(perf_suite.iter().any(|tc| tc.name == "perf_memory_stress"));

    // Test long generation case specifically
    let long_gen = perf_suite.iter().find(|tc| tc.name == "perf_long_generation").unwrap();
    ...
    let creativity = perf_suite.iter().find(|tc| tc.name == "perf_high_temp_creativity").unwrap();
...
async fn test_regression_suite() {
    ...
    assert!(regression_suite.iter().any(|tc| tc.name == "regression_tokenization_consistency"));
    assert!(regression_suite.iter().any(|tc| tc.name == "regression_memory_management"));
    assert!(regression_suite.iter().any(|tc| tc.name == "regression_float_precision"));
    assert!(regression_suite.iter().any(|tc| tc.name == "regression_context_window"));

    // Test stop token handling
    let stop_tokens =
        regression_suite.iter().find(|tc| tc.name == "regression_stop_tokens").unwrap();
...
async fn test_format_compatibility_suite() {
    ...
    assert!(format_suite.iter().any(|tc| tc.name == "format_gguf_compatibility"));
    assert!(format_suite.iter().any(|tc| tc.name == "format_safetensors_compatibility"));
    assert!(format_suite.iter().any(|tc| tc.name == "format_quantization_compatibility"));
...
async fn test_model_size_suite() {
    ...
    assert!(size_suite.iter().any(|tc| tc.name == "size_tiny_model_limits"));
    assert!(size_suite.iter().any(|tc| tc.name == "size_small_model_scaling"));
    assert!(size_suite.iter().any(|tc| tc.name == "size_medium_model_capabilities"));
    assert!(size_suite.iter().any(|tc| tc.name == "size_large_model_stress"));

    // Test large model stress test
    let large_test = size_suite.iter().find(|tc| tc.name == "size_large_model_stress").unwrap();
...
async fn test_comprehensive_suite() {
    ...
    let categories = ["basic_", "edge_", "perf_", "regression_", "format_", "size_"];

    for category_prefix in &categories {
        assert!(
            comprehensive_suite.iter().any(|tc| tc.name.starts_with(category_prefix)),
...
async fn test_suite_for_model_size() {
    ...
    assert!(medium_suite.iter().any(|tc| tc.name == "perf_long_generation"));
    assert!(medium_suite.iter().any(|tc| tc.name == "edge_very_long_input"));
...
async fn test_test_case_properties() {
    ...
    for test_case in all_tests {
        ...
        {
            assert!(min <= max, "Test case '{}' has min_tokens > max_tokens", test_case.name);
            assert!(
        ...
        let valid_prefixes = ["basic_", "edge_", "perf_", "regression_", "format_", "size_"];
        assert!(
            valid_prefixes.iter().any(|prefix| test_case.name.starts_with(prefix)),
...
async fn test_comparison_test_runner_creation() {
    ...
    assert!(runner_result.is_ok(), "Failed to create test runner: {:?}", runner_result.err());
